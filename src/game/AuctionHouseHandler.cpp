...
void WorldSession::HandleAuctionSellItem(WorldPacket& recv_data)
{
	...
	if (sWorld.getConfig(CONFIG_BOOL_SAFE_LOCK) && pl->GetPlayerSafe() == 0)
	{
		ChatHandler(pl).PSendSysMessage(LANG_SAFE_20);
		return;
	}
	...
	{
		sWorld.SendWorldText(4001, GetPlayerName(), it->GetProto()->Name1, it->GetCount(), int32(buyout / 10000));
	}
...
void WorldSession::HandleAuctionListItems(WorldPacket& recv_data)
{
	...
	if (sWorld.getConfig(CONFIG_BOOL_SAFE_LOCK) && _player->GetPlayerSafe() == 0)
	{
		ChatHandler(_player).PSendSysMessage(LANG_SAFE_20);
		return;
	}
