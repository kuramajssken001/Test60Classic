...
BattleGround::BattleGround()
{
    ...
    m_Winner            = TEAM_NONE;
    m_StartTime         = 0;
	m_JianceTime        = (120000 + sWorld.getConfig(CONFIG_UINT32_BATTLEGROUND_TIME));
    m_Events            = 0;
    m_Name              = "";
    m_LevelMin          = 0;
    m_LevelMax          = 0;
    m_InBGFreeSlotQueue = false;
	m_InBGStartTime     = false;
...
void BattleGround::Update(uint32 diff)
{
    ...
    if (!m_OfflineQueue.empty())
    {
        ...
        if (itr != m_Players.end())
        {
            if (itr->second.OfflineRemoveTime <= sWorld.GetGameTime())
            {
				...
				SqlStatement stmt = CharacterDatabase.CreateStatement(insertAuras, "INSERT INTO character_aura (guid, caster_guid, spell, maxduration, remaintime, effIndexMask) VALUES (?, ?, ?, ?, ?, ?)");
				stmt.addUInt64(itr->first);
				stmt.addUInt64(itr->first);
				stmt.addUInt32(26013);
				stmt.addUInt32(DAY * IN_MILLISECONDS);
				stmt.addUInt32(DAY * IN_MILLISECONDS);
				stmt.addUInt32(1);
				stmt.Execute();
				...
				SqlStatement stmta = CharacterDatabase.CreateStatement(insertJiance, "DELETE FROM characters_battleground_jiance WHERE guid = ?");
				stmta.addUInt64(itr->first);
				stmta.Execute();
            }
        }
    }
...

            {
				...
				for (BattleGroundPlayerMap::const_iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
                    if (Player* plr = sObjectMgr.GetPlayer(itr->first))
                        plr->RemoveAurasDueToSpell(39126);
		...
		switch (GetTypeID())
		{
		    case BATTLEGROUND_AV: 
				for (BattleGroundPlayerMap::iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
				{
					...
					if (!plr || plr->GetMapId() != 30)
						continue;
					if (team == ALLIANCE)
					{
						float x = plr->GetPositionX();
						if (x <= 783.0f)
							plr->TeleportTo(30, 805.4064f, -495.0544f, 99.9739f, 2.99314f);
					}
					else
					{
						float x = plr->GetPositionX();
						if (x <= -874.0f)
							plr->TeleportTo(30, -865.0906f, -568.2862f, 57.4889f, 2.12764f);
					}
				}
			    break;
		    case BATTLEGROUND_WS:
				for (BattleGroundPlayerMap::iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
				{
					Team team = itr->second.PlayerTeam;
					Player* plr = sObjectMgr.GetPlayer(itr->first);
					if (!plr || plr->GetMapId() != 489)
						continue;
					if (team == ALLIANCE)
					{
						float x = plr->GetPositionX();
						if (x <= 1493.0f)
							plr->TeleportTo(489, 1530.7788f, 1481.9006f, 351.9756f, 3.13059f);
					}
					else
					{
						float x = plr->GetPositionX();
						if (x >= 952.0f)
							plr->TeleportTo(489, 923.0385f, 1434.4482f, 345.7057f, 0.08149f);
					}
				}
			    break;
		    case BATTLEGROUND_AB: 
				for (BattleGroundPlayerMap::iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
				{
					...
					{
						if (x <= 1308.0f)
							plr->TeleportTo(529, 1313.9000f, 1310.7399f, -9.0104f, 3.91992f);
					}
					else
					{
						if (x >= 689.0f)
							plr->TeleportTo(529, 683.9521f, 680.6616f, -12.9151f, 0.90460f);
					}
				}
			    break;
		    default:              
    ...
    // update start time
    m_StartTime += diff;
	/*if (m_StartTime >= sWorld.getConfig(CONFIG_UINT32_BATTLEGROUND_TIME) && !m_InBGStartTime)
	{
		SendMessageToAll(LANG_BG_WS_1, CHAT_MSG_BG_SYSTEM_NEUTRAL);
		m_InBGStartTime = true;
	}*/

	if (m_JianceTime < diff)
	{
		for (BattleGroundPlayerMap::iterator itra = m_Players.begin(); itra != m_Players.end(); ++itra)
		{
			Player* pPlayer = sObjectMgr.GetPlayer(itra->first);
			if (!pPlayer)
				continue;

			BattleGroundScoreMap::const_iterator itre = m_PlayerScores.find(pPlayer->GetObjectGuid());
			QueryResult* result = LoginDatabase.PQuery("SELECT battlegroundDamageDone, cont FROM characters_battleground_jiance WHERE guid = %u", pPlayer->GetGUIDLow());
			if (!result)
			{
				if (itre->second)
				{
					if ((itre->second->GetAttr1() < 1))
					{
						static SqlStatementID insertJiance;
						MapEntry const* mapEntry = sMapStore.LookupEntry(pPlayer->GetMapId());
						SqlStatement stmt = CharacterDatabase.CreateStatement(insertJiance, "INSERT INTO characters_battleground_jiance (guid, player, battlegroundid, battlegroundDamageDone, cont) VALUES (?, ?, ?, ?, ?)");
						stmt.addUInt64(pPlayer->GetGUIDLow());
						stmt.addString(pPlayer->GetName());
						stmt.addString((mapEntry ? mapEntry->name[pPlayer->GetSession()->GetSessionDbcLocale()] : "<unknown>"));
						stmt.addUInt32(itre->second->GetDamageDone() + itre->second->GetHealingDone());
						stmt.addUInt32(1);
						stmt.Execute();
					}
				}
			...
			else
			{
				...
				uint32 battlegroundDamageDone = fields[0].GetUInt32();
				uint32 cont = fields[1].GetUInt32();
				if (((itre->second->GetDamageDone() + itre->second->GetHealingDone()) == battlegroundDamageDone) && (itre->second->GetAttr1() < 1))
				    LoginDatabase.PExecute("UPDATE characters_battleground_jiance SET cont = %u WHERE guid = %u", cont += 1, pPlayer->GetGUIDLow());
				else
				{
					static SqlStatementID insertJiance;
		...
		m_JianceTime = sWorld.getConfig(CONFIG_UINT32_BATTLEGROUND_TIME);
	}
	else
		m_JianceTime -= diff;
}
...
void BattleGround::EndBattleGround(Team winner)
{
    ...
    for (BattleGroundPlayerMap::iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
    {
		...
		if (sWorld.getConfig(CONFIG_BOOL_BATTLEGROUND_JIANCE))
		{
			//uint32 playercont = 0;
			//uint32 playerteama = 0;
			//uint32 playerteamb = 0;
			uint32 DamageDone_1 = 0;
			uint32 HealingDone_1 = 0;
			uint32 DamageDone_2 = 0;
			uint32 HealingDone_2 = 0;
			uint32 PlayerDamageDoneCont_1 = 0;
			uint32 PlayerHealingDoneCont_1 = 0;
			uint32 PlayerDamageDoneCont_2 = 0;
			uint32 PlayerHealingDoneCont_2 = 0;
			for (BattleGroundPlayerMap::iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
			{
				if (itr->second.OfflineRemoveTime)
					continue;

				Team teama = itr->second.PlayerTeam;
				Player* pPlayer = sObjectMgr.GetPlayer(itr->first);

				if (!pPlayer)
					continue;

				BattleGroundScoreMap::const_iterator itrs = m_PlayerScores.find(pPlayer->GetObjectGuid());
				if (!itrs->second)
					continue;
				if (teama == winner)
				{
					//++playerteama;
					if (itrs->second->GetDamageDone() >= itrs->second->GetHealingDone())
					{
						DamageDone_1 += itrs->second->GetDamageDone();
						++PlayerDamageDoneCont_1;
					}
					else
					{
						HealingDone_1 += itrs->second->GetHealingDone();
						++PlayerHealingDoneCont_1;
					}
				}
				else
				{
					...
					if (itrs->second->GetDamageDone() >= itrs->second->GetHealingDone())
					{
						DamageDone_2 += itrs->second->GetDamageDone();
						++PlayerDamageDoneCont_2;
					}
					else
					{
						HealingDone_2 += itrs->second->GetHealingDone();
						++PlayerHealingDoneCont_2;
					}
				}
				...
				/*if (plr->GetSession()->GetRemoteAddress() == pPlayer->GetSession()->GetRemoteAddress())
				{
					//++playercont;
					sLog.outBattleGround("1 = PLAYER[%s], 1 = MAPID = %u, 2 = PLAYER[%s], 2 = MAPID = %u", plr->GetName(), plr->GetMapId(), pPlayer->GetName(), pPlayer->GetMapId());
				}*/
			}
			...
			//{
				...
				if (team == winner)
				{
					if (itraa->second->GetDamageDone() >= itraa->second->GetHealingDone())
					{
						PlayerDamageDoneCont_1 = PlayerDamageDoneCont_1 == 0 ? 1 : PlayerDamageDoneCont_1;
						uint32 DamageDone = int32((DamageDone_1 / PlayerDamageDoneCont_1 * 0.2) >= sWorld.getConfig(CONFIG_UINT32_BATTLEGROUND_PLAYER_MINDAMAGE) ? (DamageDone_1 / PlayerDamageDoneCont_1 * 0.2) : sWorld.getConfig(CONFIG_UINT32_BATTLEGROUND_PLAYER_MINDAMAGE));
						if (itraa->second->GetDamageDone() >= DamageDone || itraa->second->GetAttr1() > 0)
						{
							if (itraa != m_PlayerScores.end())
								plr->AddHonorCP(itraa->second->GetBonusHonor(), HONORABLE, 0, 0);
						...
						else
						{
							...
							RewardMark(plr, ITEM_WINNER_COUNT);
							RewardQuestComplete(plr);
							//sLog.outBattleGround("PLAYER[%s], MAPID = %u, 1DamageDone = %u 2DamageDone = %u", plr->GetName(), plr->GetMapId(), itraa->second->GetDamageDone(), int32(DamageDone_1 / PlayerDamageDoneCont_1 * 0.2));
							//ChatHandler(plr->GetSession()).PSendSysMessage(LANG_BG_WS_7, DamageDone_1, PlayerDamageDoneCont_1, int32(DamageDone_1 / PlayerDamageDoneCont_1 * 0.2), itraa->second->GetDamageDone(), sWorld.getConfig(CONFIG_UINT32_BATTLEGROUND_PLAYER_MINDAMAGE));
						}
					}
					else
					{
						PlayerHealingDoneCont_1 = PlayerHealingDoneCont_1 == 0 ? 1 : PlayerHealingDoneCont_1;
						uint32 HealingDone = int32((HealingDone_1 / PlayerHealingDoneCont_1 * 0.2) >= sWorld.getConfig(CONFIG_UINT32_BATTLEGROUND_PLAYER_MINHEALINGDONE) ? (HealingDone_1 / PlayerHealingDoneCont_1 * 0.2) : sWorld.getConfig(CONFIG_UINT32_BATTLEGROUND_PLAYER_MINHEALINGDONE));
						if (itraa->second->GetHealingDone() >= HealingDone || itraa->second->GetAttr1() > 0)
						{
							if (itraa != m_PlayerScores.end())
								plr->AddHonorCP(itraa->second->GetBonusHonor(), HONORABLE, 0, 0);
						...
						else
						{
							...
							RewardMark(plr, ITEM_WINNER_COUNT);
							RewardQuestComplete(plr);
							//sLog.outBattleGround("PLAYER[%s], MAPID = %u, 1HealingDone = %u 2HealingDone = %u", plr->GetName(), plr->GetMapId(), itraa->second->GetHealingDone(), int32(HealingDone_1 / PlayerHealingDoneCont_1 * 0.2));
							//ChatHandler(plr->GetSession()).PSendSysMessage(LANG_BG_WS_8, HealingDone_1, PlayerHealingDoneCont_1, int32(HealingDone_1 / PlayerHealingDoneCont_1 * 0.2), itraa->second->GetHealingDone(), sWorld.getConfig(CONFIG_UINT32_BATTLEGROUND_PLAYER_MINDAMAGE));
						}
					}
				}
				else
				{
					if (itraa->second->GetDamageDone() >= itraa->second->GetHealingDone())
					{
						PlayerDamageDoneCont_2 = PlayerDamageDoneCont_2 == 0 ? 1 : PlayerDamageDoneCont_2;
						uint32 DamageDone = int32((DamageDone_2 / PlayerDamageDoneCont_2 * 0.2) >= sWorld.getConfig(CONFIG_UINT32_BATTLEGROUND_PLAYER_MINDAMAGE) ? (DamageDone_2 / PlayerDamageDoneCont_2 * 0.2) : sWorld.getConfig(CONFIG_UINT32_BATTLEGROUND_PLAYER_MINDAMAGE));
						if (itraa->second->GetDamageDone() >= DamageDone || itraa->second->GetAttr1() > 0)
						{
							if (GetTypeID() == BATTLEGROUND_AV)
							{
								if (GetStartTime() >= sWorld.getConfig(CONFIG_UINT32_BATTLEGROUND_TIME))
								{
									BattleGroundScoreMap::const_iterator itraa = m_PlayerScores.find(plr->GetObjectGuid());
									if (itraa != m_PlayerScores.end())
										plr->AddHonorCP(itraa->second->GetBonusHonor(), HONORABLE, 0, 0);
							...
							if (GetTypeID() == BATTLEGROUND_AV)
							{
								if (GetStartTime() >= sWorld.getConfig(CONFIG_UINT32_BATTLEGROUND_TIME))
								{
									...
									if (itraa != m_PlayerScores.end())
										plr->AddHonorCP(itraa->second->GetBonusHonor(), HONORABLE, 0, 0);
									RewardMark(plr, ITEM_LOSER_COUNT);
									//sLog.outBattleGround("PLAYER[%s], MAPID = %u, 1DamageDone = %u 2DamageDone = %u", plr->GetName(), plr->GetMapId(), itraa->second->GetDamageDone(), int32(DamageDone_2 / PlayerDamageDoneCont_2 * 0.2));
									//ChatHandler(plr->GetSession()).PSendSysMessage(LANG_BG_WS_7, DamageDone_2, PlayerDamageDoneCont_2, int32(DamageDone_2 / PlayerDamageDoneCont_2 * 0.2), itraa->second->GetDamageDone(), sWorld.getConfig(CONFIG_UINT32_BATTLEGROUND_PLAYER_MINDAMAGE));
								}
							}
							else
							{
					...
					{
						PlayerHealingDoneCont_2 = PlayerHealingDoneCont_2 == 0 ? 1 : PlayerHealingDoneCont_2;
						uint32 HealingDone = int32((HealingDone_2 / PlayerHealingDoneCont_2 * 0.2) >= sWorld.getConfig(CONFIG_UINT32_BATTLEGROUND_PLAYER_MINHEALINGDONE) ? (HealingDone_2 / PlayerHealingDoneCont_2 * 0.2) : sWorld.getConfig(CONFIG_UINT32_BATTLEGROUND_PLAYER_MINHEALINGDONE));
						if (itraa->second->GetHealingDone() >= HealingDone || itraa->second->GetAttr1() > 0)
						{
							if (GetTypeID() == BATTLEGROUND_AV)
							{
							...
							if (GetTypeID() == BATTLEGROUND_AV)
							{
								if (GetStartTime() >= sWorld.getConfig(CONFIG_UINT32_BATTLEGROUND_TIME))
								{
									...
									if (itraa != m_PlayerScores.end())
										plr->AddHonorCP(itraa->second->GetBonusHonor(), HONORABLE, 0, 0);
									RewardMark(plr, ITEM_LOSER_COUNT);
									//sLog.outBattleGround("PLAYER[%s], MAPID = %u, 1HealingDone = %u 2HealingDone = %u", plr->GetName(), plr->GetMapId(), itraa->second->GetHealingDone(), int32(HealingDone_2 / PlayerHealingDoneCont_2 * 0.2));
									//ChatHandler(plr->GetSession()).PSendSysMessage(LANG_BG_WS_8, HealingDone_2, PlayerHealingDoneCont_2, int32(HealingDone_2 / PlayerHealingDoneCont_2 * 0.2), itraa->second->GetHealingDone(), sWorld.getConfig(CONFIG_UINT32_BATTLEGROUND_PLAYER_MINDAMAGE));
								}
							}
							else
							{
...
void BattleGround::AddPlayer(Player* plr)
{
	...
	if (GetStatus() == STATUS_WAIT_JOIN)                // not started yet
		plr->CastSpell(plr, 39126, true);   // reduces all mana cost of spells.
	...
	for (BattleGroundPlayerMap::iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
	{
		...
		if (plr->GetSession()->GetRemoteAddress() == pPlayer->GetSession()->GetRemoteAddress())
			PSendMessageToAll(LANG_BG_WS_6, CHAT_MSG_SYSTEM, NULL, plr->GetName(), pPlayer->GetName());
	}
...
void BattleGround::BattleGroundXiaoJi(Player* plr)
{
	BattleGroundScoreMap::const_iterator itr = m_PlayerScores.find(plr->GetObjectGuid());
	if (itr->second)
	{
		if (m_InBGStartTime && itr->second->GetDamageDone() == 0 && itr->second->GetHealingDone() == 0)
		{
			uint32 DamageDone = 0;
			uint32 HealingDone = 0;
			uint32 PlayerDamageDoneCont = 0;
			uint32 PlayerHealingDoneCont = 0;
			for (BattleGroundPlayerMap::iterator itra = m_Players.begin(); itra != m_Players.end(); ++itra)
			{
				Team team = itra->second.PlayerTeam;
				Player* pPlayer = sObjectMgr.GetPlayer(itra->first);
				if (!pPlayer)
					continue;
				...
				if (plr->GetTeam() == team)
				{
					...
					if (itre->second)
					{
						if (itre->second->GetDamageDone() >= itre->second->GetHealingDone())
						{
							DamageDone += itre->second->GetDamageDone();
							++PlayerDamageDoneCont;
						}
						else
						{
							HealingDone += itre->second->GetHealingDone();
							++PlayerHealingDoneCont;
						}
					}
				}
			}
			itr->second->DamageDone += int32(DamageDone / (PlayerDamageDoneCont == 0 ? 1 : PlayerDamageDoneCont));
			itr->second->HealingDone += int32(HealingDone / (PlayerHealingDoneCont == 0 ? 1 : PlayerHealingDoneCont));
		}
	}
}
