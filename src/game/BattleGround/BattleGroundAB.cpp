    ...
    if (GetStatus() == STATUS_IN_PROGRESS)
    {
        ...
        for (uint8 team = 0; team < PVP_TEAM_COUNT; ++team)
        {
            ...
            if (m_lastTick[team] > BG_AB_TickIntervals[points])
            {
                ...
                {
					RewardHonorToTeam(BG_AB_WinMatchHonor[5], (team == TEAM_INDEX_ALLIANCE) ? ALLIANCE : HORDE);
...
void BattleGroundAB::Reset()
{
	...
	m_honorTicks = !sWorld.BattleGroundWeek() ? AB_NORMAL_HONOR_INTERVAL : AB_WEEKEND_HONOR_INTERVAL;
	m_ReputationTics = !sWorld.BattleGroundWeek() ? AB_NORMAL_REPUTATION_INTERVAL : AB_WEEKEND_REPUTATION_INTERVAL;
