	...
	/*switch (bgTypeId)
	{
		case BATTLEGROUND_AV:
			if (sWorld.GetDateToday() == sWorld.GetDateLastMaintenanceDayXp1() || sWorld.GetDateToday() == sWorld.GetDateLastMaintenanceDayXp2())
			{
				if (_player->GetGroup())
				{
					...
					{
						ChatHandler(_player).PSendSysMessage(LANG_CHANNEL_17);
						return;
					}
					for (GroupReference* itr = pGroup->GetFirstMember(); itr != NULL; itr = itr->next())
					{
						...
						{
							ChatHandler(_player).PSendSysMessage(LANG_CHANNEL_18, pPlayer->GetName());
							return;
						}

						if (pPlayer->GetDummyAura(26013))
							ChatHandler(_player).PSendSysMessage(LANG_CHANNEL_15, pPlayer->GetName());

						if (pPlayer->InBattleGround())
							ChatHandler(_player).PSendSysMessage(LANG_CHANNEL_16, pPlayer->GetName());
					}
					joinAsGroup = 1;
				}
			}
			...
			{
				ChatHandler(_player).PSendSysMessage(LANG_CHANNEL_14);
				return;
			}
			break;
		case BATTLEGROUND_WS:
			...
			if (sWorld.GetDateToday() == sWorld.GetDateLastMaintenanceDayXp2())
			{
				...
				if (sWorld.battleground_time_Start1 != 0 && sWorld.battleground_time_End1 != 0)
				{
					if (time_Start1 >= sWorld.GetGameTime() || time_End1 <= sWorld.GetGameTime())
					{
						ChatHandler(_player).PSendSysMessage(LANG_CHANNEL_19);
						return;
					}
				}
			}
		...
		case BATTLEGROUND_AB:
			...
			if (sWorld.GetDateToday() == sWorld.GetDateLastMaintenanceDayXp1())
			{
				...
				if (sWorld.battleground_time_Start1 != 0 && sWorld.battleground_time_End1 != 0)
				{
					if (time_Start1 >= sWorld.GetGameTime() || time_End1 <= sWorld.GetGameTime())
					{
						ChatHandler(_player).PSendSysMessage(LANG_CHANNEL_20);
						return;
					}
				}
			}
			...
			/*if (_player->GetGroup())
			{
				ChatHandler(_player).PSendSysMessage(LANG_CHANNEL_10);
				return;
			}
			break;
	}

	if (sWorld.GetBattleGroundTime(3, 1) == 0)
	{
		if (sWorld.GetBattleGroundTime(1, 1) == 0)
		{
			ChatHandler(_player).PSendSysMessage(LANG_JIAOBEN_21);
			return;
		}
	}*/
