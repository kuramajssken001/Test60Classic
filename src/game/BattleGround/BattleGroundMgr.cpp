...
bool BattleGroundQueue::InviteGroupToBG(GroupQueueInfo* ginfo, BattleGround* bg, Team side)
{
    ...
    if (!ginfo->IsInvitedToBGInstanceGUID)
    {
		...
		if ((bg->GetPlayersCountByTeam(side) + uint32(ginfo->Players.size())) > bg->GetMaxPlayersPerTeam())
			return false;
        ...
        for (GroupQueueInfoPlayers::iterator itr = ginfo->Players.begin(); itr != ginfo->Players.end(); ++itr)
        {
			...
			if (BattleGround* currentBg = plr->GetBattleGround())
				currentBg->RemovePlayerAtLeave(plr->GetObjectGuid(), false, true);
			plr->SetBattleGroundId(bg->GetInstanceID(), bgTypeId);
			plr->SetBGTeam(ginfo->GroupTeam);
			...
			for (uint32 i = 1; i <= 3; ++i)
			{
				...
				BattleGround* bga = sBattleGroundMgr.GetBattleGroundTemplate(BattleGroundTypeId(i));
				if (bga)
				{
					BattleGroundQueue& bgQueuea = sBattleGroundMgr.m_BattleGroundQueues[BattleGroundQueueTypeId(i)];
					if (bgQueuea.PlayerBattleGroundQueue(plr->GetObjectGuid()))
					{
						uint32 queueSlota = plr->GetBattleGroundQueueIndex(BattleGroundQueueTypeId(i));
						plr->RemoveBattleGroundQueueId(BattleGroundQueueTypeId(i));
						sBattleGroundMgr.BuildBattleGroundStatusPacket(&data, bga, queueSlota, STATUS_NONE, 0, 0);
						bgQueuea.RemovePlayer(plr->GetObjectGuid(), true);
						sBattleGroundMgr.ScheduleQueueUpdate(BattleGroundQueueTypeId(i), BattleGroundTypeId(i), plr->GetBattleGroundBracketIdFromLevel(BattleGroundTypeId(i)));
						plr->GetSession()->SendPacket(&data);
					}
				}
			}
			if (plr->HasAura(8219))
				plr->RemoveAurasDueToSpell(8219);
			if (plr->HasAura(8220))
				plr->RemoveAurasDueToSpell(8220);
			if (plr->HasAura(8221))
	...
	*data << uint32(scoreCount); 
	...
	for (BattleGround::BattleGroundScoreMap::const_iterator itr = itr2; itr != bg->GetPlayerScoresEnd();)
    {
		...
		if (!bg->IsPlayerInBattleGround(itr2->first))
            continue;
		const BattleGroundScore* score = itr2->second;
        *data << ObjectGuid(itr2->first);
...
bool BattleGroundQueue::PlayerBattleGroundQueue(ObjectGuid guid)
{
	QueuedPlayersMap::iterator itr;
	itr = m_QueuedPlayers.find(guid);
	if (itr == m_QueuedPlayers.end())
