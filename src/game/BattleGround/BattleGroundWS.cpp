...
void BattleGroundWS::AddPlayer(Player* plr)
{
	...
	//BattleGround::BattleGroundXiaoJi(plr);
}
...
void BattleGroundWS::EventPlayerCapturedFlag(Player* source)
{
	...
	RewardHonorToTeam(sWorld.BattleGroundWeek() ? BG_WSG_FlagCapturedHonor[3] : BG_WSG_FlagCapturedHonor[5], source->GetTeam());//Õ½¸èÈÙÓþ
...
void BattleGroundWS::EventPlayerClickedOnFlag(Player* source, GameObject* target_obj)
{
	...
	if (GetFlagState(ALLIANCE) == BG_WS_FLAG_STATE_ON_GROUND && source->IsWithinDistInMap(target_obj, 10) && (target_obj->GetEntry() == 179785 || target_obj->GetEntry() == 179830))
	{
		if (source->GetTeam() == ALLIANCE)
		{
		...
		else
		{
			...
			UpdateFlagState(HORDE, BG_WS_FLAG_STATE_ON_PLAYER);
			UpdateWorldState(BG_WS_FLAG_UNK_ALLIANCE, 1);
			if (((Player*)source)->HasAuraType(SPELL_AURA_MOUNTED))
				((Player*)source)->RemoveSpellsCausingAura(SPELL_AURA_MOUNTED);
		}
	...
	if (GetFlagState(HORDE) == BG_WS_FLAG_STATE_ON_GROUND && source->IsWithinDistInMap(target_obj, 10) && (target_obj->GetEntry() == 179786 || target_obj->GetEntry() == 179831))
	{
...
void BattleGroundWS::Reset()
{
	...
	m_ReputationCapture = (sWorld.BattleGroundWeek()) ? 30 : 50;
...
void BattleGroundWS::EndBattleGround(Team winner)
{
	// win reward
	if (winner == ALLIANCE)
		RewardHonorToTeam(sWorld.BattleGroundWeek() ? BG_WSG_FlagCapturedHonor[3] : BG_WSG_FlagCapturedHonor[5], ALLIANCE);
	if (winner == HORDE)
		RewardHonorToTeam(sWorld.BattleGroundWeek() ? BG_WSG_FlagCapturedHonor[3] : BG_WSG_FlagCapturedHonor[5], HORDE);
