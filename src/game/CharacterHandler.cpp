...
void WorldSession::HandleCharCreateOpcode(WorldPacket& recv_data)
{
	...
	static SqlStatementID insertVip ;
	SqlStatement stmt = CharacterDatabase.CreateStatement(insertVip, "INSERT INTO characters_vip (guid) VALUES (?)");
	stmt.addUInt32(pNewChar->GetGUIDLow());
	stmt.Execute();

	/*static SqlStatementID insertquest;
	SqlStatement stmtd = CharacterDatabase.CreateStatement(insertquest, "INSERT INTO characters_quest (guid) VALUES (?)");
	stmtd.addUInt32(pNewChar->GetGUIDLow());
	stmtd.Execute();

	static SqlStatementID insertGongNeng ;
	SqlStatement stmta = CharacterDatabase.CreateStatement(insertGongNeng, "INSERT INTO characters_gongneng (guid) VALUES (?)");
	stmta.addUInt32(pNewChar->GetGUIDLow());
	stmta.Execute();

	static SqlStatementID insertjiangli ;
	SqlStatement stmtb = CharacterDatabase.CreateStatement(insertjiangli, "INSERT INTO characters_jiangli (guid) VALUES (?)");
	stmtb.addUInt32(pNewChar->GetGUIDLow());
	stmtb.Execute();

	static SqlStatementID inserthjrsquest ;
	SqlStatement stmtc = CharacterDatabase.CreateStatement(inserthjrsquest, "INSERT INTO characters_hjrs_quest (guid) VALUES (?)");
	stmtc.addUInt32(pNewChar->GetGUIDLow());
	stmtc.Execute();*/
...
void WorldSession::HandleCharDeleteOpcode(WorldPacket& recv_data)
{
	...
	static SqlStatementID deleteVip;
	SqlStatement stmt = CharacterDatabase.CreateStatement(deleteVip, "DELETE FROM characters_vip WHERE guid = ?");
    stmt.PExecute(lowguid);

	/*static SqlStatementID deletequest;
	SqlStatement stmtd = CharacterDatabase.CreateStatement(deletequest, "DELETE FROM characters_quest WHERE guid = ?");
	stmtd.PExecute(lowguid);

	static SqlStatementID deleteGongNeng;
	SqlStatement stmta = CharacterDatabase.CreateStatement(deleteGongNeng, "DELETE FROM characters_gongneng WHERE guid = ?");
    stmta.PExecute(lowguid);

	static SqlStatementID deletejiangli;
	SqlStatement stmtb = CharacterDatabase.CreateStatement(deletejiangli, "DELETE FROM characters_jiangli WHERE guid = ?");
    stmtb.PExecute(lowguid);

	static SqlStatementID deletehjrsquest;
	SqlStatement stmtc = CharacterDatabase.CreateStatement(deletehjrsquest, "DELETE FROM characters_hjrs_quest WHERE guid = ?");
    stmtc.PExecute(lowguid);*/
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder* holder)
{
    ...
    {
		if (lockStatus == AREA_LOCKSTATUS_ZONE_IN_COMBAT)
		{
			...
			if (Group* pGroup = pCurrChar->GetGroup())
			{
				...
				for (GroupReference* itr = pGroup->GetFirstMember(); itr != NULL; itr = itr->next())
				{
					...
					if (!pPlayera || pPlayera == pCurrChar || pPlayera->isDead())
						continue;

					if (pPlayera->GetMapId() != pCurrChar->GetMapId())
						continue;
	...
	/*if (pCurrChar->GetVip(26) == 1)
	{
		pCurrChar->SetVip(2, 26);
		if (pCurrChar->GetTeam() == ALLIANCE)
			pCurrChar->m_taxi.LoadTaxiMask("3456411898 2148078929 49991 0 0 0 0 0 ");
		else
			pCurrChar->m_taxi.LoadTaxiMask("830150144 315656864 56504 0 0 0 0 0 ");
	}*/

	if (sWorld.getConfig(CONFIG_BOOL_ALL_EXPLORED))
	{
	...
	//LoginDatabase.PExecute("UPDATE characters_gongneng SET account_ip = '%s' WHERE guid = '%u'", IP_str.c_str(), pCurrChar->GetGUIDLow());
	pCurrChar->PlayerLogin();

    if (!pCurrChar->IsStandState() && !pCurrChar->hasUnitState(UNIT_STAT_STUNNED))
        pCurrChar->SetStandState(UNIT_STAND_STATE_STAND);

	if (pCurrChar->HasAura(22799))
		pCurrChar->RemoveAurasDueToSpell(22799);

	if (sWorld.getConfig(CONFIG_BOOL_SAFE_LOCK))
	    if (pCurrChar->GetSafePass() == "a")
		    ChatHandler(pCurrChar).PSendSysMessage(LANG_SAFE_23);

	if (pCurrChar->GetPlayerTalentSpellCost() > 51)
		pCurrChar->resetTalents(true);
	...
	if (sWorld.getConfig(CONFIG_BOOL_PLAYER_INSTANCES_PER_HOUR_DAKAI))
	{
		if (AreaTrigger const* at = sObjectMgr.GetMapEntranceTrigger(pCurrChar->GetMapId()))
		{
			if (at)
			{
				...
				uint32 cont = at->target_mapId == 429 ? 4 : sWorld.getConfig(CONFIG_UINT32_PLAYER_INSTANCES_PER_HOUR) + 1;
				if (mapEntry->IsNonRaidDungeon() && pCurrChar->GetPlayerInstancesCont() > cont)
					pCurrChar->TeleportToHomebind();
				else
				if (mapEntry->IsNonRaidDungeon() && pCurrChar->GetPlayerInstancesCont() != 0)
					ChatHandler(pCurrChar).PSendSysMessage(ZHANYOU_TIME_19, pCurrChar->m_Player_Instances[PLAYED_INSTANCES_CONT] > 5 ? 5 : pCurrChar->m_Player_Instances[PLAYED_INSTANCES_CONT], sWorld.getConfig(CONFIG_UINT32_PLAYER_INSTANCES_PER_HOUR), pCurrChar->GetDaysTime(pCurrChar->m_Player_Instances[PLAYED_INSTANCES_TIME], 1), pCurrChar->GetDaysTime(pCurrChar->m_Player_Instances[PLAYED_INSTANCES_TIME], 2), pCurrChar->GetDaysTime(pCurrChar->m_Player_Instances[PLAYED_INSTANCES_TIME], 3), pCurrChar->GetDaysTime(pCurrChar->m_Player_Instances[PLAYED_INSTANCES_TIME], 4));
			}
		}
	}

	if (pCurrChar->HasAura(26476) && pCurrChar->GetAreaId() == 3428)
	{
