...
Creature::Creature(CreatureSubtype subtype) : Unit(),
	...
	m_corpseDecayTimer(0), m_respawnTime(0), m_respawnDelay(25), m_corpseDelay(60), m_aggroDelay(0), m_respawnradius(5.0f), m_creatureEvadeModeTimer(1000), m_creatureEvadeModeCont(0),
	...
	m_meleeDamageSchoolMask(SPELL_SCHOOL_MASK_NORMAL), m_originalEntry(0), bossjiance(false),
    m_creatureInfo(NULL)
{
    ...
    switch (m_deathState)
    {
        ...
        case DEAD:
        {
            if (m_respawnTime <= time(NULL) && (!m_isSpawningLinked || GetMap()->GetCreatureLinkingHolder()->CanSpawn(this)))
            {
				...
				if (IsImmunedToDamage(SPELL_SCHOOL_MASK_ALL) && !IsTotem() && !bossjiance)
					ApplySpellImmune(0, IMMUNITY_DAMAGE, SPELL_SCHOOL_MASK_ALL, false);
            ...
            if (!IsInEvadeMode())
            {
                if (AI())
                {
					...
					if (getVictim() && isAlive())
						SetCreatureEvadeMode(getVictim(), diff);
                    m_AI_locked = false;
                }
            }
...
void Creature::DoFleeToGetAssistance()
{
    ...
    if (radius > 0)
    {
		...
		UpdateSpeed(MOVE_RUN, false, 0.66f);
...
void Creature::SelectLevel(const CreatureInfo* cinfo, float percentHealth /*= 100.0f*/)
{
	...
	case 309:
		...
		break;
	case 509:
		health *= _GetHealthMod(GetCreatureInfo()->Rank, 509);
		break;
	case 409:
		health *= _GetHealthMod(GetCreatureInfo()->Rank, 409);
		break;
	case 249:
		health *= _GetHealthMod(GetCreatureInfo()->Rank, 249);
		break;
	case 469:
		health *= _GetHealthMod(GetCreatureInfo()->Rank, 469);
		break;
	case 531:
		health *= _GetHealthMod(GetCreatureInfo()->Rank, 531);
		break;
	case 533:
		health *= _GetHealthMod(GetCreatureInfo()->Rank, 533);
		break;
	case 30:
		health *= _GetHealthMod(GetCreatureInfo()->Rank, 30);
		break;
	default:
		health *= _GetHealthMod(GetCreatureInfo()->Rank, 1000);
		break;
	}
    if (health < 1)
        health = 1;
	...
	case 309:
		...
		break;
	case 509:
		damagemod = _GetDamageMod(GetCreatureInfo()->Rank, 509);
		break;
	case 409:
		damagemod = _GetDamageMod(GetCreatureInfo()->Rank, 409);
		break;
	case 249:
		damagemod = _GetDamageMod(GetCreatureInfo()->Rank, 249);
		break;
	case 469:
		damagemod = _GetDamageMod(GetCreatureInfo()->Rank, 469);
		break;
	case 531:
		if (GetEntry() == 15517 || GetEntry() == 15589)
			damagemod = 1.3f;
		else
		    damagemod = _GetDamageMod(GetCreatureInfo()->Rank, 531);
		break;
	case 533:
		damagemod = _GetDamageMod(GetCreatureInfo()->Rank, 533);
		break;
	case 30:
		damagemod = _GetDamageMod(GetCreatureInfo()->Rank, 30);
		break;
	default:
		damagemod = _GetDamageMod(GetCreatureInfo()->Rank, 1000);
		break;
	}
...
float Creature::_GetHealthMod(int32 Rank, uint32 mapid)
{
	switch (mapid)
	{
		case 309:
		{
           switch (Rank)
           {
                case CREATURE_ELITE_NORMAL:
                    return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_NORMAL_HP_ZUG);
                case CREATURE_ELITE_ELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_HP_ZUG);
                case CREATURE_ELITE_RAREELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RAREELITE_HP_ZUG);
                case CREATURE_ELITE_WORLDBOSS:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_WORLDBOSS_HP_ZUG);
                case CREATURE_ELITE_RARE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RARE_HP_ZUG);
                default:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_HP_ZUG);
           }
		}break;
		case 509:
		{
           ...
           {
                case CREATURE_ELITE_NORMAL:
                    return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_NORMAL_HP_FX);
                case CREATURE_ELITE_ELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_HP_FX);
                case CREATURE_ELITE_RAREELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RAREELITE_HP_FX);
                case CREATURE_ELITE_WORLDBOSS:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_WORLDBOSS_HP_FX);
                case CREATURE_ELITE_RARE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RARE_HP_FX);
                default:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_HP_FX);
           }
		}break;
		case 409:
		{
           ...
           {
                case CREATURE_ELITE_NORMAL:
                    return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_NORMAL_HP_MC);
                case CREATURE_ELITE_ELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_HP_MC);
                case CREATURE_ELITE_RAREELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RAREELITE_HP_MC);
                case CREATURE_ELITE_WORLDBOSS:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_WORLDBOSS_HP_MC);
                case CREATURE_ELITE_RARE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RARE_HP_MC);
                default:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_HP_MC);
           }
		}break;
		case 249:
		{
           ...
           {
                case CREATURE_ELITE_NORMAL:
                    return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_NORMAL_HP_HL);
                case CREATURE_ELITE_ELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_HP_HL);
                case CREATURE_ELITE_RAREELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RAREELITE_HP_HL);
                case CREATURE_ELITE_WORLDBOSS:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_WORLDBOSS_HP_HL);
                case CREATURE_ELITE_RARE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RARE_HP_HL);
                default:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_HP_HL);
           }
		}break;
		case 469:
		{
           ...
           {
                case CREATURE_ELITE_NORMAL:
                    return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_NORMAL_HP_BWL);
                case CREATURE_ELITE_ELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_HP_BWL);
                case CREATURE_ELITE_RAREELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RAREELITE_HP_BWL);
                case CREATURE_ELITE_WORLDBOSS:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_WORLDBOSS_HP_BWL);
                case CREATURE_ELITE_RARE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RARE_HP_BWL);
                default:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_HP_BWL);
           }
		}break;
		case 531:
		{
           ...
           {
                case CREATURE_ELITE_NORMAL:
                    return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_NORMAL_HP_TAQ);
                case CREATURE_ELITE_ELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_HP_TAQ);
                case CREATURE_ELITE_RAREELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RAREELITE_HP_TAQ);
                case CREATURE_ELITE_WORLDBOSS:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_WORLDBOSS_HP_TAQ);
                case CREATURE_ELITE_RARE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RARE_HP_TAQ);
                default:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_HP_TAQ);
           }
		}break;
		case 533:
		{
           ...
           {
                case CREATURE_ELITE_NORMAL:
                    return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_NORMAL_HP_NAXX);
                case CREATURE_ELITE_ELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_HP_NAXX);
                case CREATURE_ELITE_RAREELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RAREELITE_HP_NAXX);
                case CREATURE_ELITE_WORLDBOSS:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_WORLDBOSS_HP_NAXX);
                case CREATURE_ELITE_RARE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RARE_HP_NAXX);
                default:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_HP_NAXX);
           }
		}break;
		case 30:
		{
           ...
           {
                case CREATURE_ELITE_NORMAL:
                    return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_NORMAL_HP_AS);
                case CREATURE_ELITE_ELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_HP_AS);
                case CREATURE_ELITE_RAREELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RAREELITE_HP_AS);
                case CREATURE_ELITE_WORLDBOSS:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_WORLDBOSS_HP_AS);
                case CREATURE_ELITE_RARE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RARE_HP_AS);
                default:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_HP_AS);
           }
		}break;
...
float Creature::_GetDamageMod(int32 Rank, uint32 mapid)
{
	switch (mapid)
	{
		case 309:
		{
           ...
           {
                case CREATURE_ELITE_NORMAL:
                    return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_NORMAL_DAMAGE_ZUG);
                case CREATURE_ELITE_ELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_DAMAGE_ZUG);
                case CREATURE_ELITE_RAREELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RAREELITE_DAMAGE_ZUG);
                case CREATURE_ELITE_WORLDBOSS:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_WORLDBOSS_DAMAGE_ZUG);
                case CREATURE_ELITE_RARE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RARE_DAMAGE_ZUG);
                default:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_DAMAGE_ZUG);
           }
		}break;
		case 509:
		{
           ...
           {
                case CREATURE_ELITE_NORMAL:
                    return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_NORMAL_DAMAGE_FX);
                case CREATURE_ELITE_ELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_DAMAGE_FX);
                case CREATURE_ELITE_RAREELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RAREELITE_DAMAGE_FX);
                case CREATURE_ELITE_WORLDBOSS:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_WORLDBOSS_DAMAGE_FX);
                case CREATURE_ELITE_RARE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RARE_DAMAGE_FX);
                default:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_DAMAGE_FX);
           }
		}break;
		case 409:
		{
           ...
           {
                case CREATURE_ELITE_NORMAL:
                    return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_NORMAL_DAMAGE_MC);
                case CREATURE_ELITE_ELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_DAMAGE_MC);
                case CREATURE_ELITE_RAREELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RAREELITE_DAMAGE_MC);
                case CREATURE_ELITE_WORLDBOSS:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_WORLDBOSS_DAMAGE_MC);
                case CREATURE_ELITE_RARE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RARE_DAMAGE_MC);
                default:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_DAMAGE_MC);
           }
		}break;
		case 249:
		{
           ...
           {
                case CREATURE_ELITE_NORMAL:
                    return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_NORMAL_DAMAGE_HL);
                case CREATURE_ELITE_ELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_DAMAGE_HL);
                case CREATURE_ELITE_RAREELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RAREELITE_DAMAGE_HL);
                case CREATURE_ELITE_WORLDBOSS:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_WORLDBOSS_DAMAGE_HL);
                case CREATURE_ELITE_RARE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RARE_DAMAGE_HL);
                default:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_DAMAGE_HL);
           }
		}break;
		case 469:
		{
           ...
           {
                case CREATURE_ELITE_NORMAL:
                    return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_NORMAL_DAMAGE_BWL);
                case CREATURE_ELITE_ELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_DAMAGE_BWL);
                case CREATURE_ELITE_RAREELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RAREELITE_DAMAGE_BWL);
                case CREATURE_ELITE_WORLDBOSS:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_WORLDBOSS_DAMAGE_BWL);
                case CREATURE_ELITE_RARE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RARE_DAMAGE_BWL);
                default:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_DAMAGE_BWL);
           }
		}break;
		case 531:
		{
           ...
           {
                case CREATURE_ELITE_NORMAL:
                    return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_NORMAL_DAMAGE_TAQ);
                case CREATURE_ELITE_ELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_DAMAGE_TAQ);
                case CREATURE_ELITE_RAREELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RAREELITE_DAMAGE_TAQ);
                case CREATURE_ELITE_WORLDBOSS:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_WORLDBOSS_DAMAGE_TAQ);
                case CREATURE_ELITE_RARE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RARE_DAMAGE_TAQ);
                default:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_DAMAGE_TAQ);
           }
		}break;
		case 533:
		{
           ...
           {
                case CREATURE_ELITE_NORMAL:
                    return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_NORMAL_DAMAGE_NAXX);
                case CREATURE_ELITE_ELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_DAMAGE_NAXX);
                case CREATURE_ELITE_RAREELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RAREELITE_DAMAGE_NAXX);
                case CREATURE_ELITE_WORLDBOSS:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_WORLDBOSS_DAMAGE_NAXX);
                case CREATURE_ELITE_RARE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RARE_DAMAGE_NAXX);
                default:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_DAMAGE_NAXX);
           }
		}break;
		case 30:
		{
           ...
           {
                case CREATURE_ELITE_NORMAL:
                    return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_NORMAL_DAMAGE_AS);
                case CREATURE_ELITE_ELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_DAMAGE_AS);
                case CREATURE_ELITE_RAREELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RAREELITE_DAMAGE_AS);
                case CREATURE_ELITE_WORLDBOSS:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_WORLDBOSS_DAMAGE_AS);
                case CREATURE_ELITE_RARE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RARE_DAMAGE_AS);
                default:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_DAMAGE_AS);
           }
		}break;
...
float Creature::_GetSpellDamageMod(int32 Rank, uint32 mapid)
{
	switch (mapid)
	{
		case 309:
		{
           ...
           {
                case CREATURE_ELITE_NORMAL:
                    return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_NORMAL_SPELLDAMAGE_ZUG);
                case CREATURE_ELITE_ELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_SPELLDAMAGE_ZUG);
                case CREATURE_ELITE_RAREELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RAREELITE_SPELLDAMAGE_ZUG);
                case CREATURE_ELITE_WORLDBOSS:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_WORLDBOSS_SPELLDAMAGE_ZUG);
                case CREATURE_ELITE_RARE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RARE_SPELLDAMAGE_ZUG);
                default:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_SPELLDAMAGE_ZUG);
           }
		}break;
		case 509:
		{
           ...
           {
                case CREATURE_ELITE_NORMAL:
                    return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_NORMAL_SPELLDAMAGE_FX);
                case CREATURE_ELITE_ELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_SPELLDAMAGE_FX);
                case CREATURE_ELITE_RAREELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RAREELITE_SPELLDAMAGE_FX);
                case CREATURE_ELITE_WORLDBOSS:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_WORLDBOSS_SPELLDAMAGE_FX);
                case CREATURE_ELITE_RARE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RARE_SPELLDAMAGE_FX);
                default:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_SPELLDAMAGE_FX);
           }
		}break;
		case 409:
		{
           ...
           {
                case CREATURE_ELITE_NORMAL:
                    return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_NORMAL_SPELLDAMAGE_MC);
                case CREATURE_ELITE_ELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_SPELLDAMAGE_MC);
                case CREATURE_ELITE_RAREELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RAREELITE_SPELLDAMAGE_MC);
                case CREATURE_ELITE_WORLDBOSS:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_WORLDBOSS_SPELLDAMAGE_MC);
                case CREATURE_ELITE_RARE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RARE_SPELLDAMAGE_MC);
                default:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_SPELLDAMAGE_MC);
           }
		}break;
		case 249:
		{
           ...
           {
                case CREATURE_ELITE_NORMAL:
                    return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_NORMAL_SPELLDAMAGE_HL);
                case CREATURE_ELITE_ELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_SPELLDAMAGE_HL);
                case CREATURE_ELITE_RAREELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RAREELITE_SPELLDAMAGE_HL);
                case CREATURE_ELITE_WORLDBOSS:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_WORLDBOSS_SPELLDAMAGE_HL);
                case CREATURE_ELITE_RARE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RARE_SPELLDAMAGE_HL);
                default:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_SPELLDAMAGE_HL);
           }
		}break;
		case 469:
		{
           ...
           {
                case CREATURE_ELITE_NORMAL:
                    return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_NORMAL_SPELLDAMAGE_BWL);
                case CREATURE_ELITE_ELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_SPELLDAMAGE_BWL);
                case CREATURE_ELITE_RAREELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RAREELITE_SPELLDAMAGE_BWL);
                case CREATURE_ELITE_WORLDBOSS:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_WORLDBOSS_SPELLDAMAGE_BWL);
                case CREATURE_ELITE_RARE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RARE_SPELLDAMAGE_BWL);
                default:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_SPELLDAMAGE_BWL);
           }
		}break;
		case 531:
		{
           ...
           {
                case CREATURE_ELITE_NORMAL:
                    return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_NORMAL_SPELLDAMAGE_TAQ);
                case CREATURE_ELITE_ELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_SPELLDAMAGE_TAQ);
                case CREATURE_ELITE_RAREELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RAREELITE_SPELLDAMAGE_TAQ);
                case CREATURE_ELITE_WORLDBOSS:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_WORLDBOSS_SPELLDAMAGE_TAQ);
                case CREATURE_ELITE_RARE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RARE_SPELLDAMAGE_TAQ);
                default:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_SPELLDAMAGE_TAQ);
           }
		}break;
		case 533:
		{
           ...
           {
                case CREATURE_ELITE_NORMAL:
                    return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_NORMAL_SPELLDAMAGE_NAXX);
                case CREATURE_ELITE_ELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_SPELLDAMAGE_NAXX);
                case CREATURE_ELITE_RAREELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RAREELITE_SPELLDAMAGE_NAXX);
                case CREATURE_ELITE_WORLDBOSS:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_WORLDBOSS_SPELLDAMAGE_NAXX);
                case CREATURE_ELITE_RARE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RARE_SPELLDAMAGE_NAXX);
                default:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_SPELLDAMAGE_NAXX);
           }
		}break;
		case 30:
		{
           ...
           {
                case CREATURE_ELITE_NORMAL:
                    return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_NORMAL_SPELLDAMAGE_AS);
                case CREATURE_ELITE_ELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_SPELLDAMAGE_AS);
                case CREATURE_ELITE_RAREELITE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RAREELITE_SPELLDAMAGE_AS);
                case CREATURE_ELITE_WORLDBOSS:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_WORLDBOSS_SPELLDAMAGE_AS);
                case CREATURE_ELITE_RARE:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_RARE_SPELLDAMAGE_AS);
                default:
					return sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_SPELLDAMAGE_AS);
           }
		}break;
...
bool Creature::LoadFromDB(uint32 guidlow, Map* map)
{
	...
	if (curhealth)
	{
		...
		{
		case 309:
			curhealth = uint32(curhealth * _GetHealthMod(GetCreatureInfo()->Rank, 309));
			break;
		case 509:
			curhealth = uint32(curhealth * _GetHealthMod(GetCreatureInfo()->Rank, 509));
			break;
		case 409:
			curhealth = uint32(curhealth * _GetHealthMod(GetCreatureInfo()->Rank, 409));
			break;
		case 249:
			curhealth = uint32(curhealth * _GetHealthMod(GetCreatureInfo()->Rank, 249));
			break;
		case 469:
			curhealth = uint32(curhealth * _GetHealthMod(GetCreatureInfo()->Rank, 469));
			break;
		case 531:
			curhealth = uint32(curhealth * _GetHealthMod(GetCreatureInfo()->Rank, 531));
			break;
		case 533:
			curhealth = uint32(curhealth * _GetHealthMod(GetCreatureInfo()->Rank, 533));
			break;
		case 30:
			curhealth = uint32(curhealth * _GetHealthMod(GetCreatureInfo()->Rank, 30));
			break;
		default:
			curhealth = uint32(curhealth * _GetHealthMod(GetCreatureInfo()->Rank, 1000));
			break;
		}
		if (curhealth < 1)
			curhealth = 1;
...
bool Creature::IsImmuneToSpellEffect(SpellEntry const* spellInfo, SpellEffectIndex index, bool castOnSelf) const
{
	...
	if (GetMapId() == 531 && !IsWorldBoss() && spellInfo->SpellIconID != 20 && (1 << (spellInfo->EffectMechanic[index] - 1)) == 64)
		return true;
...
void Creature::SaveRespawnTime()
{
	...
	/*if ((GetEntry() == 6109 || GetEntry() == 12397) && m_respawnTime > time(NULL))
	{
		int32 randResTime = 0;
		randResTime = int32(urand(0, 129600) - 64800);
		m_respawnTime = m_respawnTime + randResTime;
	}*/
...
bool Creature::IsOutOfThreatArea(Unit* pVictim) const
{
	...
	if (!pVictim->DoubleCheckAccess((Creature*)this))
		return true;
...
void Creature::AddCreatureSpellCooldown(uint32 spellid)
{
	...
	{
		if (owner->GetTypeId() == TYPEID_PLAYER && cooldown < 1000 && (HasAura(605) || HasAura(10911) || HasAura(10912) || HasAura(26740) || HasAura(1098) || HasAura(11725) || HasAura(11726)))
			cooldown = 10000;
	}
    if (cooldown)
...
{
	DETAIL_LOG("GetScriptName(): GetScriptId() = %u", GetScriptId());
...
void Creature::CreatureVisibilityDistance(float Distance)
{
	if (GetTypeId() == TYPEID_UNIT)
	{
		...
		{
			if (!(*iter) || (*iter)->GetTypeId() == TYPEID_UNIT)
				continue;

			if ((*iter)->HasAura(23186))
				continue;

			if ((*iter)->HasAura(23182))
			    (*iter)->CastSpell((*iter), 23186, true);
		}
	}
}

void Creature::CreatureDistance(float Distance)
{
	if (GetTypeId() == TYPEID_UNIT)
	{
		if (isDead())
		...
		for (Spell::UnitList::iterator iter = targets.begin(); iter != targets.end(); ++iter)
		{
			if (!(*iter) || (*iter)->GetTypeId() == TYPEID_UNIT || (*iter)->isDead())
				continue;
...
void Creature::CreatureBugDistance(float x, float y, float z, float o, float Distance, bool Dead)
{
	if (GetTypeId() == TYPEID_UNIT)
	{
		if (isDead())
...
void Creature::CreatureCorpseDistance(float Distance)
{
	if (GetTypeId() == TYPEID_UNIT)
	{
		if (isDead())
		...
		MaNGOS::AnyUnitInObjectCorpseRangeCheck u_check(this, Distance);
		MaNGOS::UnitListSearcher<MaNGOS::AnyUnitInObjectCorpseRangeCheck> searcher(targets, u_check);
		Cell::VisitAllObjects(this, searcher, Distance);
		for (Spell::UnitList::iterator iter = targets.begin(); iter != targets.end(); ++iter)
		{
			GuidSet::const_iterator itr = m_SiWangQiShi.find((*iter)->GetObjectGuid());
			if (!(*iter) || (*iter)->GetTypeId() != TYPEID_UNIT || (*iter)->getDeathState() != CORPSE || (*iter)->GetEntry() == 16390)
				continue;

			if (GetDistance2d((*iter)) > m_Distance)
			{
				m_Distance = GetDistance2d((*iter));
				pTarget = (*iter);
			}
		}
		...
		if (Creature* pCreature = SummonCreature(16390, pTarget->GetPositionX(), pTarget->GetPositionY(), pTarget->GetPositionZ(), 0.0f, TEMPSUMMON_TIMED_OOC_OR_DEAD_DESPAWN, 10000))
		{
			if (getVictim())
				pCreature->AI()->AttackStart(getVictim());
			else
				pCreature->SetInCombatWithZone();

			m_SiWangQiShi.insert(pTarget->GetObjectGuid());
		}
	}
}

void Creature::SetGroupLootRight(Player* player)
{
	m_ownerSet.clear();
	Group* grp = player->GetGroup();
	if (grp)
	{
		LootMethod lootMethod = grp->GetLootMethod();
		...
		ObjectGuid const& currentLooterGuid = grp->GetLooterGuid();
		...
		do
		{
			...
			if (looter)
			{
				if (cMap && cMap->IsRaid() && (looter->GetMap() == cMap) && looter->GetInstanceId() == ((Creature*)this)->GetInstanceId())
					m_ownerSet.insert(itr->guid);
				else
				if (looter->IsWithinDist(this, sWorld.getConfig(CONFIG_FLOAT_GROUP_XP_DISTANCE), false) && looter->GetInstanceId() == ((Creature*)this)->GetInstanceId())
					m_ownerSet.insert(itr->guid);   // save this guid to main owner set          
			}
			else
			{
				if (cMap && cMap->IsDungeon())
				{
					float x, y, z, o;
					uint32 map;
					uint32 instanceid;
					bool in_flight;
					if (Player::LoadPositionFromDB(itr->guid, map, x, y, z, o, in_flight) && Player::LoadCharacterInstanceFromDB(itr->guid, instanceid))
					{
						if (map == GetMapId() && ((Creature*)this)->GetInstanceId() == instanceid)
						{
							if (cMap->IsRaid())
								m_ownerSet.insert(itr->guid);
							else
							{
								if (IsWithinDist2d(x, y, sWorld.getConfig(CONFIG_FLOAT_GROUP_XP_DISTANCE)))
									m_ownerSet.insert(itr->guid);
							}
						}
					}
...
bool Creature::CanLoot(Player* player)
{
...
void Creature::SetCreatureEvadeMode(Unit* pVictim, uint32 diff)
{
	switch (GetEntry())
	{
		case 10184:
	...
	if (pVictim && pVictim->GetTypeId() == TYPEID_PLAYER && !IsFriendlyTo(pVictim) && !IsNonMeleeSpellCasted(false) && !IsTotem() && !bossjiance)
	{
		for (uint32 i = CURRENT_FIRST_NON_MELEE_SPELL; i < CURRENT_MAX_SPELL; i++)
		{
			if (Spell* pSpell = GetCurrentSpell(CurrentSpellTypes(i)))
			{
				SpellRangeEntry const* srange = sSpellRangeStore.LookupEntry(pSpell->m_spellInfo->rangeIndex);
				float max_range = GetSpellMaxRange(srange);
				float min_range = GetSpellMinRange(srange);
				float dist = GetCombatDistance(pVictim, pSpell->m_spellInfo->rangeIndex == SPELL_RANGE_IDX_COMBAT);
				if (dist < max_range && m_creatureEvadeModeCont != 0)
				{
					m_creatureEvadeModeCont = 0;
					if (IsImmunedToDamage(SPELL_SCHOOL_MASK_ALL))
						ApplySpellImmune(0, IMMUNITY_DAMAGE, SPELL_SCHOOL_MASK_ALL, false);
				}
			}
		}

		if (!((Unit*)this)->m_movementInfo.HasMovementFlag(movementFlagsMask) && !CanReachWithMeleeAttack(getVictim()))
		{
			if (!HasAuraType(SPELL_AURA_MOD_STUN) && !HasAuraType(SPELL_AURA_MOD_FEAR) && !HasAuraType(SPELL_AURA_MOD_CONFUSE) && !HasAuraType(SPELL_AURA_MOD_POSSESS) && !HasAuraType(SPELL_AURA_MOD_ROOT))
			{
				if (m_creatureEvadeModeTimer)
				{
					if (m_creatureEvadeModeTimer <= diff)
					{
						++m_creatureEvadeModeCont;
						m_creatureEvadeModeTimer = 1000;
					}
					else
						m_creatureEvadeModeTimer -= diff;
				}

				if (m_creatureEvadeModeCont >= _GetCreatureElite(GetCreatureInfo()->Rank))
				{
					if (!IsImmunedToDamage(SPELL_SCHOOL_MASK_ALL))
						ApplySpellImmune(0, IMMUNITY_DAMAGE, SPELL_SCHOOL_MASK_ALL, true);
		...
		if (IsImmunedToDamage(SPELL_SCHOOL_MASK_ALL) && CanReachWithMeleeAttack(getVictim()))
			ApplySpellImmune(0, IMMUNITY_DAMAGE, SPELL_SCHOOL_MASK_ALL, false);
	}
}
