...
class MANGOS_DLL_SPEC Creature : public Unit
{
    ...
    public:
		...
		void SetCreatureEvadeMode(Unit* pVictim, uint32 diff);
		void CreatureVisibilityDistance(float Distance);
		void CreatureDistance(float Distance);
		void CreatureBugDistance(float x, float y, float z, float o, float Distance, bool Dead = false);
		void CreatureCorpseDistance(float Distance);
		void SetGroupLootRight(Player* player);
		bool CanLoot(Player* player);
		GuidSet m_ownerSet;
		GuidSet m_SiWangQiShi;
		...
		static float _GetHealthMod(int32 Rank, uint32 mapid);             ///< Get custom factor to scale health (default 1, CONFIG_FLOAT_RATE_CREATURE_*_HP)
		static float _GetDamageMod(int32 Rank, uint32 mapid);             ///< Get custom factor to scale damage (default 1, CONFIG_FLOAT_RATE_*_DAMAGE)
		static float _GetSpellDamageMod(int32 Rank, uint32 mapid);        ///< Get custom factor to scale spell damage (default 1, CONFIG_FLOAT_RATE_*_SPELLDAMAGE)
    ...
    protected:
		...
		uint32 m_creatureEvadeModeCont;
		uint32 m_creatureEvadeModeTimer;
