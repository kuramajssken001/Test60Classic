...
namespace MaNGOS
{
    namespace Honor
    {
        ...
        inline HonorScores GenerateScores(HonorStandingList standingList, uint32 team)
        {
			...
			float minimun_size = standingList.size() > 250 ? standingList.size() : 250;
            for (uint8 group = 0; group < 14; group++)
				sc.BRK[group] = floor((sc.BRK[group] * minimun_size) + 0.5f);
        ...
        inline float HonorableKillPoints(Player* killer, Player* victim, uint32 groupsize)
        {
            ...
            int f = (3 - total_kills) >= 0 ? (3 - total_kills) : 0;
            int honor_points = int(((float)(f * 0.5) * (float)((k_level + (v_rank * 5 + 1)) * (1 + 0.05 * diff_honor) * diff_level)));
