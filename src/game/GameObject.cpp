        ...
        case GO_ACTIVATED:
        {
            switch (GetGoType())
            {
				...
				case GAMEOBJECT_TYPE_TRAP:
					if (GetEntry() == 179784)
					{
						if (GetGOInfo()->GetAutoCloseTime() && ((m_cooldownTime + 45) < time(NULL)))
						{
...
{
	if (GetMap()->IsDungeon() && GetGoType() == GAMEOBJECT_TYPE_CHEST && !sObjectMgr.IsGameObjectForQuests(GetEntry()))
	    if (m_respawnTime > time(NULL) && m_spawnedByDefault)
		    m_respawnTime = m_respawnTime + 604800;
...
void GameObject::Use(Unit* user)
{
    ...
    switch (GetGoType())
    {
        ...
        case GAMEOBJECT_TYPE_TRAP:                          // 6
        {
			...
			if (GetDisplayId() == 4392 || GetDisplayId() == 4472 || GetDisplayId() == 4491 || GetDisplayId() == 6785 || GetDisplayId() == 3073 || GetDisplayId() == 5874)
                SendGameObjectCustomAnim(GetObjectGuid());
        ...
        case GAMEOBJECT_TYPE_FLAGSTAND:                     // 24
        {
			...
			if (player->isAlive() && !player->HasInvisibilityAura() && !player->isTotalImmune() &&
