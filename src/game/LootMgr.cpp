...
void LootTemplate::LootGroup::Process(Loot& loot) const
{
	...
	if (item != NULL)
	{
		...
		{
			if (loot.GetLootTarget()->GetTypeId() == TYPEID_UNIT && ((Creature*)loot.GetLootTarget())->IsWorldBoss())
			{
				ItemPrototype const* pProto = ObjectMgr::GetItemPrototype(item->itemid);
				if (pProto && pProto->Quality >= 3 && (loot.GetLootTarget()->GetMapId() == 309 || loot.GetLootTarget()->GetMapId() == 509))
				    sLog.outLoot("[LOOT]Name = %s, InstanceId = %u, MapId = %u, ItemName = %s", loot.GetLootTarget()->GetName(), loot.GetLootTarget()->GetInstanceId(), loot.GetLootTarget()->GetMapId(), pProto->Name1);
				else
				if (pProto && pProto->Quality >= 4 && sMapStore.LookupEntry(loot.GetLootTarget()->GetMapId())->IsRaid())
					sLog.outLoot("[LOOT]Name = %s, InstanceId = %u, MapId = %u, ItemName = %s", loot.GetLootTarget()->GetName(), loot.GetLootTarget()->GetInstanceId(), loot.GetLootTarget()->GetMapId(), pProto->Name1);
			}
			else
			if (loot.GetLootTarget()->GetTypeId() == TYPEID_GAMEOBJECT)
			{
				if (loot.GetLootTarget()->GetEntry() == 179703)
				{
					ItemPrototype const* pProto = ObjectMgr::GetItemPrototype(item->itemid);
					GameObjectInfo const* gInfo = ObjectMgr::GetGameObjectInfo(loot.GetLootTarget()->GetEntry());
					if (pProto && gInfo && pProto->Quality >= 3 && (loot.GetLootTarget()->GetMapId() == 309 || loot.GetLootTarget()->GetMapId() == 509))
						sLog.outLoot("[LOOT]Name = %s, InstanceId = %u, MapId = %u, ItemName = %s", gInfo->name, loot.GetLootTarget()->GetInstanceId(), loot.GetLootTarget()->GetMapId(), pProto->Name1);
					else
					if (pProto && gInfo && pProto->Quality >= 4 && sMapStore.LookupEntry(loot.GetLootTarget()->GetMapId())->IsRaid())
						sLog.outLoot("[LOOT]Name = %s, InstanceId = %u, MapId = %u, ItemName = %s", gInfo->name, loot.GetLootTarget()->GetInstanceId(), loot.GetLootTarget()->GetMapId(), pProto->Name1);
				}
			}
		}
...
void LootTemplate::Process(Loot& loot, LootStore const& store, bool rate, uint8 groupId) const
{
    ...
    for (LootStoreItemList::const_iterator i = Entries.begin() ; i != Entries.end() ; ++i)
    {
		...
		if (!i->group && loot.GetLootTarget())
		{
			if (loot.GetLootTarget()->GetTypeId() == TYPEID_UNIT && ((Creature*)loot.GetLootTarget())->IsWorldBoss())
			{
				ItemPrototype const* pProto = ObjectMgr::GetItemPrototype(i->itemid);
				if (pProto && pProto->Quality >= 3 && (loot.GetLootTarget()->GetMapId() == 309 || loot.GetLootTarget()->GetMapId() == 509))
				    sLog.outLoot("[LOOT]Name = %s, InstanceId = %u, MapId = %u, ItemName = %s", loot.GetLootTarget()->GetName(), loot.GetLootTarget()->GetInstanceId(), loot.GetLootTarget()->GetMapId(), pProto->Name1);
				else
				if (pProto && pProto->Quality >= 4 && sMapStore.LookupEntry(loot.GetLootTarget()->GetMapId())->IsRaid())
...
bool LootTemplate::HasStartingQuestDropForPlayer(LootTemplateMap const& store, Player const* player, uint8 groupId) const
{
	...
	for (LootStoreItemList::const_iterator i = Entries.begin(); i != Entries.end(); ++i)
	{
		...
		{
			return false;
		}                               // player doesn't respect the conditions.
