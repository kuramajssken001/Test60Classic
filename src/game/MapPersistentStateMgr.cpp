...
void DungeonPersistentState::DeleteFromDB()
{
	...
	if (GetMapId() == 309 || GetMapId() == 409 || GetMapId() == 469 || GetMapId() == 249)
	    sLog.outBug("DungeonPersistentState::DeleteFromDB() = %u GetMapId = %u", GetInstanceId(), GetMapId());
}
...
void DungeonResetScheduler::LoadResetTimes()
{
            ...
            if (time_t resettime = time_t((*result)[2].GetUInt64()))
            {
                ...
                if (!mapEntry || !mapEntry->IsDungeon())
                {
					if (mapid == 309 || mapid == 409 || mapid == 469 || mapid == 249)
					    sLog.outBug("sMapPersistentStateMgr.DeleteInstanceFromDB = %u mapid = %u", id, mapid);
    ...
    for (uint32 i = 0; i < sInstanceTemplate.GetMaxEntry(); i++)
    {
		...
		/*if (temp->map == 534)
			sWorld.SetHuoLia(t, now, nextWeek, today, period, diff);*/
        if (!t)
        {
			...
			if (temp->map == 469)
			{
				CharacterDatabase.PExecute("DELETE FROM instancespell_Chromaggus WHERE mapid = '%u'", temp->map);
				CharacterDatabase.PExecute("DELETE FROM instancespell_VictorNefariu WHERE mapid = '%u'", temp->map);
			}
        }
        ...
        if (t < now || t > nextWeek)
        {
			...
			if (temp->map == 469)
			{
				QueryResult* result = CharacterDatabase.PQuery("SELECT resettime FROM instance_reset WHERE mapid = %u", temp->map);
				if (result)
				{
					Field *fields = result->Fetch();
					uint64 resettime = fields[0].GetUInt64();
					if (resettime != (uint64)t)
					{
						CharacterDatabase.PExecute("DELETE FROM instancespell_Chromaggus WHERE mapid = '%u'", temp->map);
						CharacterDatabase.PExecute("DELETE FROM instancespell_VictorNefariu WHERE mapid = '%u'", temp->map);
					}
    ...
    while (!m_resetTimeQueue.empty() && (t = m_resetTimeQueue.begin()->first) < now)
    {
        ...
        else
        {
            ...
            else
            {
				...
				/*if (event.mapid == 534)//注释服务器重启事件Start
				{
					sWorld.SetHuoLib(uint64(next_reset));
					sWorld.ShutdownServ(300, SHUTDOWN_MASK_RESTART, 2);
				}*///注释服务器重启事件End
				if (event.mapid == 469)
				{
					CharacterDatabase.PExecute("DELETE FROM instancespell_Chromaggus WHERE mapid = '%u'", event.mapid);
					CharacterDatabase.PExecute("DELETE FROM instancespell_VictorNefariu WHERE mapid = '%u'", event.mapid);
				}
...
void MapPersistentStateManager::DeleteInstanceFromDB(uint32 instanceid)
{
    if (instanceid)
    {
		QueryResult* result = LoginDatabase.PQuery("SELECT map FROM instance WHERE id = %u", instanceid);
		if (result)
		{
			Field *fields = result->Fetch();
			uint32 map = fields[0].GetUInt32();
			if (map == 309 || map == 409 || map == 469 || map == 249)
			    sLog.outBug("MapPersistentStateManager::DeleteInstanceFromDB = %u map = %u", instanceid, map);
		}
        CharacterDatabase.BeginTransaction();
		...
		CharacterDatabase.PExecute("DELETE FROM instancespell_Chromaggus WHERE id = '%u'", instanceid);
		CharacterDatabase.PExecute("DELETE FROM instancespell_VictorNefariu WHERE id = '%u'", instanceid);
        CharacterDatabase.CommitTransaction();
    }
}
...
void MapPersistentStateManager::PackInstances()
{
    ...
    for (std::set<uint32>::iterator i = InstanceSet.begin(); i != InstanceSet.end(); ++i)
    {
        if (*i != InstanceNumber)
        {
			...
			CharacterDatabase.PExecute("UPDATE instancespell_Chromaggus SET id = '%u' WHERE id = '%u'", InstanceNumber, *i);
			CharacterDatabase.PExecute("UPDATE instancespell_VictorNefariu SET id = '%u' WHERE id = '%u'", InstanceNumber, *i);
...
void MapPersistentStateManager::_ResetInstance(uint32 mapid, uint32 instanceId)
{
	...
	if (mapid == 309 || mapid == 409 || mapid == 469 || mapid == 249)
	    sLog.outBug("MapPersistentStateManager::_ResetInstance = %u mapid = %u", instanceId, mapid);
