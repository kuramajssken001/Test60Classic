...
void WorldSession::HandleAreaTriggerOpcode(WorldPacket& recv_data)
{
	...
	{
		if (targetMapEntry->IsNonRaidDungeon() && player->isAlive())
		{
			uint32 cont = at->target_mapId == 429 ? 3 : sWorld.getConfig(CONFIG_UINT32_PLAYER_INSTANCES_PER_HOUR);
			if (player->GetPlayerInstancesCont() >= cont)
			{
				if (player->m_Player_Instances[PLAYED_INSTANCES_TIME] > 0)
					ChatHandler(player).PSendSysMessage(ZHANYOU_TIME_10, player->GetDaysTime(player->m_Player_Instances[PLAYED_INSTANCES_TIME], 1), player->GetDaysTime(player->m_Player_Instances[PLAYED_INSTANCES_TIME], 2), player->GetDaysTime(player->m_Player_Instances[PLAYED_INSTANCES_TIME], 3), player->GetDaysTime(player->m_Player_Instances[PLAYED_INSTANCES_TIME], 4));
				return;
			}
		}
	}
