...
void PetAI::AttackStart(Unit* u)
{
    ...
    if (m_creature->Attack(u, true))
    {
        ...
        inCombat = true;
		if (m_creature->GetEntry() == 1863)
			m_creature->RemoveAurasWithInterruptFlags(AURA_INTERRUPT_FLAG_UNK2);
    }
}
...
void PetAI::UpdateAI(const uint32 diff)
{
    ...
    if (!m_creature->IsNonMeleeSpellCasted(false))
    {
        ...
        for (uint8 i = 0; i < m_creature->GetPetAutoSpellSize(); ++i)
        {
			...
			if (m_creature->GetPower(POWER_MANA))
			{
			    if (spellInfo->manaCost > m_creature->GetPower(POWER_MANA))
                    continue;
			}
			else
			if (m_creature->GetPower(POWER_FOCUS))
			{
			    if (spellInfo->manaCost > m_creature->GetPower(POWER_FOCUS))
                    continue;
			}
