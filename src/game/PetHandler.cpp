...
void WorldSession::HandlePetAction(WorldPacket& recv_data)
{
    ...
    switch (flag)
    {
        ...
        case ACT_ENABLED:                                   // 0xC1    spell
        {
			...
			if (((Creature*)pet)->GetPower(POWER_MANA))
			{
			    if (spellInfo->manaCost > ((Creature*)pet)->GetPower(POWER_MANA))
                    return;
			}
			else
			if (((Creature*)pet)->GetPower(POWER_FOCUS))
			{
		        if (spellInfo->manaCost > ((Creature*)pet)->GetPower(POWER_FOCUS))
                    return;
			}
            ...
            if (result == SPELL_CAST_OK)
            {
				...
				if (pet->GetEntry() == 1863)
					pet->RemoveAurasWithInterruptFlags(AURA_INTERRUPT_FLAG_UNK2);
            }
            else
            {
				...
				else
				{
					...
					if (owner && owner->GetTypeId() == TYPEID_PLAYER)
						Spell::SendCastResult((Player*)owner, spellInfo, result);
				}
