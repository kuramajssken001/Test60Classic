...
Player::Player(WorldSession* session): Unit(), m_mover(this), m_camera(this), m_reputationMgr(this)
{
	...
	m_PlayerZhanYouTimer = 1 * IN_MILLISECONDS;
...
bool Player::Create(uint32 guidlow, const std::string& name, uint8 race, uint8 class_, uint8 gender, uint8 skin, uint8 face, uint8 hairStyle, uint8 hairColor, uint8 facialHair, uint8 /*outfitId */)
{
	...
	m_Player_time[PLAYER_TIME] = 0;
	m_Player_time[PLAYER_TIME_COOLDOWN] = 0;
	m_Player_time[PLAYER_TIME_ZHANYOU] = 0;

	m_Player_GongNeng[PLAYED_HUOLI_LEVEL] = 0;
	m_Player_GongNeng[PLAYED_ZHANYOU_FLAGS] = 0;
	m_Player_GongNeng[PLAYED_MONEY] = 0;
	m_Player_GongNeng[PLAYED_HUOLI] = 0;
	m_Player_GongNeng[PLAYED_ADDHUOLI] = 0;
	m_Player_GongNeng[PLAYED_TIANFU] = 0;
	m_Player_GongNeng[PLAYED_BIANSHEN] = 0;
	m_Player_GongNeng[PLAYED_SUODUAN] = 0;
	m_Player_GongNeng[PLAYED_CHONGZHI] = 0;
	m_Player_GongNeng[PLAYED_MONEY_TIME] = 0;
	m_Player_GongNeng[PLAYED_HUOLI_TIME] = 0;
	m_Player_GongNeng[PLAYED_ADDHUOLI_TIME] = 0;
	m_Player_GongNeng[PLAYED_TIANFU_TIME] = 0;
	m_Player_GongNeng[PLAYED_BIANSHEN_TIME] = 0;
	m_Player_GongNeng[PLAYED_SUODUAN_TIME] = 0;
	m_Player_GongNeng[PLAYED_CHONGZHI_TIME] = 0;
	...
	m_Player_GongNeng[PLAYED_SHANGYEJINENGCONT] = 0;

	m_Player_Instances[PLAYED_INSTANCES_CONT] = 0;
	m_Player_Instances[PLAYED_INSTANCES_TIME] = 0;

	m_Player_GuiId[PLAYED_GUIID_MIANFEI] = 0;
	m_Player_GuiId[PLAYED_GUIID_SHOUFEI] = 0;
	m_Player_GuiId[PLAYED_GUIID_MIANFEITIME] = 0;
...
void Player::Update(uint32 update_diff, uint32 p_time)
{
    ...
    if (now > m_Last_tick)
    {
	    ...
	    {
		    if (GetPlayerMoneyTime() <= sWorld.GetGameTime())
		    {
			    m_Player_GongNeng[PLAYED_MONEY] = 0;
			    m_Player_GongNeng[PLAYED_MONEY_TIME] = 0;
			    SaveToDB();
	    ...
	    if (GetPlayerHuoLiLevel() >= 1)
	    {
		    if (GetPlayerHuoLiTime() <= sWorld.GetGameTime())
		    {
			    m_Player_GongNeng[PLAYED_HUOLI] = 0;
			    m_Player_GongNeng[PLAYED_HUOLI_TIME] = 0;
			    SaveToDB();
	    ...
	    if (GetPlayerAddHuoLiLevel() >= 1)
	    {
		    if (GetPlayerAddHuoLiTime() <= sWorld.GetGameTime())
		    {
			    m_Player_GongNeng[PLAYED_ADDHUOLI] = 0;
			    m_Player_GongNeng[PLAYED_ADDHUOLI_TIME] = 0;
			    SaveToDB();
	    ...
	    if (GetPlayerTianFuLevel() >= 1)
	    {
		    if (GetPlayerTianFuTime() <= sWorld.GetGameTime())
		    {
			    m_Player_GongNeng[PLAYED_TIANFU] = 0;
			    m_Player_GongNeng[PLAYED_TIANFU_TIME] = 0;
			    SaveToDB();
	    ...
	    if (GetPlayerBianShenLevel() >= 1)
	    {
		    if (GetPlayerBianShenTime() <= sWorld.GetGameTime())
		    {
			    m_Player_GongNeng[PLAYED_BIANSHEN] = 0;
			    m_Player_GongNeng[PLAYED_BIANSHEN_TIME] = 0;
			    SaveToDB();
	    ...
	    {
		    if (GetPlayerCdSuoDuanTime() <= sWorld.GetGameTime())
		    {
			    m_Player_GongNeng[PLAYED_SUODUAN] = 0;
			    m_Player_GongNeng[PLAYED_SUODUAN_TIME] = 0;
			    SaveToDB();
	    ...
	    if (GetPlayerCdChongZhiLevel() >= 1)
	    {
		    if (GetPlayerCdChongZhiTime() <= sWorld.GetGameTime())
		    {
			    m_Player_GongNeng[PLAYED_CHONGZHI] = 0;
			    m_Player_GongNeng[PLAYED_CHONGZHI_TIME] = 0;
			    SaveToDB();
		...
		{
			if (GetShuangTftime() <= sWorld.GetGameTime())
			{
				m_Player_GongNeng[PLAYED_SHUANGTIANFU] = 0;
				m_Player_GongNeng[PLAYED_SHUANGTIANFU_CONT] = 0;
				m_Player_GongNeng[PLAYED_SHUANGTIANFU_TIME] = 0;
		...
		if (sWorld.getConfig(CONFIG_BOOL_PLAYER_INSTANCES_PER_HOUR_DAKAI))
		{
			if (GetPlayerInstancesCont() >= 1 && GetPlayerInstancesTime() > 0)
			{
				if (GetPlayerInstancesTime() <= sWorld.GetGameTime())
				{
					m_Player_Instances[PLAYED_INSTANCES_CONT] = 0;
					m_Player_Instances[PLAYED_INSTANCES_TIME] = 0;
					SaveToDB();
		...
		if (GetPlayerGuiIdMianFeiTime() > 0)
		{
			if (GetPlayerGuiIdMianFeiTime() <= sWorld.GetGameTime())
			{
				m_Player_GuiId[PLAYED_GUIID_MIANFEITIME] = 0;
				m_Player_GuiId[PLAYED_GUIID_MIANFEI] = 0;
				SaveToDB();
		...
		{
			if (GetHuoLiLevel() < 60)
		        m_Player_time[PLAYER_TIME] += elapsed;

		    m_Player_time[PLAYER_TIME_COOLDOWN] += elapsed;
			sGet.SetPlayerLevel(this);

			if (m_PlayerZhanYouTimer > 0)
			{
                if (update_diff >= m_PlayerZhanYouTimer)
                {
                    sGet.PlayerOnline(this, elapsed);
                    m_PlayerZhanYouTimer = 10000;
                }
                else
                    m_PlayerZhanYouTimer -= update_diff;
            }

			if (GetGuildId() && sGet.GetGuildSw(GetGuildId()) < 2000000000 && sGet.GetGuildSw_CaiLiao(sGet.GetGuildLevel(GetGuildId())) >= sGet.GetGuildSw(GetGuildId()) && sGet.GetGuildLevel(GetGuildId()) < 5)
				SetGuildSw(this, GetGuildSw(GetGuildId()) + elapsed);

			if (GetGuildId())
			{
				if (GetGuildLevel(GetGuildId()) >= 1 && GetGuild_Money(GetGUIDLow()) != 1 && GetGuild_Time(GetGUIDLow()) < 3600)
					SetGuild_Time(this, GetGuild_Time(GetGUIDLow()) + elapsed);
				else
				if (GetGuildLevel(GetGuildId()) >= 2 && GetGuild_Money(GetGUIDLow()) == 1 && GetGuild_Sw(GetGUIDLow()) != 1 && GetGuild_Time(GetGUIDLow()) < 10800)
					SetGuild_Time(this, GetGuild_Time(GetGUIDLow()) + elapsed);
				else
				if (GetGuildLevel(GetGuildId()) >= 3 && GetGuild_Money(GetGUIDLow()) == 1 && GetGuild_Sw(GetGUIDLow()) == 1 && GetGuild_TianFu(GetGUIDLow()) != 1 && GetGuild_Time(GetGUIDLow()) < 21600)
					SetGuild_Time(this, GetGuild_Time(GetGUIDLow()) + elapsed);
				else
				if (GetGuildLevel(GetGuildId()) >= 3 && GetGuild_Money(GetGUIDLow()) == 1 && GetGuild_Sw(GetGUIDLow()) == 1 && GetGuild_TianFu(GetGUIDLow()) == 1 && GetGuild_Spell(GetGUIDLow()) != 1 && GetGuild_Time(GetGUIDLow()) < 43200)
					SetGuild_Time(this, GetGuild_Time(GetGUIDLow()) + elapsed);
				else
				if (GetGuildLevel(GetGuildId()) >= 5 && GetGuild_Money(GetGUIDLow()) == 1 && GetGuild_Sw(GetGUIDLow()) == 1 && GetGuild_TianFu(GetGUIDLow()) == 1 && GetGuild_Spell(GetGUIDLow()) == 1 && GetGuild_HuoLi(GetGUIDLow()) != 1 && GetGuild_Time(GetGUIDLow()) < 86400)
					SetGuild_Time(this, GetGuild_Time(GetGUIDLow()) + elapsed);
			}
		}*/
        m_Last_tick = now;
	...
	if (HasAuraType(SPELL_AURA_MOD_CHARM))
	{
		...
		if (owner && owner->GetTypeId() == TYPEID_UNIT)
			AIoncharm.UpdateAI(this, owner, update_diff);
	}
}
...
void Player::ProcessDelayedOperations()
{
    ...
    if (m_DelayedOperations & DELAYED_SPELL_CAST_DESERTER)
    {
		...
		else
		{
			...
			SpellAuraHolder* holder = CreateSpellAuraHolder(spellInfo, ((Unit*)this), this);
			...
			for (uint32 i = 0; i < MAX_EFFECT_INDEX; ++i)
			{
				...
				{
					Aura* aur = CreateAura(spellInfo, SpellEffectIndex(i), NULL, holder, ((Unit*)this));
					holder->AddAura(aur, SpellEffectIndex(i));
				}
			}
			((Unit*)this)->AddSpellAuraHolder(holder);
		}
    }
...
uint32 Player::DurabilityRepair(uint16 pos, bool cost, float discountMod)
{
    ...
    if (cost)
    {
        ...
        if (LostDurability > 0)
        {
			...
			costs = uint32(costs * discountMod * sWorld.getConfig(CONFIG_FLOAT_XIOULIFEI));
...
void Player::UpdateLocalChannels(uint32 newZone)
{
    ...
    for (JoinedChannelsList::iterator i = m_channels.begin(), next; i != m_channels.end(); i = next)
    {
		...
		std::string channelname = GetZiZhiName(500);
...
void Player::UpdateSkillsToMaxSkillsForLevel(bool zanzhu)
{
    for (SkillStatusMap::iterator itr = mSkillStatus.begin(); itr != mSkillStatus.end(); ++itr)
    {
        SkillStatusData& skillStatus = itr->second;
        ...
        if (IsProfessionOrRidingSkill(pskill))
            continue;
		if (zanzhu && (pskill == SKILL_POISONS || pskill == SKILL_LOCKPICKING))
			continue;
        uint32 valueIndex = PLAYER_SKILL_VALUE_INDEX(skillStatus.pos);
        uint32 data = GetUInt32Value(valueIndex);
...
void Player::UpdateHonor()
{
	...
	QueryResult* result = CharacterDatabase.PQuery("SELECT kills, honor_week, standing_week FROM character_history_honor WHERE week = %u and guid = %u", lastWeekBegin, GetGUIDLow());
	if (result)
	{
		Field* fields = result->Fetch();
		uint32 kills = fields[0].GetUInt32();
		uint32 honor_week = fields[1].GetUInt32();
		uint32 standing_week = fields[2].GetUInt32();
		lastWeekKills = kills;
		lastWeekHonor = honor_week;
		lastWeekStanding = standing_week;
	}
	...
	SetUInt32Value(PLAYER_FIELD_LAST_WEEK_RANK, lastWeekStanding);
    // LIFE TIME
...
bool Player::RewardHonor(Unit* uVictim, uint32 groupsize)
{
    ...
    else if (uVictim->GetTypeId() == TYPEID_PLAYER)
    {
		...
		if (getLevel() < (pVictim->getLevel() + 5) && !pVictim->HasAura(SPELL_ID_PASSIVE_RESURRECTION_SICKNESS))
		{
			bool VictimPower;
			uint32 honor_flag;
			if (pVictim->GetPowerType() == POWER_MANA)
			{
				if (pVictim->GetPower(POWER_MANA) == pVictim->GetMaxPower(POWER_MANA))
					VictimPower = true;
				else
					VictimPower = false;
			}
			...
			honor_flag = MaNGOS::Honor::HonorableKillPoints(this, pVictim, groupsize) == (MaNGOS::Honor::HonorableKillPoints(this, pVictim, groupsize) * groupsize) ? 1 : 0;
			CharacterDatabase.PExecute("INSERT INTO character_honor_killer (killer_id, victim_id, map_id, victim_position_x, victim_postion_y, honor_flag, ip, death_time, killer_hp_flag, victim_mana_flag) VALUES (%u, %u, %u, %f, %f, %u, %u, %u, %u, %u)", GetGUIDLow(), pVictim->GetGUIDLow(), pVictim->GetMapId(), pVictim->GetPositionX(), pVictim->GetPositionY(), honor_flag, GetSession()->GetRemoteAddress() == pVictim->GetSession()->GetRemoteAddress() ? 1 : 0, uint32(sWorld.GetGameTime()), GetHealth() == GetMaxHealth() ? 1 : 0, VictimPower);
		}

        if (GetSession()->GetRemoteAddress() != pVictim->GetSession()->GetRemoteAddress() && !pVictim->HasAura(SPELL_ID_PASSIVE_RESURRECTION_SICKNESS))
		{
			if (getLevel() < (pVictim->getLevel() + 5))
			{
				if (MaNGOS::Honor::HonorableKillPoints(this, pVictim, groupsize) > 0)
				{
					WorldPacket data(SMSG_PVP_CREDIT, 4 + 8 + 4);
					data << uint32(MaNGOS::Honor::HonorableKillPoints(this, pVictim, groupsize));
					data << uint64(pVictim->GetGUIDLow());
					data << uint32(pVictim->GetHonorRankInfo().visualRank + 4);
					GetSession()->SendPacket(&data);
				}
            ...
            else
            {
                ...
                if (!creature->lootForBody)
                {
					...
					if (uint32 lootid = creatureInfo->LootId)
					{
						...
						if (sMapStore.LookupEntry(GetMapId())->IsRaid() && creature->IsWorldBoss())
						{
							if (Group* pGroupa = GetGroup())
							{
								for (GroupReference* itra = pGroupa->GetFirstMember(); itra != NULL; itra = itra->next())
								{
									...
									ChatHandler(pPlayera->GetSession()).PSendSysMessage(LANG_GONGGAO_27, GetName(), creature->GetName());
								}
							}
						}
					}
...
void Player::SetplayerFaction(Player* _player, uint32 factionId, uint32 Reputation, uint32 playerjf)
{
	if(_player->getLevel() == 60)
	{
		if (uint32(_player->GetReputationRank(factionId) < 7))
		{
		    if (_player->Getjifen() >= playerjf)
		    {
	            FactionEntry const* factionEntry = sFactionStore.LookupEntry(factionId);
	            if (!factionEntry)
                    return;

				uint32 amount = _player->Getjifen() - playerjf;
			    _player->Setjifen(amount);
	            _player->GetReputationMgr().SetReputation(factionEntry, Reputation);
				GetSession()->SendNotification(LANG_CHANNEL_1);
			}
			else
	           GetSession()->SendNotification(LANG_CHANNEL_2, playerjf);
		}
		else
	       GetSession()->SendNotification(LANG_CHANNEL_3);
	}
	else
...
void Player::ShueiJiQuest(uint32 quest, uint32 money, Player* _player)
{
	if(_player->getLevel() == 60)
	{
	   ...
	   if (qInfo)
	   {
		   if (_player->CanTakeQuest(qInfo, true))
		   {
			   if (_player->GetMoney() >= money)
			   {
			       uint32 Money = _player->GetMoney() - money;
			       _player->SetMoney(Money);
                   _player->AddQuest(qInfo, NULL);
				   _player->GetSession()->SendNotification(LANG_CHANNEL_6);
			   }
			   else
	              GetSession()->SendNotification(LANG_CHANNEL_5);
		   }
	   }
	}
	else
	   GetSession()->SendNotification(LANG_CHANNEL_7);
}

void Player::GroupQuest(uint32 quest1, uint32 quest2, uint32 money, Player* _player, uint32 placout)
{
	Quest const* qInfoa = sObjectMgr.GetQuestTemplate(quest1);
	Quest const* qInfob = sObjectMgr.GetQuestTemplate(quest2);
	uint32 playercout = 0;
	if (Group* pGroup = _player->GetGroup())
    {
		if (pGroup && pGroup->IsLeader(_player->GetGUID()))
		{
            for (GroupReference* itr = pGroup->GetFirstMember(); itr != NULL; itr = itr->next())
		    {
				...
				if (!pPlayer)
			    {
				    _player->GetSession()->SendNotification(LANG_QUES_1);
					playercout = 0;
				    return;
			    }
			    ...
			    {
				    _player->GetSession()->SendNotification(LANG_QUES_2);
					playercout = 0;
				    return;
			    }

				if (!pPlayer->CanTakeQuest(qInfoa, true) && !pPlayer->CanTakeQuest(qInfob, true))
				{
					if (_player == pPlayer)
					    pPlayer->GetSession()->SendNotification(LANG_QUES_8);
					else
					{
						pPlayer->GetSession()->SendNotification(LANG_QUES_8);
					    _player->GetSession()->SendNotification(LANG_QUES_9);
					}
					playercout = 0;
				    return;
				}
			...
			if (playercout == placout)
			{
				if (qInfoa || qInfob)
	            {
		            if (_player->CanTakeQuest(qInfoa, true))
					{
						if (_player->GetMoney() >= money)
						{
                             uint32 Money = _player->GetMoney() - money;
			                 _player->SetMoney(Money);
							 _player->AddQuest(qInfoa, NULL);
							 _player->GetSession()->SendNotification(LANG_QUES_3);
						}
						else
						{
							 _player->GetSession()->SendNotification(LANG_QUES_4);
							return;
					    }
				     }

				     for (GroupReference* itra = pGroup->GetFirstMember(); itra != NULL; itra = itra->next())
		             {
			              Player* pPlayer_2 = itra->getSource();

					      if (pPlayer_2 == _player)
						      continue;

				          if (pPlayer_2->CanTakeQuest(qInfob, true))
					      {
                             pPlayer_2->AddQuest(qInfob, NULL);
							  pPlayer_2->GetSession()->SendNotification(LANG_QUES_3);
					      }
				    }
				}
		    }
			...
			{
				_player->GetSession()->SendNotification(LANG_QUES_5, placout);
				playercout = 0;
				return;
			}
		}
		else
			_player->GetSession()->SendNotification(LANG_QUES_6);
	}
	else
		_player->GetSession()->SendNotification(LANG_QUES_7);
}

void Player::HueiSitem(uint32 item1, uint32 item2, uint32 item3, uint32 item4, uint32 item5, uint32 item7, uint32 item8, uint32 item9, uint32 item11, uint32 cout, uint32 Class_, uint32 HasitemId1, uint32 Hascount1, Player *player, uint32 HasitemId, uint32 Hascount)
{
    if (player->HasItemCount(HasitemId, Hascount) && player->HasItemCount(HasitemId1, Hascount1))
	{
	    uint32 itemId;
	    switch (Class_)
	    {
        ...
        uint8 msg = player->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, itemId, cout, &noSpaceForCount);
        if (msg != EQUIP_ERR_OK)
            cout -= noSpaceForCount;

	    if (cout == 0 || dest.empty())
	    {
		    player->GetSession()->SendNotification(LANG_ITEM_1);
            return;
	    }

        player->DestroyItemCount(HasitemId, Hascount, true);
		player->DestroyItemCount(HasitemId1, Hascount1, true);
        Item* item = player->StoreNewItem(dest, itemId, true, Item::GenerateItemRandomPropertyId(itemId));
	    player->SendNewItem(item, cout, true, false);
        ItemPrototype const* pProto1 = ObjectMgr::GetItemPrototype(itemId);
	    ItemPrototype const* pProto2 = ObjectMgr::GetItemPrototype(HasitemId);
		player->GetSession()->SendNotification(LANG_ITEM_2, pProto2->Name1, pProto1->Name1);
	}
	else
	{
		ItemPrototype const* pProto1 = ObjectMgr::GetItemPrototype(HasitemId);
		ItemPrototype const* pProto2 = ObjectMgr::GetItemPrototype(HasitemId1);
		player->GetSession()->SendNotification(LANG_ITEM_3, pProto1->Name1, Hascount, pProto2->Name1, Hascount1);
	}
}

void Player::setRemoveSpellPlayerCooldown(Player* player, uint32 categoryid, uint32 jf, bool leixing)
{
	if (player->getLevel() == 60)
	{
		if (player->GetCharacters_Quest(player->GetGUIDLow(), 2) >= jf)
		{
			uint32 cout = 0;
			...
			for (SpellCooldowns::const_iterator itr = cm.begin(); itr != cm.end();)
			{
				...
				if ((!leixing ? spellInfo->Category == categoryid : spellInfo->Id == categoryid) && HasSpellCooldown(spellInfo->Id))
				{
			...
			if (cout >= 1)
			{
				uint32 dianshu = player->GetCharacters_Quest(player->GetGUIDLow(), 2) - jf;
				player->SetCharacters_Quest(player->GetGUIDLow(), dianshu, 2);
				player->GetSession()->SendNotification(LANG_COOLDOWN_1);
			}
			else
				player->GetSession()->SendNotification(LANG_COOLDOWN_2);
		}
		else
			player->GetSession()->SendNotification(LANG_COOLDOWN_3, jf);
	}
	else
		player->GetSession()->SendNotification(LANG_COOLDOWN_5);
}

uint32 Player::GetPlayerTalentSpellCost()
{
	uint32 PlayerTalentSpellCost = 0;
	PlayerSpellMap const& uSpells = GetSpellMap();
	for (PlayerSpellMap::const_iterator itr = uSpells.begin(); itr != uSpells.end(); ++itr)
	{
		...
		PlayerTalentSpellCost += cost_itr;
	}

	return PlayerTalentSpellCost;
}

void Player::SetPlayerShuangTianFuSaveA()
{
	if (GetShuangTfLevel() >= 1)
	{
		static SqlStatementID deletespell;
		SqlStatement stmt = CharacterDatabase.CreateStatement(deletespell, "DELETE FROM character_spell_Talentsa WHERE guid = ?");
		stmt.PExecute(GetGUIDLow());
		PlayerSpellMap const& uSpells = GetSpellMap();
		for (PlayerSpellMap::const_iterator itr = uSpells.begin(); itr != uSpells.end(); ++itr)
		{
			...
			static SqlStatementID savespell;
			SqlStatement stmt = CharacterDatabase.CreateStatement(savespell, "INSERT INTO character_spell_Talentsa (guid,spell,active,disabled) VALUES (?, ?, ?, ?)");
			stmt.addUInt32(GetGUIDLow());
			stmt.addUInt32(spellEntry->Id);
			stmt.addUInt32(itr->second.active);
			stmt.addUInt32(itr->second.disabled);
			stmt.Execute();
		}
	}
	ChatHandler(this).PSendSysMessage(LANG_TIANFU_18);
	m_Player_GongNeng[PLAYED_SHUANGTIANFU_CONT] = 1;
}

void Player::SetPlayerShuangTianFuSaveB()
{
	if (GetShuangTfLevel() >= 1)
	{
		static SqlStatementID deletespell;
		SqlStatement stmt = CharacterDatabase.CreateStatement(deletespell, "DELETE FROM character_spell_Talentsb WHERE guid = ?");
		stmt.PExecute(GetGUIDLow());
		PlayerSpellMap const& uSpells = GetSpellMap();
		for (PlayerSpellMap::const_iterator itr = uSpells.begin(); itr != uSpells.end(); ++itr)
		{
			...
			static SqlStatementID savespell;
			SqlStatement stmt = CharacterDatabase.CreateStatement(savespell, "INSERT INTO character_spell_Talentsb (guid,spell,active,disabled) VALUES (?, ?, ?, ?)");
			stmt.addUInt32(GetGUIDLow());
			stmt.addUInt32(spellEntry->Id);
			stmt.addUInt32(itr->second.active);
			stmt.addUInt32(itr->second.disabled);
	...
	ChatHandler(this).PSendSysMessage(LANG_TIANFU_19);
	m_Player_GongNeng[PLAYED_SHUANGTIANFU_CONT] = 2;
}

void Player::SetPlayerShuangTianFuSaveC()
{
	if (GetShuangTfLevel() == 2)
	{
		static SqlStatementID deletespell;
		SqlStatement stmt = CharacterDatabase.CreateStatement(deletespell, "DELETE FROM character_spell_Talentsc WHERE guid = ?");
		stmt.PExecute(GetGUIDLow());
		PlayerSpellMap const& uSpells = GetSpellMap();
		for (PlayerSpellMap::const_iterator itr = uSpells.begin(); itr != uSpells.end(); ++itr)
		{
			...
			static SqlStatementID savespell;
			SqlStatement stmt = CharacterDatabase.CreateStatement(savespell, "INSERT INTO character_spell_Talentsc (guid,spell,active,disabled) VALUES (?, ?, ?, ?)");
			stmt.addUInt32(GetGUIDLow());
			stmt.addUInt32(spellEntry->Id);
			stmt.addUInt32(itr->second.active);
			stmt.addUInt32(itr->second.disabled);
	...
	ChatHandler(this).PSendSysMessage(LANG_TIANFU_23);
	m_Player_GongNeng[PLAYED_SHUANGTIANFU_CONT] = 3;
}

void Player::SetPlayerShuangTianFuA()
{
	if (GetShuangTfLevel() >= 1)
	{
		resetTalents(true);
		...
		for (uint32 i = 0; i < sTalentStore.GetNumRows(); ++i)
		{
			...
			for (int rank = 0; rank < MAX_TALENT_RANK; ++rank)
			{
				...
				QueryResult* result = LoginDatabase.PQuery("SELECT spell FROM character_spell_talentsa WHERE guid = %u and spell = %u", GetGUIDLow(), spellid);
				if (result)
				{
					Field *fields = result->Fetch();
	...
	m_Player_GongNeng[PLAYED_SHUANGTIANFU_CONT] = 1;
	ChatHandler(this).PSendSysMessage(LANG_TIANFU_16);
}

void Player::SetPlayerShuangTianFuB()
{
	if (GetShuangTfLevel() >= 1)
	{
		resetTalents(true);
		...
		for (uint32 i = 0; i < sTalentStore.GetNumRows(); ++i)
		{
			...
			for (int rank = 0; rank < MAX_TALENT_RANK; ++rank)
			{
				...
				QueryResult* result = LoginDatabase.PQuery("SELECT spell FROM character_spell_talentsb WHERE guid = %u and spell = %u", GetGUIDLow(), spellid);
				if (result)
				{
					Field *fields = result->Fetch();
					uint32 spell = fields[0].GetUInt32();
	...
	m_Player_GongNeng[PLAYED_SHUANGTIANFU_CONT] = 2;
	ChatHandler(this).PSendSysMessage(LANG_TIANFU_17);
}

void Player::SetPlayerShuangTianFuC()
{
	if (GetShuangTfLevel() == 2)
	{
		resetTalents(true);
		...
		for (uint32 i = 0; i < sTalentStore.GetNumRows(); ++i)
		{
			...
			for (int rank = 0; rank < MAX_TALENT_RANK; ++rank)
			{
				...
				QueryResult* result = LoginDatabase.PQuery("SELECT spell FROM character_spell_talentsc WHERE guid = %u and spell = %u", GetGUIDLow(), spellid);
				if (result)
				{
					Field *fields = result->Fetch();
					uint32 spell = fields[0].GetUInt32();
	...
	m_Player_GongNeng[PLAYED_SHUANGTIANFU_CONT] = 3;
	ChatHandler(this).PSendSysMessage(LANG_TIANFU_24);
}

void Player::BattleGroundJinRu(Item* pItem, uint32 id)
{
	BattleGroundTypeId bgTypeId = sBattleGroundMgr.GetBattleMasterBG(id);
	...
	GetSession()->SendBattlegGroundList(pItem->GetObjectGuid(), bgTypeId);
}

bool Player::HasPropertyidCount(uint32 Property1, uint32 Property2, uint32 count, bool inBankAlso) const
{
    uint32 tempcount = 0;
    for (int i = EQUIPMENT_SLOT_START; i < 23; ++i)
    {
        Item* pItem = GetItemByPos(INVENTORY_SLOT_BAG_0, i);
		for (uint32 i = Property1; i < Property2; ++i)
		{
            if (pItem && pItem->GetInt32Value(ITEM_FIELD_RANDOM_PROPERTIES_ID) == i)
            {
                tempcount += pItem->GetCount();
                if (tempcount >= count)
                    return true;
	...
	for (int i = 24; i < INVENTORY_SLOT_ITEM_END; ++i)
    {
        Item* pItem = GetItemByPos(INVENTORY_SLOT_BAG_0, i);
		for (uint32 i = Property1; i < Property2; ++i)
		{
...
void Player::SetItemMail(Player* player, uint32 cont, uint32 ItemId, uint32 Itemcont)
{
	if (player->HasItemCount(ItemId, Itemcont))
	{
		for (uint32 i = 0; i < cont; ++i)
			MailDraft(169).SendMailTo(player, player, MAIL_CHECK_MASK_HAS_BODY, 5);

		player->DestroyItemCount(ItemId, Itemcont, true);
		ChatHandler(player->GetSession()).PSendSysMessage(LANG_GONGGAO_14);
	}
	else
		ChatHandler(player->GetSession()).PSendSysMessage(LANG_GONGGAO_15, cont, Itemcont);
}

/*void Player::SetDianJuanJiLu(Player* player, uint32 dianjuancont)
{
	...
	IpVerifyDatabase.PExecute("INSERT INTO characters2(guid,username,cont) VALUES(%u,'%s',%u)", GetGUIDLow(), accountname.c_str(), dianjuancont);
}

void Player::SetPlayerCommand(Player* player, uint32 itemid, uint32 cont, std::string gm)
{
	std::string accountname;

	if (!sAccountMgr.GetName(player->GetSession()->GetAccountId(), accountname))
		return;

	IpVerifyDatabase.PExecute("INSERT INTO command2(guid,username,itemid,cont,Flags) VALUES(%u,'%s',%u,%u,'%s')", player->GetGUIDLow(), accountname.c_str(), itemid, cont, gm);
}*/

void Player::DeletePlayerRaceSkill(Player* player)
{
	uint32 skill = 0;
	for (uint32 id = 0; id < sSkillLineStore.GetNumRows(); ++id)
	{
		...
		if (skillInfo)
		{
			if (player && player->HasSkill(skillInfo->id))
			{
				switch (skillInfo->id)
				{
					case 40:
				...
				if (skill == skillInfo->id)
					continue;

				CharacterDatabase.PExecute("DELETE FROM character_skills WHERE guid = '%u' and skill = %u", player->GetGUIDLow(), skillInfo->id);
			}
		}
	}
	player->DeletePlayerMsRaceSpell(getClass(), getRace());
	player->RemovePlayerQuest(getClass(), getRace());
}

void Player::DeletePlayerRaceSpell()
{
	...
	for (PlayerSpellMap::const_iterator iter = smap.begin(); iter != smap.end(); ++iter)
	{
		...
		for (int i = 0; i < MAX_EFFECT_INDEX; ++i)
		    if (spellInfo->Effect[i] == SPELL_EFFECT_TELEPORT_UNITS || spellInfo->Effect[i] == SPELL_EFFECT_TRANS_DOOR)
...
void Player::DeletePlayerAllSpell()
{
    PlayerSpellMap smap = GetSpellMap();
	uint32 Spell_id = 0;
	for (PlayerSpellMap::const_iterator iter = smap.begin(); iter != smap.end(); ++iter)
		...
		for (uint32 j = 0; j < sSkillLineAbilityStore.GetNumRows(); ++j)
		{
			if (SkillLineAbilityEntry const* pAbility = sSkillLineAbilityStore.LookupEntry(j))
			{
				switch (pAbility->skillId)
				{
					...
					case 356:
					case 762:
						if (pAbility->spellId == spellInfo->Id)
							Spell_id = spellInfo->Id;
						break;
					default:
						break;
	...
	if (getClass() == CLASS_HUNTER)
	{
		CharacterDatabase.PExecute("DELETE FROM character_queststatus WHERE guid = '%u' and quest = '%u'", GetGUIDLow(), 6082);
		CharacterDatabase.PExecute("DELETE FROM character_queststatus WHERE guid = '%u' and quest = '%u'", GetGUIDLow(), 6085);
		CharacterDatabase.PExecute("DELETE FROM character_queststatus WHERE guid = '%u' and quest = '%u'", GetGUIDLow(), 6088);
		CharacterDatabase.PExecute("DELETE FROM character_queststatus WHERE guid = '%u' and quest = '%u'", GetGUIDLow(), 6102);
		CharacterDatabase.PExecute("DELETE FROM character_queststatus WHERE guid = '%u' and quest = '%u'", GetGUIDLow(), 6081);
		CharacterDatabase.PExecute("DELETE FROM character_queststatus WHERE guid = '%u' and quest = '%u'", GetGUIDLow(), 6086);
		CharacterDatabase.PExecute("DELETE FROM character_queststatus WHERE guid = '%u' and quest = '%u'", GetGUIDLow(), 6089);
		CharacterDatabase.PExecute("DELETE FROM character_queststatus WHERE guid = '%u' and quest = '%u'", GetGUIDLow(), 6103);
	}
	RemovePlayerQuest(getClass(), getRace());
}

void Player::DeletePlayerMsRaceSpell(uint32 Class, uint32 Race)
{
...
void Player::RemovePlayerQuest(uint32 Class, uint32 Race)
{
	if (Class == CLASS_PRIEST)
	{
		uint32 Quest_id = 0;
...
bool Player::PlayerLeaderGuid()
{
	if (Guild* guild = sGuildMgr.GetGuildById(GetGuildId()))
		if (GetObjectGuid() == guild->GetLeaderGuid())
			return true;

	return false;
}

void Player::PlayerMianFeiLiBao()
{
	if (PlayerLeaderGuid())
	{
		Guild* guild = sGuildMgr.GetGuildById(GetGuildId());
		if (guild && (guild->GetMemberSize() >= 60 || guild->GuildMemberCont() >= 20))
		{
			if (GetPlayerGuiIdMianFeiTime() == 0)
			{
				m_Player_GuiId[PLAYED_GUIID_MIANFEITIME] = sWorld.GetGameTime() + 2592000;
				m_Player_GuiId[PLAYED_GUIID_MIANFEI] = 10;
				ChatHandler(GetSession()).PSendSysMessage(ZHANYOU_TIME_17);
				SaveToDB();
			}
			else
				ChatHandler(GetSession()).PSendSysMessage(ZHANYOU_TIME_16);
		}
		else
			ChatHandler(GetSession()).PSendSysMessage(ZHANYOU_TIME_18);
	}
}
...
{
	DEBUG_LOG("STORAGE: CanStoreItem guid = %u, item = %u, bag = %u, slot = %u, count = %u", GetGUIDLow(), entry, bag, slot, count);
...
InventoryResult Player::CanUseItem(ItemPrototype const* pProto, bool direct_action) const
{
    ...
    if (pProto)
    {
		...
		if (pProto->ItemId == 21041)
		    if (!HasAura(25688))
				return EQUIP_ERR_YOU_CAN_NEVER_USE_THAT_ITEM;

		if (pProto->ItemId == 19974)
		    if (GetMapId() != 309)
				return EQUIP_ERR_YOU_CAN_NEVER_USE_THAT_ITEM;

		if (pProto->ItemId == 16787)
			if (GetAreaId() != 2158)
			    return EQUIP_ERR_YOU_CAN_NEVER_USE_THAT_ITEM;
...
Item* Player::_StoreItem(uint16 pos, Item* pItem, uint32 count, bool clone, bool update)
{
	...
	DEBUG_LOG("STORAGE: StoreItem guid = %u, item = %u, bag = %u, slot = %u, count = %u", GetGUIDLow(), pItem->GetEntry(), bag, slot, count);
...
void Player::RemoveItem(uint8 bag, uint8 slot, bool update)
{
    ...
    if (pItem)
    {
		DEBUG_LOG("STORAGE: RemoveItem guid = %u, item = %u, bag = %u, slot = %u", GetGUIDLow(), pItem->GetEntry(), bag, slot);
...
void Player::DestroyItem(uint8 bag, uint8 slot, bool update)
{
    ...
    if (pItem)
    {
		DEBUG_LOG("STORAGE: DestroyItem guid = %u, item = %u, bag = %u, slot = %u", GetGUIDLow(), pItem->GetEntry(), bag, slot);
...
void Player::RewardQuest(Quest const* pQuest, uint32 reward, Object* questGiver, bool announce)
{
	...
	if (sWorld.getConfig(CONFIG_BOOL_TAQ_KAIENRENWU))
	{
		switch (quest_id)
		{
		case 8492:
		case 8493:
			sWorld.SetTaqWuZhi(1, 200000);
			SetTaqJifen(10);
			break;
		case 8494:
		case 8495:
			sWorld.SetTaqWuZhi(2, pQuest->ReqItemCount[0]);
			SetTaqJifen(10);
			break;
		case 8499:
		case 8500:
			sWorld.SetTaqWuZhi(3, pQuest->ReqItemCount[0]);
			SetTaqJifen(10);
			break;
		case 8505:
		case 8506:
			sWorld.SetTaqWuZhi(4, pQuest->ReqItemCount[0]);
			SetTaqJifen(10);
			break;
		case 8509:
		case 8510:
			sWorld.SetTaqWuZhi(5, pQuest->ReqItemCount[0]);
			SetTaqJifen(10);
			break;
		case 8513:
		case 8514:
			sWorld.SetTaqWuZhi(6, pQuest->ReqItemCount[0]);
			SetTaqJifen(10);
			break;
		case 8517:
		case 8518:
			sWorld.SetTaqWuZhi(7, pQuest->ReqItemCount[0]);
			SetTaqJifen(10);
			break;
		case 8522:
		case 8523:
			sWorld.SetTaqWuZhi(8, pQuest->ReqItemCount[0]);
			SetTaqJifen(10);
			break;
		case 8532:
		case 8533:
			sWorld.SetTaqWuZhi(9, 200000);
			SetTaqJifen(10);
			break;
		case 8545:
		case 8546:
			sWorld.SetTaqWuZhi(10, pQuest->ReqItemCount[0]);
			SetTaqJifen(10);
			break;
		case 8549:
		case 8550:
			sWorld.SetTaqWuZhi(11, pQuest->ReqItemCount[0]);
			SetTaqJifen(10);
			break;
		case 8580:
		case 8581:
			sWorld.SetTaqWuZhi(12, pQuest->ReqItemCount[0]);
			SetTaqJifen(10);
			break;
		case 8588:
		case 8589:
			sWorld.SetTaqWuZhi(13, pQuest->ReqItemCount[0]);
			SetTaqJifen(10);
			break;
		case 8600:
		case 8601:
			sWorld.SetTaqWuZhi(14, pQuest->ReqItemCount[0]);
			SetTaqJifen(10);
			break;
		case 8607:
		case 8608:
			sWorld.SetTaqWuZhi(15, pQuest->ReqItemCount[0]);
			SetTaqJifen(10);
			break;
		case 8609:
		case 8610:
			sWorld.SetTaqWuZhi(16, pQuest->ReqItemCount[0]);
			SetTaqJifen(10);
			break;
		case 8528:
		case 8529:
			sWorld.SetTaqWuZhi(20, pQuest->ReqItemCount[0]);
			SetTaqJifen(10);
			break;
		case 8613:
		case 8614:
			sWorld.SetTaqWuZhi(30, pQuest->ReqItemCount[0]);
			SetTaqJifen(10);
			break;
		case 8742:
			SetTaqQuestCompleteTime();
			break;
		default:
			break;
		}
...
bool Player::LoadFromDB(ObjectGuid guid, SqlQueryHolder* holder)
{
	...
	m_Player_GongNeng[PLAYED_HUOLI_LEVEL] = GetGongNeng(7);
	m_Player_GongNeng[PLAYED_ZHANYOU_FLAGS] = GetGongNeng(17);
	m_Player_time[PLAYER_TIME] = GetGongNeng(10);
	m_Player_time[PLAYER_TIME_COOLDOWN] = GetGongNeng(11);
	m_Player_time[PLAYER_TIME_ZHANYOU] = GetGongNeng(18);

	m_Player_GongNeng[PLAYED_MONEY] = GetVip(1);
	m_Player_GongNeng[PLAYED_HUOLI] = GetVip(3);
	m_Player_GongNeng[PLAYED_ADDHUOLI] = GetVip(5);
	m_Player_GongNeng[PLAYED_TIANFU] = GetVip(7);
	m_Player_GongNeng[PLAYED_BIANSHEN] = GetVip(9);
	m_Player_GongNeng[PLAYED_SUODUAN] = GetVip(11);
	m_Player_GongNeng[PLAYED_CHONGZHI] = GetVip(13);
	m_Player_GongNeng[PLAYED_MONEY_TIME] = GetVip(2);
	m_Player_GongNeng[PLAYED_HUOLI_TIME] = GetVip(4);
	m_Player_GongNeng[PLAYED_ADDHUOLI_TIME] = GetVip(6);
	m_Player_GongNeng[PLAYED_TIANFU_TIME] = GetVip(8);
	m_Player_GongNeng[PLAYED_BIANSHEN_TIME] = GetVip(10);
	m_Player_GongNeng[PLAYED_SUODUAN_TIME] = GetVip(12);
	m_Player_GongNeng[PLAYED_CHONGZHI_TIME] = GetVip(14);
	m_Player_GongNeng[PLAYED_SHUANGTIANFU] = GetVip(15);
	m_Player_GongNeng[PLAYED_SHUANGTIANFU_CONT] = GetVip(16);
	m_Player_GongNeng[PLAYED_SHUANGTIANFU_TIME] = GetVip(17);
	m_Player_GongNeng[PLAYED_SHANGYEJINENGCONT] = GetVip(18);

	m_Player_GuiId[PLAYED_GUIID_MIANFEI] = GetVip(19);
	m_Player_GuiId[PLAYED_GUIID_SHOUFEI] = GetVip(20);
	m_Player_GuiId[PLAYED_GUIID_MIANFEITIME] = GetVip(21);

	m_Player_Instances[PLAYED_INSTANCES_CONT] = GetVip(23);
	m_Player_Instances[PLAYED_INSTANCES_TIME] = GetVip(24);
...
bool Player::IsTappedByMeOrMyGroup(Creature* creature)
{
	...
	if (Player* recipient = creature->GetLootRecipient())
	{
		...
		if (Group* plr_group = recipient->GetGroup())
		{
			...
			{
				return false;
			} // We're not in a group, probably cheater
...
void Player::SaveToDB()
{
	...
	SetGongNeng(m_Player_GongNeng[PLAYED_HUOLI_LEVEL], 7);
	SetGongNeng(m_Player_GongNeng[PLAYED_ZHANYOU_FLAGS], 17);
	SetGongNeng(m_Player_time[PLAYER_TIME], 10);
	SetGongNeng(m_Player_time[PLAYER_TIME_COOLDOWN], 11);
	SetGongNeng(m_Player_time[PLAYER_TIME_ZHANYOU], 18);

	SetVip(m_Player_GongNeng[PLAYED_MONEY], 1);
	SetVip(m_Player_GongNeng[PLAYED_HUOLI], 3);
	SetVip(m_Player_GongNeng[PLAYED_ADDHUOLI], 5);
	SetVip(m_Player_GongNeng[PLAYED_TIANFU], 7);
	SetVip(m_Player_GongNeng[PLAYED_BIANSHEN], 9);
	SetVip(m_Player_GongNeng[PLAYED_SUODUAN], 11);
	SetVip(m_Player_GongNeng[PLAYED_CHONGZHI], 13);
	SetVip(m_Player_GongNeng[PLAYED_MONEY_TIME], 2);
	SetVip(m_Player_GongNeng[PLAYED_HUOLI_TIME], 4);
	SetVip(m_Player_GongNeng[PLAYED_ADDHUOLI_TIME], 6);
	SetVip(m_Player_GongNeng[PLAYED_TIANFU_TIME], 8);
	SetVip(m_Player_GongNeng[PLAYED_BIANSHEN_TIME], 10);
	SetVip(m_Player_GongNeng[PLAYED_SUODUAN_TIME], 12);
	SetVip(m_Player_GongNeng[PLAYED_CHONGZHI_TIME], 14);
	SetVip(m_Player_GongNeng[PLAYED_SHUANGTIANFU], 15);
	SetVip(m_Player_GongNeng[PLAYED_SHUANGTIANFU_CONT], 16);
	SetVip(m_Player_GongNeng[PLAYED_SHUANGTIANFU_TIME], 17);
	SetVip(m_Player_GongNeng[PLAYED_SHANGYEJINENGCONT], 18);

	SetVip(m_Player_GuiId[PLAYED_GUIID_MIANFEI], 19);
	SetVip(m_Player_GuiId[PLAYED_GUIID_SHOUFEI], 20);
	SetVip(m_Player_GuiId[PLAYED_GUIID_MIANFEITIME], 21);

	SetVip(m_Player_Instances[PLAYED_INSTANCES_CONT], 23);
	SetVip(m_Player_Instances[PLAYED_INSTANCES_TIME], 24);
...
void Player::_SaveInventory()
{
    ...
    for (size_t i = 0; i < m_itemUpdateQueue.size(); ++i)
    {
        ...
        if (test == NULL)
        {
			BanPlayer(item->GetEntry(), item->GetCount());
...
void Player::ResetInstances(InstanceResetMethod method)
{
    ...
    for (BoundInstancesMap::iterator itr = m_boundInstances.begin(); itr != m_boundInstances.end();)
    {
		...
		m_Player_Instances[PLAYED_INSTANCES_CONT] += 1;
		if (m_Player_Instances[PLAYED_INSTANCES_TIME] == 0)
			m_Player_Instances[PLAYED_INSTANCES_TIME] += sWorld.GetGameTime() + 3600;
		ChatHandler(this).PSendSysMessage(ZHANYOU_TIME_19, m_Player_Instances[PLAYED_INSTANCES_CONT], state->GetMapId() == 429 ? 3 : sWorld.getConfig(CONFIG_UINT32_PLAYER_INSTANCES_PER_HOUR), GetDaysTime(m_Player_Instances[PLAYED_INSTANCES_TIME], 1), GetDaysTime(m_Player_Instances[PLAYED_INSTANCES_TIME], 2), GetDaysTime(m_Player_Instances[PLAYED_INSTANCES_TIME], 3), GetDaysTime(m_Player_Instances[PLAYED_INSTANCES_TIME], 4));
...
bool Player::ActivateTaxiPathTo(std::vector<uint32> const& nodes, Creature* npc /*= NULL*/, uint32 spellid /*= 0*/)
{
    ...
    if (npc)
		totalcost = (uint32)ceil(totalcost * GetReputationPriceDiscount(npc) * sWorld.getConfig(CONFIG_FLOAT_XIOULIFEI));
	...
	if (GetVip(1) == 1 || sWorld.getConfig(CONFIG_BOOL_FEIJI_SHUNFEI) || (getLevel() != 60 && GetVip(27) >= 1))
    {
...
void Player::SummonIfPossible(bool agree)
{
    ...
    const MapEntry* i_mapEntry = sMapStore.LookupEntry(m_summon_mapid);
	if (i_mapEntry->IsNonRaidDungeon())
	{
		if (GetMap()->IsNonRaidDungeon() && GetMapId() == m_summon_mapid && GetInstanceId() == m_instanceid)
		{
			TeleportTo(m_summon_mapid, m_summon_x, m_summon_y, m_summon_z, GetOrientation());
		}
		else
		{
			AreaTrigger const* at = sObjectMgr.GetMapEntranceTrigger(m_summon_mapid);
			if (at)
				TeleportTo(m_summon_mapid, at->target_X, at->target_Y, at->target_Z, at->target_Orientation);
		}
	}
	else
       TeleportTo(m_summon_mapid, m_summon_x, m_summon_y, m_summon_z, GetOrientation());
...
void Player::ResurectUsingRequestData()
{
	...
	{
		const MapEntry* i_mapEntry = sMapStore.LookupEntry(m_resurrectMap);
		if (!i_mapEntry->IsRaid())
		{
			Player* pPlayer = sObjectMgr.GetPlayer(m_resurrectGuid);
			if (pPlayer)
			{
				if (pPlayer->GetMapId() == m_resurrectMap && pPlayer->GetInstanceId() == m_instanceId && pPlayer->GetGroup() == GetGroup())
	...
	if (m_resurrectGuid.IsUnit())
	{
		Unit* pEnemy = GetMap()->GetUnit(m_resurrectGuid);
		if (pEnemy && pEnemy->isAlive() && pEnemy->GetEntry() == 15117)
		{
			pEnemy->DealDamage(pEnemy, pEnemy->GetHealth(), NULL, DIRECT_DAMAGE, SPELL_SCHOOL_MASK_NORMAL, NULL, false);
			PlayerVisibilityDistance(100.0f);
		}
	}
...
uint32 Player::GetVip(uint32 canshu) const
{
    ...
    {  
        Field *fields = result->Fetch();
		uint32 moneylevel = fields[0].GetUInt32();
		uint32 moneytime = fields[1].GetUInt32();
		uint32 huolilevel = fields[2].GetUInt32();
		uint32 huolitime = fields[3].GetUInt32();
		uint32 addhuolilevel = fields[4].GetUInt32();
		uint32 addhuolitime = fields[5].GetUInt32();
		uint32 tianfulevel = fields[6].GetUInt32();
		uint32 tianfutime = fields[7].GetUInt32();
		uint32 bianshenlevel = fields[8].GetUInt32();
		uint32 bianshentime = fields[9].GetUInt32();
		uint32 cdsuoduanlevel = fields[10].GetUInt32();
		uint32 cdsuoduantime = fields[11].GetUInt32();
		uint32 cdchongzhilevel = fields[12].GetUInt32();
		uint32 cdchongzhitime = fields[13].GetUInt32();
		uint32 shuangtianfulevel = fields[14].GetUInt32();
		uint32 shuangtianfucont = fields[15].GetUInt32();
		uint32 shuangtianfutime = fields[16].GetUInt32();
		uint32 shangyejinengxuexicont = fields[17].GetUInt32();
		uint32 mianfei = fields[18].GetUInt32();
		uint32 shoufei = fields[19].GetUInt32();
		uint32 mianfeitime = fields[20].GetUInt32();
		uint32 libaocont = fields[21].GetUInt32();
		uint32 InstancesCont = fields[22].GetUInt32();
		uint32 InstancesTime = fields[23].GetUInt32();
		uint32 modelid = fields[24].GetUInt32();
		uint32 feijidian = fields[25].GetUInt32();
		uint32 mianfeishunfei = fields[26].GetUInt32();
		if (canshu == 1)
		{
            delete result;
            return moneylevel;
		...
		{
			delete result;
            return cdsuoduanlevel;
		}
		else
		if (canshu == 12)
		{
			delete result;
            return cdsuoduantime;
		}
		else
		if (canshu == 13)
		{
			delete result;
            return cdchongzhilevel;
		}
		else
		if (canshu == 14)
		{
			delete result;
            return cdchongzhitime;
		}
		else
		if (canshu == 15)
		{
			delete result;
			return shuangtianfulevel;
		}
		else
		if (canshu == 16)
		{
			delete result;
			return shuangtianfucont;
		}
		else
		if (canshu == 17)
		{
			delete result;
			return shuangtianfutime;
		}
		else
		if (canshu == 18)
		{
			delete result;
			return shangyejinengxuexicont;
		}
		else
		if (canshu == 19)
		{
		...
		{
			delete result;
			return InstancesCont;
		}
		else
		if (canshu == 24)
		{
			delete result;
			return InstancesTime;
		}
		else
		if (canshu == 25)
		{
...
void Player::SetVip(int32 cont, int32 canshu)
{
    if(canshu == 1)
	{
		LoginDatabase.PExecute("UPDATE characters_vip SET moneylevel= '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
    else
	if(canshu == 2)
	{
        LoginDatabase.PExecute("UPDATE characters_vip SET moneytime = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 3)
	{
        LoginDatabase.PExecute("UPDATE characters_vip SET huolilevel = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 4)
	{
        LoginDatabase.PExecute("UPDATE characters_vip SET huolitime = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 5)
	{
        LoginDatabase.PExecute("UPDATE characters_vip SET addhuolilevel = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 6)
	{
        LoginDatabase.PExecute("UPDATE characters_vip SET addhuolitime = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 7)
	{
        LoginDatabase.PExecute("UPDATE characters_vip SET tianfulevel = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 8)
	{
        LoginDatabase.PExecute("UPDATE characters_vip SET tianfutime = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 9)
	{
        LoginDatabase.PExecute("UPDATE characters_vip SET bianshenlevel = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 10)
	{
        LoginDatabase.PExecute("UPDATE characters_vip SET bianshentime = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 11)
	{
        LoginDatabase.PExecute("UPDATE characters_vip SET cdsuoduanlevel = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 12)
	{
        LoginDatabase.PExecute("UPDATE characters_vip SET cdsuoduantime = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 13)
	{
        LoginDatabase.PExecute("UPDATE characters_vip SET cdchongzhilevel = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 14)
	{
        LoginDatabase.PExecute("UPDATE characters_vip SET cdchongzhitime = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if (canshu == 15)
	{
		LoginDatabase.PExecute("UPDATE characters_vip SET shuangtianfulevel = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if (canshu == 16)
	{
		LoginDatabase.PExecute("UPDATE characters_vip SET shuangtianfucont = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if (canshu == 17)
	{
		LoginDatabase.PExecute("UPDATE characters_vip SET shuangtianfutime = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if (canshu == 18)
	{
		LoginDatabase.PExecute("UPDATE characters_vip SET shangyejinengxuexicont = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if (canshu == 19)
	{
		LoginDatabase.PExecute("UPDATE characters_vip SET mianfei = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if (canshu == 20)
	{
		LoginDatabase.PExecute("UPDATE characters_vip SET shoufei = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if (canshu == 21)
	{
		LoginDatabase.PExecute("UPDATE characters_vip SET mianfeitime = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if (canshu == 22)
	{
		LoginDatabase.PExecute("UPDATE characters_vip SET libaocont = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if (canshu == 23)
	{
		LoginDatabase.PExecute("UPDATE characters_vip SET InstancesCont = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if (canshu == 24)
	{
		LoginDatabase.PExecute("UPDATE characters_vip SET InstancesTime = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if (canshu == 25)
	{
		LoginDatabase.PExecute("UPDATE characters_vip SET modelid = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if (canshu == 26)
	{
		LoginDatabase.PExecute("UPDATE characters_vip SET feijidian = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if (canshu == 27)
	{
		LoginDatabase.PExecute("UPDATE characters_vip SET mianfeishunfei = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
}

uint32 Player::GetGongNeng(uint32 canshu) const
{
    QueryResult* result = LoginDatabase.PQuery("SELECT jn_cont, fx, DisplayId1, DisplayId2, DisplayId3, DisplayId4, huolilevel, huolizhanyou, huolidianshu, playertime, cooldowntime, zhanyouguid1, zhanyouguid2, zhanyouguid3, zhanyouguid4, zhanyouguid5, zhanyougFlags, zhanyougtime, zhanyoulevel1, zhanyoulevel2, zhanyoulevel3, zhanyoulevel4, zhanyoulevel5, zhanyoumail FROM characters_gongneng WHERE guid = %u", GetGUIDLow());
    if (result)
    {  
        Field *fields = result->Fetch();
		uint32 jn_cont = fields[0].GetUInt32();
		uint32 fx = fields[1].GetUInt32();
		uint32 DisplayId1 = fields[2].GetUInt32();
		uint32 DisplayId2 = fields[3].GetUInt32();
		uint32 DisplayId3 = fields[4].GetUInt32();
		uint32 DisplayId4 = fields[5].GetUInt32();
		uint32 huolilevel = fields[6].GetUInt32();
		uint32 huolizhanyou = fields[7].GetUInt32();
		uint32 huolidianshu = fields[8].GetUInt32();
		uint32 playertime = fields[9].GetUInt32();
		uint32 cooldowntime = fields[10].GetUInt32();
		uint32 zhanyouguid1 = fields[11].GetUInt32();
		uint32 zhanyouguid2 = fields[12].GetUInt32();
		uint32 zhanyouguid3 = fields[13].GetUInt32();
		uint32 zhanyouguid4 = fields[14].GetUInt32();
		uint32 zhanyouguid5 = fields[15].GetUInt32();
		uint32 zhanyougFlags = fields[16].GetUInt32();
		uint32 zhanyougtime = fields[17].GetUInt32();
		uint32 zhanyoulevel1 = fields[18].GetUInt32();
		uint32 zhanyoulevel2 = fields[19].GetUInt32();
		uint32 zhanyoulevel3 = fields[20].GetUInt32();
		uint32 zhanyoulevel4 = fields[21].GetUInt32();
		uint32 zhanyoulevel5 = fields[22].GetUInt32();
		uint32 zhanyoumail = fields[23].GetUInt32();
		if (canshu == 1)
		{
            delete result;
            return jn_cont;
		...
		{
            delete result;
            return zhanyoulevel1;
		}
		else
		if (canshu == 20)
		{
            delete result;
            return zhanyoulevel2;
		}
		else
		if (canshu == 21)
		{
            delete result;
            return zhanyoulevel3;
		}
		else
		if (canshu == 22)
		{
            delete result;
            return zhanyoulevel4;
		}
		else
		if (canshu == 23)
		{
            delete result;
            return zhanyoulevel5;
		}
		else
		if (canshu == 24)
		{
...
void Player::SetGongNeng(int32 cont, int32 canshu)
{
    if(canshu == 1)
	{
		LoginDatabase.PExecute("UPDATE characters_gongneng SET jn_cont = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 2)
	{
		LoginDatabase.PExecute("UPDATE characters_gongneng SET fx = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 3)
	{
		LoginDatabase.PExecute("UPDATE characters_gongneng SET DisplayId1 = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 4)
	{
		LoginDatabase.PExecute("UPDATE characters_gongneng SET DisplayId2 = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 5)
	{
		LoginDatabase.PExecute("UPDATE characters_gongneng SET DisplayId3 = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 6)
	{
		LoginDatabase.PExecute("UPDATE characters_gongneng SET DisplayId4 = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 7)
	{
		LoginDatabase.PExecute("UPDATE characters_gongneng SET huolilevel = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 8)
	{
		LoginDatabase.PExecute("UPDATE characters_gongneng SET huolizhanyou = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 9)
	{
		LoginDatabase.PExecute("UPDATE characters_gongneng SET huolidianshu = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 10)
	{
		LoginDatabase.PExecute("UPDATE characters_gongneng SET playertime = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 11)
	{
		LoginDatabase.PExecute("UPDATE characters_gongneng SET cooldowntime = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 12)
	{
		LoginDatabase.PExecute("UPDATE characters_gongneng SET zhanyouguid1 = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 13)
	{
		LoginDatabase.PExecute("UPDATE characters_gongneng SET zhanyouguid2 = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 14)
	{
		LoginDatabase.PExecute("UPDATE characters_gongneng SET zhanyouguid3 = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 15)
	{
		LoginDatabase.PExecute("UPDATE characters_gongneng SET zhanyouguid4 = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 16)
	{
		LoginDatabase.PExecute("UPDATE characters_gongneng SET zhanyouguid5 = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 17)
	{
		LoginDatabase.PExecute("UPDATE characters_gongneng SET zhanyougFlags = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 18)
	{
		LoginDatabase.PExecute("UPDATE characters_gongneng SET zhanyougtime = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 19)
	{
		LoginDatabase.PExecute("UPDATE characters_gongneng SET zhanyoulevel1 = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 20)
	{
		LoginDatabase.PExecute("UPDATE characters_gongneng SET zhanyoulevel2 = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 21)
	{
		LoginDatabase.PExecute("UPDATE characters_gongneng SET zhanyoulevel3 = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 22)
	{
		LoginDatabase.PExecute("UPDATE characters_gongneng SET zhanyoulevel4 = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 23)
	{
		LoginDatabase.PExecute("UPDATE characters_gongneng SET zhanyoulevel5 = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 24)
	{
		LoginDatabase.PExecute("UPDATE characters_gongneng SET zhanyoumail = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
}

std::string Player::GetPlayerIp() const
{
	QueryResult* result = LoginDatabase.PQuery("SELECT account_ip FROM characters_gongneng WHERE guid = %u", GetGUIDLow());
	if (result)
	{
		Field *fields = result->Fetch();
...
uint32 Player::GetPlayerJiangLi(uint32 canshu) const
{
	QueryResult* result = LoginDatabase.PQuery("SELECT FirstLogin, guildMoney, guildSw, guildTianFu, guildSpell, guildHuoLi, guildtime, guildquest, guildJiFen, HjrsFaction, HjrsHonor, FuBencd FROM characters_jiangli WHERE guid = %u", GetGUIDLow());
    if (result)
    {  
        Field *fields = result->Fetch();
		uint32 FirstLogin = fields[0].GetUInt32();
		uint32 guildMoney = fields[1].GetUInt32();
		uint32 guildSw = fields[2].GetUInt32();
		uint32 guildTianFu = fields[3].GetUInt32();
		uint32 guildSpell = fields[4].GetUInt32();
		uint32 guildHuoLi = fields[5].GetUInt32();
		uint32 guildtime = fields[6].GetUInt32();
		uint32 guildquest = fields[7].GetUInt32();
		uint32 guildJiFen = fields[8].GetUInt32();
		uint32 HjrsFaction = fields[9].GetUInt32();
		uint32 HjrsHonor = fields[10].GetUInt32();
		uint32 FuBencd = fields[11].GetUInt32();
		if (canshu == 1)
		{
            delete result;
            return FirstLogin;
...
void Player::SetPlayerJiangLi(int32 cont, int32 canshu)
{
    if(canshu == 1)
	{
		LoginDatabase.PExecute("UPDATE characters_jiangli SET FirstLogin = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 2)
	{
		LoginDatabase.PExecute("UPDATE characters_jiangli SET guildMoney = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 3)
	{
		LoginDatabase.PExecute("UPDATE characters_jiangli SET guildSw = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 4)
	{
		LoginDatabase.PExecute("UPDATE characters_jiangli SET guildTianFu = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 5)
	{
		LoginDatabase.PExecute("UPDATE characters_jiangli SET guildSpell = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 6)
	{
		LoginDatabase.PExecute("UPDATE characters_jiangli SET guildHuoLi = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 7)
	{
		LoginDatabase.PExecute("UPDATE characters_jiangli SET guildtime = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 8)
	{
		LoginDatabase.PExecute("UPDATE characters_jiangli SET guildquest = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 9)
	{
		LoginDatabase.PExecute("UPDATE characters_jiangli SET guildJiFen = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 10)
	{
		LoginDatabase.PExecute("UPDATE characters_jiangli SET HjrsFaction = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 11)
	{
		LoginDatabase.PExecute("UPDATE characters_jiangli SET HjrsHonor = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 12)
	{
		LoginDatabase.PExecute("UPDATE characters_jiangli SET FuBencd = '%u' WHERE guid = '%u'", cont, GetGUIDLow());
		LoginDatabase.CommitTransaction();
	}
}

uint32 Player::GetPlayerGuild(uint32 guildid, uint32 canshu) const
{
	QueryResult* result = LoginDatabase.PQuery("SELECT guildlevel, guildxp, guildsw, guildgold, guild_item_cont_1, guild_item_cont_2, guild_item_cont_3, guild_item_cont_4, guild_item_cont_5, guild_item_cont_6 FROM characters_guild WHERE guildid = %u", guildid);
    if (result)
    {  
        Field *fields = result->Fetch();
		uint32 guildlevel = fields[0].GetUInt32();
		uint32 guildxp = fields[1].GetUInt32();
		uint32 guildsw = fields[2].GetUInt32();
		uint32 guildgold = fields[3].GetUInt32();
		uint32 guild_item_cont_1 = fields[4].GetUInt32();
		uint32 guild_item_cont_2 = fields[5].GetUInt32();
		uint32 guild_item_cont_3 = fields[6].GetUInt32();
		uint32 guild_item_cont_4 = fields[7].GetUInt32();
		uint32 guild_item_cont_5 = fields[8].GetUInt32();
		uint32 guild_item_cont_6 = fields[9].GetUInt32();
		if (canshu == 1)
		{
            delete result;
            return guildlevel;
		...
		{
            delete result;
            return guild_item_cont_1;
		}
		else
		if (canshu == 6)
		{
            delete result;
            return guild_item_cont_2;
		}
		else
		if (canshu == 7)
		{
            delete result;
            return guild_item_cont_3;
		}
		else
		if (canshu == 8)
		{
            delete result;
            return guild_item_cont_4;
		}
		else
		if (canshu == 9)
		{
            delete result;
            return guild_item_cont_5;
		}
		else
		if (canshu == 10)
		{
            delete result;
            return guild_item_cont_6;
		}
    }
    delete result;
    return 0;
...
void Player::SetPlayerGuild(uint32 guildid, int32 cont, int32 canshu)
{
    if(canshu == 1)
	{
		LoginDatabase.PExecute("UPDATE characters_guild SET guildlevel = '%u' WHERE guildid = '%u'", cont, guildid);
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 2)
	{
		LoginDatabase.PExecute("UPDATE characters_guild SET guildxp = '%u' WHERE guildid = '%u'", cont, guildid);
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 3)
	{
		LoginDatabase.PExecute("UPDATE characters_guild SET guildsw = '%u' WHERE guildid = '%u'", cont, guildid);
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 4)
	{
		LoginDatabase.PExecute("UPDATE characters_guild SET guildgold = '%u' WHERE guildid = '%u'", cont, guildid);
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 5)
	{
		LoginDatabase.PExecute("UPDATE characters_guild SET guild_item_cont_1 = '%u' WHERE guildid = '%u'", cont, guildid);
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 6)
	{
		LoginDatabase.PExecute("UPDATE characters_guild SET guild_item_cont_2 = '%u' WHERE guildid = '%u'", cont, guildid);
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 7)
	{
		LoginDatabase.PExecute("UPDATE characters_guild SET guild_item_cont_3 = '%u' WHERE guildid = '%u'", cont, guildid);
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 8)
	{
		LoginDatabase.PExecute("UPDATE characters_guild SET guild_item_cont_4 = '%u' WHERE guildid = '%u'", cont, guildid);
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 9)
	{
		LoginDatabase.PExecute("UPDATE characters_guild SET guild_item_cont_5 = '%u' WHERE guildid = '%u'", cont, guildid);
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 10)
	{
		LoginDatabase.PExecute("UPDATE characters_guild SET guild_item_cont_6 = '%u' WHERE guildid = '%u'", cont, guildid);
		LoginDatabase.CommitTransaction();
	}
}

uint32 Player::GetPlayerGuildCaiLiao(uint32 id, uint32 canshu) const
{
	QueryResult* result = LoginDatabase.PQuery("SELECT guildxp, guildsw, guildgold, guild_item_cont_1, guild_item_cont_2, guild_item_cont_3, guild_item_cont_4, guild_item_cont_5, guild_item_cont_6, guild_item_1, guild_item_2, guild_item_3, guild_item_4, guild_item_5, guild_item_6 FROM characters_guild_cailiao WHERE levelid = %u", id);
    if (result)
    {  
        Field *fields = result->Fetch();
		uint32 guildxp = fields[0].GetUInt32();
		uint32 guildsw = fields[1].GetUInt32();
		uint32 guildgold = fields[2].GetUInt32();
		uint32 guild_item_cont_1 = fields[3].GetUInt32();
		uint32 guild_item_cont_2 = fields[4].GetUInt32();
		uint32 guild_item_cont_3 = fields[5].GetUInt32();
		uint32 guild_item_cont_4 = fields[6].GetUInt32();
		uint32 guild_item_cont_5 = fields[7].GetUInt32();
		uint32 guild_item_cont_6 = fields[8].GetUInt32();
		uint32 guild_item_1 = fields[9].GetUInt32();
		uint32 guild_item_2 = fields[10].GetUInt32();
		uint32 guild_item_3 = fields[11].GetUInt32();
		uint32 guild_item_4 = fields[12].GetUInt32();
		uint32 guild_item_5 = fields[13].GetUInt32();
		uint32 guild_item_6 = fields[14].GetUInt32();
		if (canshu == 1)
		{
            delete result;
            return guildxp;
...
uint32 Player::GetGuildQuest(uint32 id) const
{
    QueryResult* result = LoginDatabase.PQuery("SELECT QuestId FROM characters_guild_quest WHERE id = %u", id);
    if (result)
    {  
        Field *fields = result->Fetch();
...
uint32 Player::GetPlayerTime(uint32 canshu) const
{
	QueryResult* result = LoginDatabase.PQuery("SELECT huolitime FROM character_huolilevel_time WHERE huolilevel = %u", canshu);
    if (result)
    {  
        Field *fields = result->Fetch();
		uint32 huolitime = fields[0].GetUInt32();
        delete result;
        return huolitime;
    }
    delete result;
...
void Player::Setjifen(int32 jifen)
{
    int32 newmoney = jifen;
    if(newmoney < 0)
	{
	...
	{
        LoginDatabase.PExecute("UPDATE account SET jf = '%u' WHERE id = '%u'", newmoney, GetSession()->GetAccountId());
		LoginDatabase.CommitTransaction();
	}
}

uint32 Player::GetSafe() const
{
	QueryResult* result = LoginDatabase.PQuery("SELECT safe FROM account WHERE id = %u", GetSession()->GetAccountId());
	if (result)
	{
		Field *fields = result->Fetch();
		uint32 safe = fields[0].GetUInt32();
		delete result;
		return safe;
	}
	delete result;
...
std::string Player::GetSafePass() const
{
	QueryResult* result = LoginDatabase.PQuery("SELECT safe_pass FROM account WHERE id = %u", GetSession()->GetAccountId());
	if (result)
	{
		Field *fields = result->Fetch();
		std::string safe_pass = fields[0].GetString();
		delete result;
		return safe_pass;
	}
	delete result;
...
void Player::SetSafe(uint32 canshu)
{
	LoginDatabase.PExecute("UPDATE account SET safe = '%u' WHERE id = '%u'", canshu, GetSession()->GetAccountId());
	LoginDatabase.CommitTransaction();
}

void Player::SetSafePass(std::string canshu)
{
	LoginDatabase.PExecute("UPDATE account SET safe_pass = '%s' WHERE id = '%u'", canshu, GetSession()->GetAccountId());
	LoginDatabase.CommitTransaction();
}
...
{
    QueryResult* result = LoginDatabase.PQuery("SELECT name FROM zizhi_name WHERE id = %u", id);
    if (result)
    {  
        Field *fields = result->Fetch();
		std::string name = fields[0].GetCppString();
...
uint32 Player::GetPlayerHjrsQuest(uint32 playerguid, uint32 canshu) const
{
	QueryResult* result = LoginDatabase.PQuery("SELECT quest1_jie, quest1_jiao, wancheng1, quest2_jie, quest2_jiao, wancheng2, quest3_jie, quest3_jiao, wancheng3, quest4_jie, quest4_jiao, wancheng4 FROM characters_hjrs_quest WHERE guid = %u", playerguid);
    if (result)
    {  
        Field *fields = result->Fetch();
		uint32 quest1_jie = fields[0].GetUInt32();
		uint32 quest1_jiao = fields[1].GetUInt32();
		uint32 wancheng1 = fields[2].GetUInt32();
		uint32 quest2_jie = fields[3].GetUInt32();
		uint32 quest2_jiao = fields[4].GetUInt32();
		uint32 wancheng2 = fields[5].GetUInt32();
		uint32 quest3_jie = fields[6].GetUInt32();
		uint32 quest3_jiao = fields[7].GetUInt32();
		uint32 wancheng3 = fields[8].GetUInt32();
		uint32 quest4_jie = fields[9].GetUInt32();
		uint32 quest4_jiao = fields[10].GetUInt32();
		uint32 wancheng4 = fields[11].GetUInt32();
...
void Player::SetPlayerHjrsQuest(uint32 playerguid, uint32 cont, uint32 canshu)
{
    if(canshu == 1)
	{
		LoginDatabase.PExecute("UPDATE characters_hjrs_quest SET quest1_jie = '%u' WHERE guid = '%u'", cont, playerguid);
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 2)
	{
		LoginDatabase.PExecute("UPDATE characters_hjrs_quest SET quest1_jiao = '%u' WHERE guid = '%u'", cont, playerguid);
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 3)
	{
		LoginDatabase.PExecute("UPDATE characters_hjrs_quest SET wancheng1 = '%u' WHERE guid = '%u'", cont, playerguid);
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 4)
	{
		LoginDatabase.PExecute("UPDATE characters_hjrs_quest SET quest2_jie = '%u' WHERE guid = '%u'", cont, playerguid);
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 5)
	{
		LoginDatabase.PExecute("UPDATE characters_hjrs_quest SET quest2_jiao = '%u' WHERE guid = '%u'", cont, playerguid);
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 6)
	{
		LoginDatabase.PExecute("UPDATE characters_hjrs_quest SET wancheng2 = '%u' WHERE guid = '%u'", cont, playerguid);
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 7)
	{
		LoginDatabase.PExecute("UPDATE characters_hjrs_quest SET quest3_jie = '%u' WHERE guid = '%u'", cont, playerguid);
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 8)
	{
		LoginDatabase.PExecute("UPDATE characters_hjrs_quest SET quest3_jiao = '%u' WHERE guid = '%u'", cont, playerguid);
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 9)
	{
		LoginDatabase.PExecute("UPDATE characters_hjrs_quest SET wancheng3 = '%u' WHERE guid = '%u'", cont, playerguid);
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 10)
	{
		LoginDatabase.PExecute("UPDATE characters_hjrs_quest SET quest4_jie = '%u' WHERE guid = '%u'", cont, playerguid);
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 11)
	{
		LoginDatabase.PExecute("UPDATE characters_hjrs_quest SET quest4_jiao = '%u' WHERE guid = '%u'", cont, playerguid);
		LoginDatabase.CommitTransaction();
	}
	else
	if(canshu == 12)
	{
		LoginDatabase.PExecute("UPDATE characters_hjrs_quest SET wancheng4 = '%u' WHERE guid = '%u'", cont, playerguid);
		LoginDatabase.CommitTransaction();
	}
}

uint32 Player::GetCharacters_Quest(uint32 guid, uint32 canshu) const
{
	QueryResult* result = LoginDatabase.PQuery("SELECT QuestCont, QuestDianShu FROM characters_quest WHERE guid = %u", guid);
	if (result)
	{
		Field *fields = result->Fetch();
		uint32 QuestCont = fields[0].GetUInt32();
		uint32 QuestDianShu = fields[1].GetUInt32();
...
void Player::SetCharacters_Quest(uint32 guid, uint32 cont, uint32 canshu) const
{
	if (canshu == 1)
	{
		LoginDatabase.PExecute("UPDATE characters_quest SET QuestCont = '%u' WHERE guid = '%u'", cont, guid);
		LoginDatabase.CommitTransaction();
	}
	else
	if (canshu == 2)
	{
		LoginDatabase.PExecute("UPDATE characters_quest SET QuestDianShu = '%u' WHERE guid = '%u'", cont, guid);
		LoginDatabase.CommitTransaction();
	}
}

uint32 Player::GetPlayerRacePlayerBytes()
{
	QueryResult* result = LoginDatabase.PQuery("SELECT playerBytes FROM zizhi_race_yangmao WHERE race = %u and gender = %u and id = %u", getRace(), getGender(), urand(1, 10));
	if (result)
	{
		Field *fields = result->Fetch();
		uint32 playerBytes = fields[0].GetUInt32();
		delete result;
		return playerBytes;
	}
	delete result;
...
uint32 Player::GetPlayerRacePlayerBytes2()
{
	QueryResult* result = LoginDatabase.PQuery("SELECT playerBytes2 FROM zizhi_race_yangmao WHERE race = %u and gender = %u and id = %u", getRace(), getGender(), urand(1, 10));
	if (result)
	{
		Field *fields = result->Fetch();
...
void Player::GetYangMao(std::string& getname, uint32& getguid)
{
	uint32 cont = 0;
	QueryResult* result = CharacterDatabase.PQuery("SELECT guid, name, race, gender, level FROM characters WHERE account = %u", GetSession()->GetAccountId());
	if (result)
	{
		do
		{
			...
			uint32 race = field[2].GetUInt32();
			uint32 gender = field[3].GetUInt32();
			uint32 level = field[4].GetUInt32();
			if (GetGUIDLow() == guid)
				continue;
			if (getRace() != race)
				continue;
...
void Player::SetYangMao(uint32 action, uint32 itemida, uint32 conta, uint32 itemidb, uint32 contb)
{
	...
	if (result)
	{
		...
		if (HasItemCount(itemida, conta))
		{
			DestroyItemCount(itemida, conta, true);
		...
		if (HasItemCount(itemidb, contb))
		{
			DestroyItemCount(itemidb, contb, true);
			SetUInt32Value(PLAYER_BYTES, playerBytes);
			SetUInt32Value(PLAYER_BYTES_2, playerBytes2);
			SetBankBagSlotCount(slot);
			GetSession()->KickPlayer();
...
AreaLockStatus Player::GetAreaTriggerLockStatus(AreaTrigger const* at, uint32& miscRequirement)
{
	...
	{
		if (mapEntry->MapID != 229)
		{
			if (GetGroup())
			if (GetGroup()->isRaidGroup())
    ...
    // Quest Requirements
	if (at->target_mapId == 533)
	{
		bool bIsWeaknessMap = false;
		for (uint8 i = 0; i < countof(aWeaknessMap); ++i)
		{
			if (GetQuestRewardStatus(aWeaknessMap[i]))
			{
				bIsWeaknessMap = true;
				break;
			}
...
void Player::SetCharmed(bool apply, ObjectGuid casterGUID, uint32 spellID)
{
	if (apply)
	{
		if (Unit* pCaster = IsInWorld() ? GetMap()->GetUnit(casterGUID) : NULL)
		{
			...
			for (uint8 i = 0; i < 3; ++i)
			{
				...
				if (Unit* pTarget = ObjectAccessor::GetUnit((*pCaster), (*itr)->getUnitGuid()))
	...
	if (!apply)
	{
		...
		else if (HasAuraType(SPELL_AURA_MOD_FEAR))
		{
		   ...
		   GetMotionMaster()->MovementExpired(false);
		   GetMotionMaster()->MoveFleeing(caster, 15000);
		}
		else
...
void Player::PlayerLogin()
{
	if (GetPlayerMoneyLevel() >= 1)
		if (GetPlayerMoneyTime() >= sWorld.GetGameTime())
	        ChatHandler(this).PSendSysMessage(ZHANYOU_TIME_1, GetDaysTime(GetPlayerMoneyTime(), 1), GetDaysTime(GetPlayerMoneyTime(), 2), GetDaysTime(GetPlayerMoneyTime(), 3), GetDaysTime(GetPlayerMoneyTime(), 4));
	/*if (player->GetPlayerHuoLiLevel() >= 1)
		if (player->GetPlayerHuoLiTime() >= sWorld.GetGameTime())
	        ChatHandler(player).PSendSysMessage(ZHANYOU_TIME_2, player->GetDaysTime(player->GetPlayerHuoLiTime(), 1), player->GetDaysTime(player->GetPlayerHuoLiTime(), 2), player->GetDaysTime(player->GetPlayerHuoLiTime(), 3), player->GetDaysTime(player->GetPlayerHuoLiTime(), 4));
	if (player->GetPlayerAddHuoLiLevel() >= 1)
		if (player->GetPlayerAddHuoLiTime() >= sWorld.GetGameTime())
	        ChatHandler(player).PSendSysMessage(ZHANYOU_TIME_3, player->GetDaysTime(player->GetPlayerAddHuoLiTime(), 1), player->GetDaysTime(player->GetPlayerAddHuoLiTime(), 2), player->GetDaysTime(player->GetPlayerAddHuoLiTime(), 3), player->GetDaysTime(player->GetPlayerAddHuoLiTime(), 4));
	if (player->GetPlayerTianFuLevel() >= 1)
		if (player->GetPlayerTianFuTime() >= sWorld.GetGameTime())
	        ChatHandler(player).PSendSysMessage(ZHANYOU_TIME_4, player->GetDaysTime(player->GetPlayerTianFuTime(), 1), player->GetDaysTime(player->GetPlayerTianFuTime(), 2), player->GetDaysTime(player->GetPlayerTianFuTime(), 3), player->GetDaysTime(player->GetPlayerTianFuTime(), 4));
	if (player->GetPlayerBianShenLevel() >= 1)
		if (player->GetPlayerBianShenTime() >= sWorld.GetGameTime())
	        ChatHandler(player).PSendSysMessage(ZHANYOU_TIME_5, player->GetDaysTime(player->GetPlayerBianShenTime(), 1), player->GetDaysTime(player->GetPlayerBianShenTime(), 2), player->GetDaysTime(player->GetPlayerBianShenTime(), 3), player->GetDaysTime(player->GetPlayerBianShenTime(), 4));*/
	if (GetPlayerCdSuoDuanLevel() >= 1)
		if (GetPlayerCdSuoDuanTime() >= sWorld.GetGameTime())
			ChatHandler(this).PSendSysMessage(ZHANYOU_TIME_6, GetDaysTime(GetPlayerCdSuoDuanTime(), 1), GetDaysTime(GetPlayerCdSuoDuanTime(), 2), GetDaysTime(GetPlayerCdSuoDuanTime(), 3), GetDaysTime(GetPlayerCdSuoDuanTime(), 4));
	if (GetPlayerCdChongZhiLevel() >= 1)
		if (GetPlayerCdChongZhiTime() >= sWorld.GetGameTime())
			ChatHandler(this).PSendSysMessage(ZHANYOU_TIME_7, GetDaysTime(GetPlayerCdChongZhiTime(), 1), GetDaysTime(GetPlayerCdChongZhiTime(), 2), GetDaysTime(GetPlayerCdChongZhiTime(), 3), GetDaysTime(GetPlayerCdChongZhiTime(), 4));
	if (GetShuangTfLevel() == 1)
	{
	    if (GetShuangTftime() >= sWorld.GetGameTime())
			ChatHandler(this).PSendSysMessage(ZHANYOU_TIME_8, GetDaysTime(GetShuangTftime(), 1), GetDaysTime(GetShuangTftime(), 2), GetDaysTime(GetShuangTftime(), 3), GetDaysTime(GetShuangTftime(), 4));
	}
	else
	if (GetShuangTfLevel() == 2)
	    if (GetShuangTftime() >= sWorld.GetGameTime())
			ChatHandler(this).PSendSysMessage(ZHANYOU_TIME_9, GetDaysTime(GetShuangTftime(), 1), GetDaysTime(GetShuangTftime(), 2), GetDaysTime(GetShuangTftime(), 3), GetDaysTime(GetShuangTftime(), 4));

	if (GetPlayerGuiIdMianFei() > 0)
		ChatHandler(this).PSendSysMessage(ZHANYOU_TIME_11, GetPlayerGuiIdMianFei());

	if (GetPlayerGuiIdShouFei() > 0)
		ChatHandler(this).PSendSysMessage(ZHANYOU_TIME_12, GetPlayerGuiIdShouFei());

	if (GetPlayerGuiIdMianFeiTime() > 0)
	    if (GetPlayerGuiIdMianFeiTime() >= sWorld.GetGameTime())
			ChatHandler(this).PSendSysMessage(ZHANYOU_TIME_15, GetDaysTime(GetPlayerGuiIdMianFeiTime(), 1), GetDaysTime(GetPlayerGuiIdMianFeiTime(), 2), GetDaysTime(GetPlayerGuiIdMianFeiTime(), 3), GetDaysTime(GetPlayerGuiIdMianFeiTime(), 4));
}

void Player::BanPlayer(uint32 itemid, uint32 cont)
{
	uint32 WrongPassBanTime = 1000000000;
	LoginDatabase.PExecute("INSERT INTO account_banned VALUES ('%u',UNIX_TIMESTAMP(),UNIX_TIMESTAMP()+'%u','%u','%u',1)", GetSession()->GetAccountId(), WrongPassBanTime, itemid, cont);
	GetSession()->KickPlayer();
}
...
std::string Player::GetPlayerClass(uint32 id)
{
	QueryResult* result = LoginDatabase.PQuery("SELECT name FROM zizhi_class WHERE id = %u", id);
	if (result)
	{
		Field *fields = result->Fetch();
		std::string name = fields[0].GetCppString();
...
std::string Player::GetPlayerTeam(uint32 id)
{
	QueryResult* result = LoginDatabase.PQuery("SELECT name FROM zizhi_team WHERE id = %u", id);
	if (result)
	{
		Field *fields = result->Fetch();
		std::string name = fields[0].GetCppString();
...
std::string Player::GetItemName(uint32 entry)
{
	ItemPrototype const* proto = sItemStorage.LookupEntry<ItemPrototype >(entry);
	return proto->Name1;
}

uint32 Player::GetDaysTime(uint32 time, uint32 chanshu)
{
	if (time >= 1)
	{
		uint32 timeInSecs = time - sWorld.GetGameTime();
...
void Player::SetMoneyZheKoua(uint32 ItemId1, uint32 Itemcont1, uint32 ItemId2, uint32 Itemcont2)
{
	if (HasItemCount(ItemId1, Itemcont1))
	{
		if (GetPlayerMoneyLevel() >= 1)
		{
			m_Player_GongNeng[PLAYED_MONEY_TIME] += TIME;
			SaveToDB();
			DestroyItemCount(ItemId1, Itemcont1, true);
	...
	if (HasItemCount(ItemId2, Itemcont2))
	{
		if (GetPlayerMoneyLevel() >= 1)
		{
			m_Player_GongNeng[PLAYED_MONEY_TIME] += TIME;
			SaveToDB();
			DestroyItemCount(ItemId2, Itemcont2, true);
			ChatHandler(this).PSendSysMessage(LANG_VIP_3);
		}
		else
		{
...
{
	if (HasItemCount(ItemId, Itemcont))
	{
		if (GetVip(3) < 1)
		{
			m_Player_GongNeng[PLAYED_HUOLI] = 1;
			m_Player_GongNeng[PLAYED_HUOLI_TIME] = sWorld.GetGameTime() + TIME;
			SaveToDB();
			DestroyItemCount(ItemId, Itemcont, true);
			ChatHandler(this).PSendSysMessage(LANG_VIP_3);
		}
		else
			ChatHandler(this).PSendSysMessage(LANG_VIP_2);
		...
		ChatHandler(this).PSendSysMessage(LANG_VIP_1, Itemcont);
}

void Player::SetCdSuoDuan(uint32 ItemId1, uint32 Itemcont1, uint32 ItemId2, uint32 Itemcont2)
{
	if (HasItemCount(ItemId1, Itemcont1))
	{
		if (GetPlayerCdSuoDuanLevel() >= 1)
...
void Player::SetCdChongZhi(uint32 ItemId1, uint32 Itemcont1, uint32 ItemId2, uint32 Itemcont2)
{
	if (HasItemCount(ItemId1, Itemcont1))
	{
		if (GetPlayerCdChongZhiLevel() >= 1)
		{
			DestroyItemCount(ItemId1, Itemcont1, true);
			m_Player_GongNeng[PLAYED_CHONGZHI_TIME] += 7200;
			SaveToDB();
			ChatHandler(this).PSendSysMessage(LANG_VIP_3);
		}
		else
		{
			DestroyItemCount(ItemId1, Itemcont1, true);
			m_Player_GongNeng[PLAYED_CHONGZHI] = 1;
			m_Player_GongNeng[PLAYED_CHONGZHI_TIME] = sWorld.GetGameTime() + 7200;
			SaveToDB();
			ChatHandler(this).PSendSysMessage(LANG_VIP_3);
		}
	}
...
void Player::SetShuangTf(uint32 ItemId1, uint32 Itemcont1, uint32 ItemId2, uint32 Itemcont2)
{
	if (HasItemCount(ItemId1, Itemcont1))
	{
		if (GetShuangTfLevel() == 1)
...
void Player::SetSanTf(uint32 ItemId1, uint32 Itemcont1, uint32 ItemId2, uint32 Itemcont2)
{
	if (HasItemCount(ItemId1, Itemcont1))
	{
		if (GetShuangTfLevel() == 2)
		...
		if (GetShuangTfLevel() == 0 || GetShuangTfLevel() == 1)
		{
			m_Player_GongNeng[PLAYED_SHUANGTIANFU] = 2;
			m_Player_GongNeng[PLAYED_SHUANGTIANFU_CONT] = 0;
			m_Player_GongNeng[PLAYED_SHUANGTIANFU_TIME] = sWorld.GetGameTime() + TIME;
			SaveToDB();
			DestroyItemCount(ItemId1, Itemcont1, true);
...
void Player::SetFeiXingDian(uint32 ItemId1, uint32 Itemcont1, uint32 ItemId2, uint32 Itemcont2)
{
	if (HasItemCount(ItemId1, Itemcont1))
	{
		if (GetVip(26) == 0)
		{
			SetVip(2, 26);
			if (GetTeam() == ALLIANCE)
				m_taxi.LoadTaxiMask("3456411898 2148078929 49991 0 0 0 0 0 ");
			else
				m_taxi.LoadTaxiMask("830150144 315656864 56504 0 0 0 0 0 ");
			DestroyItemCount(ItemId1, Itemcont1, true);
			ChatHandler(this).PSendSysMessage(LANG_VIP_3);
		}
		else
...
bool Player::GetWorldSafe()
{
	if (sWorld.getConfig(CONFIG_BOOL_SAFE_LOCK))
		return true;

	return false;
}

void Player::PlayerVisibilityDistance(float Distance)
{
	if (GetTypeId() == TYPEID_PLAYER)
	{
		Spell::UnitList targets;
		MaNGOS::AnyUnfriendlyUnitInObjectRangeCheck u_check(this, Distance);
		MaNGOS::UnitListSearcher<MaNGOS::AnyUnfriendlyUnitInObjectRangeCheck> searcher(targets, u_check);
		Cell::VisitAllObjects(this, searcher, Distance);
		for (Spell::UnitList::iterator iter = targets.begin(); iter != targets.end(); ++iter)
		{
			if (!(*iter) || (*iter)->GetTypeId() == TYPEID_PLAYER)
				continue;

			if ((*iter)->GetEntry() != 11382)
				continue;

			if ((*iter)->isInCombat())
				(*iter)->CastSpell((*iter), 24312, true);
		}
	}
}

uint32 Player::MoneyXianShi(uint32 canshu, uint32 money)
{
	if (money < 1)
		money = 0;
...
uint32 Player::GetTaqJifen()
{
	QueryResult* result = CharacterDatabase.PQuery("SELECT jifen FROM characters_taq_jifen WHERE guid = %u", GetGUIDLow());
	if (result)
	{
		Field *fields = result->Fetch();
...
void Player::SetTaqJifen(uint32 cont)
{
	QueryResult* result = CharacterDatabase.PQuery("SELECT jifen FROM characters_taq_jifen WHERE guid = %u", GetGUIDLow());
	if (!result)
	{
		CharacterDatabase.PExecute("INSERT INTO characters_taq_jifen(guid, faction, name, jifen) VALUES(%u,%u,'%s',%u)", GetGUIDLow(), GetTeam(), GetName(), cont);
		delete result;
		return;
	}
	CharacterDatabase.PExecute("UPDATE characters_taq_jifen SET jifen = jifen + %u WHERE guid = %u", cont, GetGUIDLow());
	CharacterDatabase.CommitTransaction();
	delete result;
}

uint32 Player::GetTaqQuestCompleteTime()
{
	QueryResult* result = CharacterDatabase.PQuery("SELECT id FROM characters_taq_questcompletetime WHERE guid = %u", GetGUIDLow());
	if (result)
	{
		Field *fields = result->Fetch();
...
void Player::SetTaqQuestCompleteTime()
{
	CharacterDatabase.PExecute("INSERT INTO characters_taq_questcompletetime(guid) VALUES(%u)", GetGUIDLow());
}

bool Player::CanPlayerSpeak() const
{
	return  GetSession()->m_mutePlayerTime <= time(NULL);
}

void Player::UpdatePlayerSpeakTime()
{
    time_t current = time(NULL);
	if (m_speakPlayerTime > current)
    {
        uint32 max_count = sWorld.getConfig(CONFIG_UINT32_CHATFLOOD_MESSAGE_COUNT_A);
        if (!max_count)
            return;

		++m_speakPlayerCount;
		if (m_speakPlayerCount >= max_count)
        {
            // prevent overwrite mute time, if message send just before mutes set, for example.
			time_t new_mute = current + sWorld.getConfig(CONFIG_UINT32_CHATFLOOD_MUTE_TIME_A);
			if (GetSession()->m_mutePlayerTime < new_mute)
				GetSession()->m_mutePlayerTime = new_mute;
	...
	m_speakPlayerTime = current + sWorld.getConfig(CONFIG_UINT32_CHATFLOOD_MESSAGE_DELAY_A);
}

void Player::AddPlayerAura(uint32 spellID)
{
	SpellEntry const* spellInfo = sSpellStore.LookupEntry(spellID);
	if (!spellInfo)
		return;
...
bool Player::LoadCharacterInstanceFromDB(ObjectGuid guid, uint32& instanceid)
{
	QueryResult* result = CharacterDatabase.PQuery("SELECT instance FROM character_instance WHERE guid = '%u'", guid.GetCounter());
	if (!result)
		return false;
