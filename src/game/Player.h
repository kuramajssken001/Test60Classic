...
static const uint32 aWeaknessMap[] = { 9121, 9122, 9123 };
...
enum PlayerGongNeng
{
	...
	PLAYED_MONEY_TIME           = 7,
    PLAYED_HUOLI_TIME           = 8,
	PLAYED_ADDHUOLI_TIME        = 9,
	PLAYED_TIANFU_TIME          = 10,
	PLAYED_BIANSHEN_TIME        = 11,
	PLAYED_SUODUAN_TIME         = 12,
	PLAYED_CHONGZHI_TIME        = 13,
	PLAYED_HUOLI_LEVEL          = 14,
	PLAYED_ZHANYOU_FLAGS        = 15,
	PLAYED_SHUANGTIANFU         = 16,
	PLAYED_SHUANGTIANFU_CONT    = 17,
	PLAYED_SHUANGTIANFU_TIME    = 18,
	PLAYED_SHANGYEJINENGCONT    = 19
};

enum PlayerInstances
{
	PLAYED_INSTANCES_CONT = 0,
	PLAYED_INSTANCES_TIME = 1
};

enum PlayerGuiId
{
	PLAYED_GUIID_MIANFEI = 0,
	PLAYED_GUIID_SHOUFEI = 1,
	PLAYED_GUIID_MIANFEITIME = 2
};

enum PlayeTimeIndex
{
    PLAYER_TIME                 = 0,
	PLAYER_TIME_COOLDOWN        = 1,
	PLAYER_TIME_ZHANYOU         = 2
};
...
class MANGOS_DLL_SPEC Player : public Unit
{
    ...
    public:
		...
		void SetSummonPoint(uint32 mapid, float x, float y, float z, uint32 instid)
        {
		...
		uint32 m_Player_time[MAX_PLAYED_TIME_INDEXX];
		uint32 GetPlayeTime() { return m_Player_time[PLAYER_TIME]; }
		uint32 GetCooldownPlayeTime() { return m_Player_time[PLAYER_TIME_COOLDOWN]; }
		uint32 GetZhanYouPlayeTime() { return m_Player_time[PLAYER_TIME_ZHANYOU]; }

		uint32 m_Player_GongNeng[MAX_PLAYED_GONGNENG];
        uint32 GetPlayerMoneyLevel() { return m_Player_GongNeng[PLAYED_MONEY]; }
        uint32 GetPlayerHuoLiLevel() { return m_Player_GongNeng[PLAYED_HUOLI]; }
		uint32 GetPlayerAddHuoLiLevel() { return m_Player_GongNeng[PLAYED_ADDHUOLI]; }
		uint32 GetPlayerTianFuLevel() { return m_Player_GongNeng[PLAYED_TIANFU]; }
		uint32 GetPlayerBianShenLevel() { return m_Player_GongNeng[PLAYED_BIANSHEN]; }
		uint32 GetPlayerCdSuoDuanLevel() { return m_Player_GongNeng[PLAYED_SUODUAN]; }
		uint32 GetPlayerCdChongZhiLevel() { return m_Player_GongNeng[PLAYED_CHONGZHI]; }
		uint32 GetPlayerMoneyTime() { return m_Player_GongNeng[PLAYED_MONEY_TIME]; }
        uint32 GetPlayerHuoLiTime() { return m_Player_GongNeng[PLAYED_HUOLI_TIME]; }
		uint32 GetPlayerAddHuoLiTime() { return m_Player_GongNeng[PLAYED_ADDHUOLI_TIME]; }
		uint32 GetPlayerTianFuTime() { return m_Player_GongNeng[PLAYED_TIANFU_TIME]; }
		uint32 GetPlayerBianShenTime() { return m_Player_GongNeng[PLAYED_BIANSHEN_TIME]; }
		uint32 GetPlayerCdSuoDuanTime() { return m_Player_GongNeng[PLAYED_SUODUAN_TIME]; }
		uint32 GetPlayerCdChongZhiTime() { return m_Player_GongNeng[PLAYED_CHONGZHI_TIME]; }
		uint32 GetHuoLiLevel() { return m_Player_GongNeng[PLAYED_HUOLI_LEVEL]; }
		uint32 GetZhanYougFlags() { return m_Player_GongNeng[PLAYED_ZHANYOU_FLAGS]; }
		uint32 GetShuangTfLevel() { return m_Player_GongNeng[PLAYED_SHUANGTIANFU]; }
		...
		uint32 GetPlayerInstancesTime() { return m_Player_Instances[PLAYED_INSTANCES_TIME]; }

		uint32 m_Player_GuiId[MAX_PLAYED_GUIID];
		uint32 GetPlayerGuiIdMianFei() { return m_Player_GuiId[PLAYED_GUIID_MIANFEI]; }
		uint32 GetPlayerGuiIdShouFei() { return m_Player_GuiId[PLAYED_GUIID_SHOUFEI]; }
		uint32 GetPlayerGuiIdMianFeiTime() { return m_Player_GuiId[PLAYED_GUIID_MIANFEITIME]; }
		...
		bool m_ChouhenDebug;
		bool ChouhenDebug() { return m_ChouhenDebug; }
		void setChouhenDebug(bool flag) { m_ChouhenDebug = flag; }
        ...
        **/
		...
		bool GetWorldSafe();
		void PlayerVisibilityDistance(float Distance);
		void SetTaqJifen(uint32 cont);
		void SetTaqQuestCompleteTime();
		void AddPlayerAura(uint32 spellID);
		uint32 GetTaqJifen();
		uint32 MoneyXianShi(uint32 canshu, uint32 money);
		...
		void SetCdChongZhi(uint32 ItemId1, uint32 Itemcont1, uint32 ItemId2, uint32 Itemcont2);
		void SetShuangTf(uint32 ItemId1, uint32 Itemcont1, uint32 ItemId2, uint32 Itemcont2);
		void SetSanTf(uint32 ItemId1, uint32 Itemcont1, uint32 ItemId2, uint32 Itemcont2);
		void SetFeiXingDian(uint32 ItemId1, uint32 Itemcont1, uint32 ItemId2, uint32 Itemcont2);
		void SetItemMail(Player* player, uint32 cont, uint32 ItemId, uint32 Itemcont);
		//void SetDianJuanJiLu(Player* player, uint32 dianjuancont);
		//void SetPlayerCommand(Player* player, uint32 itemid, uint32 cont, std::string gm);
		void DeletePlayerMsRaceSpell(uint32 Class, uint32 Race);
		void RemovePlayerQuest(uint32 Class, uint32 Race);
		void DeletePlayerRaceSkill(Player* player);
		void DeletePlayerRaceSpell();
		void DeletePlayerAllSpell();
		uint32 GetPlayerTalentSpellCost();
		void SetPlayerShuangTianFuSaveA();
		void SetPlayerShuangTianFuSaveB();
		void SetPlayerShuangTianFuSaveC();
		void SetPlayerShuangTianFuA();
		void SetPlayerShuangTianFuB();
		void SetPlayerShuangTianFuC();
		void BattleGroundJinRu(Item* pItem, uint32 id);
		void PlayerMianFeiLiBao();
		bool PlayerLeaderGuid();
		bool HasPropertyidCount(uint32 Property1, uint32 Property2, uint32 count, bool inBankAlso) const;
		...
		void SetPlayerTalentsTime(time_t time){ m_resetTalentsTime = time; }
		...
		void setResurrectRequestData(ObjectGuid guid, uint32 mapId, float X, float Y, float Z, uint32 health, uint32 mana, uint32 instanceId)
        {
		...
		void SetCharmed(bool apply, ObjectGuid casterGUID, uint32 spellID);
		...
		void UpdateSkillsToMaxSkillsForLevel(bool zanzhu = false);             // for .levelup
		...
		uint32 GetPlayerTime(uint32 canshu) const;
		uint32 GetPlayerJiangLi(uint32 canshu) const;
		uint32 GetPlayerGuild(uint32 guildid, uint32 canshu) const;
		uint32 GetPlayerGuildCaiLiao(uint32 id, uint32 canshu) const;
		uint32 GetGuildQuest(uint32 id) const;
		uint32 GetPlayerHjrsQuest(uint32 playerguid, uint32 canshu) const;
		uint32 GetCharacters_Quest(uint32 guid, uint32 canshu) const;
		uint32 GetPlayerRacePlayerBytes();
		...
		void SetPlayerJiangLi(int32 cont, int32 canshu);
		void SetPlayerGuild(uint32 guildid, int32 cont, int32 canshu);
		void SetPlayerHjrsQuest(uint32 playerguid, uint32 cont, uint32 canshu);
		void SetCharacters_Quest(uint32 guid, uint32 cont, uint32 canshu) const;
		...
		void UpdatePlayerSpeakTime();
        bool CanSpeak() const;
		bool CanPlayerSpeak() const;
		...
		bool HaveAtClient(WorldObject const* u) {
			...
			if (m_clientGUIDs.find(u->GetObjectGuid()) != m_clientGUIDs.end()) {
				return true;
			}
		...
		std::string GetPlayerClass(uint32 id);
		std::string GetPlayerTeam(uint32 id);
		std::string GetItemName(uint32 entry);
		uint32 GetDaysTime(uint32 time, uint32 chanshu);
    ...
    protected:
        ...
        */
        ...
        uint32 m_nextSave;
        time_t m_speakTime;
		time_t m_speakPlayerTime;
        uint32 m_speakCount;
		uint32 m_speakPlayerCount;
    ...
    private:
		...
		void BanPlayer(uint32 itemid, uint32 cont);
};
