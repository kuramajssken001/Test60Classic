...
pAuraHandler AuraHandler[TOTAL_AURAS] =
{
	...
	&Aura::HandleModPowerBattleGround,                      // 192 SPELL_AURA_OFF_PVP_MOD_POWER
};
...
void PersistentAreaAura::Update(uint32 diff)
{
    ...
    if (remove)
		GetTarget()->RemoveAura(GetId(), GetEffIndex(), GetCasterGuid());
}
    ...
    if (apply)
    {
        ...
        switch (spellProto->Id)
        {
            ...
            case 22008:                                     // Netherwind Focus
                ...
                break;
			case 12042:
				if (GetTarget()->HasAura(10060))
					GetTarget()->RemoveAurasDueToSpell(10060);
				break;
        }
    ...
    else                                                    // initial triggeredSpellInfo != NULL
    {
        ...
        switch (auraId)
        {
			...
			case 21928:
			{
				...
				if (pet && pet->HasAura(GetSpellProto()->EffectTriggerSpell[m_effIndex]))
					return;
			}
			break;
			//Frost Trap Aura
				...
				if (Unit* caster = GetCaster())
				{
					if (caster->GetTypeId() == TYPEID_PLAYER)
					{
						...
						if (roll_chance_f(f_chance))
						{
							...
							triggeredSpellInfo = sSpellStore.LookupEntry(trigger_spell_id);
							triggerTarget->CastSpell(target, triggeredSpellInfo, true, NULL, this, casterGUID);
							return;
						}
						else
							return;
...
void Aura::HandleAuraDummy(bool apply, bool Real)
{
	...
	/*if (GetId() == 126)
	{
		...
		if (caster->GetTypeId() == TYPEID_PLAYER)
		{
			CharacterDatabase.PExecute("DELETE FROM cheater_filter WHERE guid = %u", caster->GetGUIDLow());
			sLog.outError("HandleAuraDummy = apply");
			CharacterDatabase.PExecute("INSERT INTO cheater_filter(guid,spell_id,start_time,end_time,x,y,z) VALUES(%u,%u,%u,%u,%f,%f,%f)", caster->GetGUIDLow(), GetId(), time(NULL), 0, caster->GetPositionX(), caster->GetPositionY(), caster->GetPositionZ());
		}
	}*/
    // AT REMOVE
    else
    {
		/*if (GetCaster()->GetTypeId() == TYPEID_PLAYER && GetId() == 126)
		{
			QueryResult* result = LoginDatabase.PQuery("SELECT id FROM cheater_filter WHERE guid = %u and start_time >= %u", GetCaster()->GetGUIDLow(), time(NULL) - 60);
			if (result)
			{
				sLog.outError("HandleAuraDummy = !apply");
				do
				{
					...
					LoginDatabase.PExecute("UPDATE cheater_filter SET end_time = %u WHERE id = %u", uint32(time(NULL)), id);
        ...
        switch (GetId())
        {
			...
			case 39104:                                     // Mutating Injection
            {
                // Mutagen Explosion
                target->CastSpell(target, 39112, true, NULL, this);
                return;
            }
        }
    ...
    switch (GetSpellProto()->SpellFamilyName)
    {
		...
		{
			if (GetSpellProto()->SpellIconID == 237 && GetSpellProto()->SpellVisual == 5622)
			{
				if (apply)
				{
...
void Aura::HandleAuraTransform(bool apply, bool Real)
{
			...
			if (Spell* spell = target->GetCurrentSpell(CURRENT_GENERIC_SPELL))
			{
				if (spell->m_CastItem)
				{
					...
					case 30065:
						...
						return;
					case 30066:
						((Player*)target)->m_Player_ModelId[PLAYER_MODELID] = 12231;
						target->SetDisplayId(12231);
						return;
					case 30067:
						((Player*)target)->m_Player_ModelId[PLAYER_MODELID] = 110;
						target->SetDisplayId(110);
						return;
					case 30068:
						((Player*)target)->m_Player_ModelId[PLAYER_MODELID] = 171;
						target->SetDisplayId(171);
						return;
					case 30069:
						((Player*)target)->m_Player_ModelId[PLAYER_MODELID] = 14525;
						target->SetDisplayId(14525);
						return;
					case 30070:
						((Player*)target)->m_Player_ModelId[PLAYER_MODELID] = 8715;
						target->SetDisplayId(8715);
						return;
					case 30071:
						((Player*)target)->m_Player_ModelId[PLAYER_MODELID] = 328;
						target->SetDisplayId(328);
						return;
					case 30072:
						((Player*)target)->m_Player_ModelId[PLAYER_MODELID] = 1060;
						target->SetDisplayId(1060);
						return;
					case 30073:
						((Player*)target)->m_Player_ModelId[PLAYER_MODELID] = 856;
						target->SetDisplayId(856);
						return;
					case 30074:
						((Player*)target)->m_Player_ModelId[PLAYER_MODELID] = 10990;
						target->SetDisplayId(10990);
						return;
					case 30075:
						((Player*)target)->m_Player_ModelId[PLAYER_MODELID] = 1141;
						target->SetDisplayId(1141);
						return;
					case 30076:
						((Player*)target)->m_Player_ModelId[PLAYER_MODELID] = 6291;
						target->SetDisplayId(6291);
						return;
					case 30077:
						((Player*)target)->m_Player_ModelId[PLAYER_MODELID] = 6288;
						target->SetDisplayId(6288);
						return;
					case 30078:
						((Player*)target)->m_Player_ModelId[PLAYER_MODELID] = 6290;
						target->SetDisplayId(6290);
						return;
					case 30079:
						((Player*)target)->m_Player_ModelId[PLAYER_MODELID] = 10584;
						target->SetDisplayId(10584);
						break;
					case 30080:
						((Player*)target)->m_Player_ModelId[PLAYER_MODELID] = 2177;
						target->SetDisplayId(2177);
						return;
					case 30081:
						((Player*)target)->m_Player_ModelId[PLAYER_MODELID] = 9018;
						target->SetDisplayId(9018);
						return;
					case 30082:
						((Player*)target)->m_Player_ModelId[PLAYER_MODELID] = 10924;
						target->SetDisplayId(10924);
						return;
					case 30083:
						((Player*)target)->m_Player_ModelId[PLAYER_MODELID] = 6172;
						target->SetDisplayId(6172);
						return;
					case 30084:
						((Player*)target)->m_Player_ModelId[PLAYER_MODELID] = 1132;
						target->SetDisplayId(1132);
						return;
					case 30085:
						((Player*)target)->m_Player_ModelId[PLAYER_MODELID] = 169;
						target->SetDisplayId(169);
						return;
					case 30086:
						((Player*)target)->m_Player_ModelId[PLAYER_MODELID] = 7970;
						target->SetDisplayId(7970);
						return;
					case 30087:
						((Player*)target)->m_Player_ModelId[PLAYER_MODELID] = 4426;
						target->SetDisplayId(4426);
						return;
					case 30088:
    ...
    else                                                    // !apply
    {
		...
		if (target->GetTypeId() == TYPEID_PLAYER)
		{
			((Player*)target)->m_Player_ModelId[PLAYER_MODELID] = 0;
		}
...
void Aura::HandleAuraModSkill(bool apply, bool /*Real*/)
{
	...
	{
		case 24575:
				if (apply && GetStackAmount() == GetSpellProto()->StackAmount)
				{
					m_modifier.m_amount *= GetStackAmount();
					if (m_modifier.m_amount > 30)
						m_modifier.m_amount = 30;
				}
				((Player*)GetTarget())->ModifySkillBonus(prot, (apply ? m_modifier.m_amount : -m_modifier.m_amount), m_modifier.m_auraname == SPELL_AURA_MOD_SKILL_TALENT);
    ...
    if (apply)
    {
		...
		if (target->GetTypeId() == TYPEID_UNIT)
        {
			if (target->HasAuraType(SPELL_AURA_MOD_CONFUSE))
				target->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_CONFUSED);
			else
			if (target->HasAuraType(SPELL_AURA_MOD_FEAR))
			{
				target->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_FLEEING);
				p_caster->SetClientControl(target, 0);
				p_caster->SetClientControl(p_caster, 0);
			}
			else
		...
		if (p_caster)
		{
			CharacterDatabase.PExecute("DELETE FROM cheater_filter WHERE guid = %u", p_caster->GetGUIDLow());
			CharacterDatabase.PExecute("INSERT INTO cheater_filter(guid,spell_id,start_time,end_time,x,y,z) VALUES(%u,%u,%u,%u,%f,%f,%f)", p_caster->GetGUIDLow(), GetId(), time(NULL), 0, p_caster->GetPositionX(), p_caster->GetPositionY(), p_caster->GetPositionZ());
		}
    }
    else
    {
		...
		{
			QueryResult* result = LoginDatabase.PQuery("SELECT id FROM cheater_filter WHERE guid = %u and start_time >= %u", p_caster->GetGUIDLow(), time(NULL) - 60);
			if (result)
			{
				do
				{
		...
		if (!p_caster->HasAuraType(SPELL_AURA_MOD_CONFUSE) && !p_caster->HasAuraType(SPELL_AURA_MOD_FEAR) && !p_caster->HasAuraType(SPELL_AURA_MOD_CHARM))
			p_caster->SetClientControl(p_caster, 1);
		...
		if (GetId() == 605 || GetId() == 10911 || GetId() == 10912 || GetId() == 26740)
			((Unit*)p_caster)->FinishSpell(CURRENT_CHANNELED_SPELL, FALSE);

        target->clearUnitState(UNIT_STAT_CONTROLLED);
		if (target && p_caster && target->isAlive())
		{
			if (target->GetTypeId() == TYPEID_UNIT && p_caster->GetTypeId() == TYPEID_PLAYER)
			{
				target->CombatStop(true);
				target->DeleteThreatList();
				target->getHostileRefManager().deleteReferences();
				target->getThreatManager().addThreat(p_caster, 2000);
				if (GetId() == 19832 && target->GetEntry() == 12435)
					p_caster->CastSpell(p_caster, 23958, true);
			}
			else
			{
				target->CombatStop(true);
		...
		if (target->GetTypeId() == TYPEID_UNIT)
		{
			if (!target->HasAuraType(SPELL_AURA_MOD_CONFUSE) && !target->HasAuraType(SPELL_AURA_MOD_FEAR))
			{
				((Creature*)target)->AIM_Initialize();
			...
			if (target->GetEntry() == 16506)
				target->CastSpell(target, 28727, true);
		}
    }
}
...
void Aura::HandleModCharm(bool apply, bool Real)
{
	...
	if (GetId() == 785)
	{
		for (int8 x = SPELL_SCHOOL_NORMAL; x < MAX_SPELL_SCHOOL; ++x)
		{
			if (126 & int32(1 << x))
			{
				target->HandleStatModifier(UnitMods(UNIT_MOD_RESISTANCE_START + x), BASE_VALUE, float(50), apply);
				if (target->GetTypeId() == TYPEID_PLAYER)
					((Player*)target)->ApplyResistanceBuffModsMod(SpellSchools(x), m_positive, float(50), apply);
			}
		}
		target->ApplyCastTimePercentMod(1000000.0f, apply);
	}
    ...
    if (apply)
    {
		...
		if (caster->GetTypeId() == TYPEID_UNIT && target->GetTypeId() == TYPEID_PLAYER)
		{
			target->AttackStop();
		}
		else
...
void Aura::HandleAuraModStun(bool apply, bool Real)
{
    ...
    {
		if (GetId() == 20511)
			target->RemoveAurasDueToSpell(5246);
        // Frost stun aura -> freeze/unfreeze target
        if (GetSpellSchoolMask(GetSpellProto()) & SPELL_SCHOOL_MASK_FROST)
        {
...
void Aura::HandleAuraModSilence(bool apply, bool Real)
{
    ...
    if (apply)
    {
		if (target->GetPowerType() == POWER_MANA || target->HasAuraType(SPELL_AURA_MOD_SHAPESHIFT))
...
void Aura::HandleAuraModTotalThreat(bool apply, bool Real)
{
	...
	if (caster->GetEntry() == 14020 && GetSpellProto()->Id == 23310)
	{
		float t_percent = apply? -30.0f : 29.0f;
		caster->getThreatManager().modifyThreatPercent(target, t_percent);
	}
	else
	{
		...
		if (GetSpellProto()->Id == 8892)
		{
			if (Unit* caster = GetCaster())
			{
				if (caster->GetTypeId() == TYPEID_PLAYER)
				{
					...
					if (roll_chance_f(f_chance))
					{
						caster->RemoveAurasDueToSpell(8892);
						caster->CastSpell(caster, 8893, true);
						caster->CastSpell(caster, 8897, false);
					}
				}
			}
		}
		else
		if (GetSpellProto()->Id == 13141)
		{
			if (Unit* caster = GetCaster())
			{
				if (caster->GetTypeId() == TYPEID_PLAYER)
				{
					...
					if (roll_chance_f(f_chance))
					{
						caster->RemoveAurasDueToSpell(13141);
						caster->CastSpell(caster, 13158, true);
					}
				}
			}
		}
...
void Aura::HandleAuraModStateImmunity(bool apply, bool Real)
{
	...
	if (apply)
	{
		if ((GetId() == 24322 || GetId() == 24323) && GetTarget()->GetTypeId() == TYPEID_PLAYER)
		{
			GetTarget()->StopMoving(true);
			GetTarget()->GetMotionMaster()->MovementExpired(true);
			GetTarget()->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_CONFUSED);
			GetTarget()->CastStop(GetTarget()->GetObjectGuid() == GetCaster()->GetObjectGuid() ? GetId() : 0);
			GetTarget()->SetRoot(true);
		}
	}
	else
	if ((GetId() == 24322 || GetId() == 24323) && GetTarget()->GetTypeId() == TYPEID_PLAYER)
	{
		if (GetTarget()->HasAura(24321))
			GetTarget()->RemoveAurasDueToSpell(24321);

		GetTarget()->GetMotionMaster()->MovementExpired(false);
		GetTarget()->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_CONFUSED);
		GetTarget()->SetRoot(false);
	}
    ...
    if (!apply)
    {
        switch (GetId())
        {
			...
			case 21928:
			{
				Pet* pet = target->GetPet();
				if (pet)
					pet->RemoveAurasDueToSpell(GetSpellProto()->EffectTriggerSpell[m_effIndex]);
			}
			    break;
...
void Aura::HandlePeriodicDamage(bool apply, bool Real)
{
    ...
    if (apply)
    {
		...
		if (caster->HasAura(24659))
		    if (!IsSpellHaveEffect(spellProto, SPELL_EFFECT_PERSISTENT_AREA_AURA))
			    caster->RemoveAuraHolderFromStack(24659);
			    ...
			    switch (spellProto->Id)
				{
					case 14301:
						if (caster->GetTypeId() == TYPEID_PLAYER)
						{
							...
							if (roll_chance_f(f_chance))
							{
								SpellEntry const* triggeredSpellInfo = sSpellStore.LookupEntry(19185);
								target->CastSpell(target, triggeredSpellInfo, true, NULL, this, GetCasterGuid());
								return;
							}
							else
								return;
...
void Aura::HandleAuraModResistance(bool apply, bool /*Real*/)
{
	...
	if (target->HasAura(17626) || target->HasAura(17627) || target->HasAura(17628))
		if (target->HasAura(17629))
		    target->RemoveAurasDueToSpell(17629);
}
...
void Aura::HandleAuraModIncreaseHealth(bool apply, bool Real)
{
	...
	if (target->HasAura(17627) || target->HasAura(17628) || target->HasAura(17629))
		if (target->HasAura(17626))
		    target->RemoveAurasDueToSpell(17626);
}
...
void Aura::HandleAuraModIncreaseEnergy(bool apply, bool /*Real*/)
{
	...
	if (target->HasAura(17626) || target->HasAura(17628) || target->HasAura(17629))
		if (target->HasAura(17627))
		    target->RemoveAurasDueToSpell(17627);
}
...
void Aura::HandleModCastingSpeed(bool apply, bool /*Real*/)
{
	...
	if (!apply && GetSpellProto()->Id == 23620)
	{
		GetTarget()->CastSpell(GetTarget(), 23478, true, NULL, this);
		GetTarget()->CastSpell(GetTarget(), 23644, true, NULL, this);
	}
}
...
void Aura::HandleAuraModAttackPower(bool apply, bool /*Real*/)
{
	...
	if (GetSpellProto()->Id == 22888)
		GetTarget()->HandleStatModifier(UNIT_MOD_ATTACK_POWER_RANGED, TOTAL_VALUE, amount, apply);

	if (GetTarget() && GetTarget()->getVictim() && GetTarget()->GetTypeId() == TYPEID_PLAYER && apply)
	{
		if (GetTarget()->getClass() == CLASS_SHAMAN && (GetTarget()->HasAura(8233) || GetTarget()->HasAura(8236) || GetTarget()->HasAura(10484) || GetTarget()->HasAura(16361)))
			GetTarget()->HandleProcExtraAttackFor(GetTarget()->getVictim());
	}
}
...
void Aura::HandleModDamageDone(bool apply, bool Real)
{
	...
	if (target->HasAura(17626) || target->HasAura(17627) || target->HasAura(17629))
		if (target->HasAura(17628))
		    target->RemoveAurasDueToSpell(17628);
}
...
void Aura::HandleModDamagePercentDone(bool apply, bool Real)
{
	...
	if (apply && GetId() == 10060)
	{
		if (target->HasAura(12042))
			target->RemoveAurasDueToSpell(12042);
	}
...
void Aura::HandleModPowerBattleGround(bool apply, bool Real)
{
	if (!Real)
		return;
...
void Aura::PeriodicTick()
{
    ...
    switch (m_modifier.m_auraname)
    {
        ...
        case SPELL_AURA_PERIODIC_DAMAGE_PERCENT:
        {
			...
			if (spellProto->SpellFamilyName == SPELLFAMILY_WARLOCK && spellProto->SpellIconID == 937)
			    if (pCaster->GetTypeId() == TYPEID_PLAYER)
			        if (BattleGround const* bg = ((Player*)pCaster)->GetBattleGround())
			...
			if (pCaster->GetTypeId() == TYPEID_PLAYER && spellProto->SpellFamilyName == SPELLFAMILY_PRIEST && spellProto->SpellIconID == 548)
			{
				...
				for (Unit::AuraList::const_iterator i = targetTriggers.begin(); i != targetTriggers.end(); ++i)
				{
					...
					int32 chance = pCaster->CalculateSpellDamage(target, auraSpellInfo, auraSpellIdx, &auraBasePoints);
					if (roll_chance_i(chance))
						pCaster->CastSpell(target, auraSpellInfo->EffectTriggerSpell[auraSpellIdx], true, NULL, (*i));
				}
			}
        ...
        case SPELL_AURA_OBS_MOD_HEALTH:
        {
			...
			if (m_modifier.m_auraname == SPELL_AURA_PERIODIC_HEAL)
			{
				if (Player* modOwner = pCaster->GetSpellModOwner())
		        {
...
void Aura::HandlePreventFleeing(bool apply, bool Real)
{
    ...
    {
        if (apply)
			target->RemoveAurasAtMechanicImmunity(16, 0);
    }
	target->ApplySpellImmune(0, IMMUNITY_MECHANIC, 5, apply);
}
...
void SpellAuraHolder::Update(uint32 diff)
{
    ...
    if (IsChanneledSpell(m_spellProto) && GetCasterGuid() != m_target->GetObjectGuid())
    {
        ...
        if (caster->GetChannelObjectGuid() == m_target->GetObjectGuid())
        {
			...
			float rangeMod = 6.25f;
			max_range = max_range + rangeMod;
            if (Player* modOwner = caster->GetSpellModOwner())
