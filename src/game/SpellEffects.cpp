...
void Spell::EffectResurrectNew(SpellEffectIndex eff_idx)
{
	...
	pTarget->setResurrectRequestData(m_caster->GetObjectGuid(), m_caster->GetMapId(), m_caster->GetPositionX(), m_caster->GetPositionY(), m_caster->GetPositionZ(), health, mana, m_caster->GetInstanceId());
        ...
        switch (m_spellInfo->SpellFamilyName)
        {
            case SPELLFAMILY_GENERIC:
            {
                switch (m_spellInfo->Id)                    // better way to check unknown
                {
					...
					case 21992:
						if (unitTarget->HasAuraType(SPELL_AURA_MOD_CONFUSE) || unitTarget->HasAuraType(SPELL_AURA_MOD_STUN))
							damage = 0;
						break;
					case 26103:
						if (m_caster->GetTypeId() == TYPEID_UNIT && unitTarget->GetTypeId() == TYPEID_PLAYER)
						{
    ...
    switch (m_spellInfo->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            switch (m_spellInfo->Id)
            {
                ...
                case 14185:                                 // Preparation Rogue
                {
                    ...
                    for (SpellCooldowns::const_iterator itr = cm.begin(); itr != cm.end();)
                    {
                        ...
                        if (spellInfo->Category == 39 || spellInfo->Category == 44 || spellInfo->Category == 66 || spellInfo->Id == 2094 || spellInfo->Id == 14177 &&
				...
				case 13006:
				{
					...
					if (unitTarget->GetTypeId() == TYPEID_UNIT)
					    if (((Creature*)unitTarget)->GetCreatureInfo()->Rank == 3)
							return;
				...
				case 29201:
				{
					...
					switch (unitTarget->getClass())
					{
						...
						case CLASS_SHAMAN:
						case CLASS_DRUID: 
							unitTarget->CastSpell(unitTarget, 29198, true, 0, 0, m_caster->GetObjectGuid());
							break;
							default: break;
					}break;
					return;
				...
				case 28797:
				{
					...
					m_caster->CastSpell(m_caster, 28732, true);
            ...
            switch (m_spellInfo->SpellIconID)
            {
                ...
                case 1661:
                {
					...
					int32 hasteModBasePoints1 = (melee_mod - 5 < 0 ? 5 : melee_mod - 5);
            ...
            switch (m_spellInfo->Id)
            {
                ...
                case 12472:                                 // Cold Snap
                {
                    ...
                    for (SpellCooldowns::const_iterator itr = cm.begin(); itr != cm.end();)
                    {
						...
						if (spellInfo->Category == 37 || spellInfo->School == 4 && spellInfo->Id != m_spellInfo->Id)
                            ((Player*)m_caster)->RemoveSpellCooldown((itr++)->first, true);
                        else
                            ++itr;
                    }
        ...
        case SPELLFAMILY_WARLOCK:
        {
            ...
            if (m_spellInfo->SpellFamilyFlags & UI64LIT(0x0000000000040000))
            {
				...
				int32 health = m_spellInfo->EffectBasePoints[0] + 1;
				if (int32(m_caster->GetHealth()) > health)
                {
                    // Shouldn't Appear in Combat Log
					m_caster->ModifyHealth(-health);
            ...
            switch (m_spellInfo->Id)
            {
                case 23989:                                 // Readiness talent
                {
                    ...
                    for (SpellCooldowns::const_iterator itr = cm.begin(); itr != cm.end();)
                    {
                        ...
                        if (spellInfo->SpellFamilyName == SPELLFAMILY_HUNTER && spellInfo->Id == 5384 
							|| spellInfo->Id == 19263 
							|| spellInfo->Id == 13809 
							|| spellInfo->Id == 1499 
							|| spellInfo->Id == 14310 
							...
							|| spellInfo->Id == 14303 
							|| spellInfo->Id == 14304 
							|| spellInfo->Id == 14305 && GetSpellRecoveryTime(spellInfo) > 0)
                            ((Player*)m_caster)->RemoveSpellCooldown((itr++)->first, true);
                        else
                            ++itr;
                    }
				...
				case 24531:                                 // Readiness talent
                {
                    ...
                    for (SpellCooldowns::const_iterator itr = cm.begin(); itr != cm.end();)
                    {
                        ...
                        if (spellInfo->SpellFamilyName == SPELLFAMILY_HUNTER && spellInfo->Id == 19434 
                            || spellInfo->Id == 20900 
                            || spellInfo->Id == 20901 
                            || spellInfo->Id == 20902 
                            || spellInfo->Id == 20903 
                            ...
                            || spellInfo->Id == 14285 
                            || spellInfo->Id == 14286 
                            || spellInfo->Id == 14287 && GetSpellRecoveryTime(spellInfo) > 0)
                            ((Player*)m_caster)->RemoveSpellCooldown((itr++)->first, true);
                        else
                            ++itr;
                    }
...
void Spell::EffectHeal(SpellEffectIndex /*eff_idx*/)
{
    if (unitTarget && unitTarget->isAlive() && damage >= 0)
    {
		...
		if (caster->HasAura(23401) && caster->getClass() == CLASS_PRIEST)
			((Player*)unitTarget)->AddPlayerAura(23402);
    }
}
...
void Spell::DoCreateItem(SpellEffectIndex eff_idx, uint32 itemtype)
{
    ...
    if (no_space > 0 && bgType)
    {
		...
		{
			if (sWorld.getConfig(CONFIG_BOOL_BATTLEGROUND_MAIL))
				bg->SendRewardMarkByMail(player, newitemid, no_space);
		}
    }

	if (newitemid == 21175 && sWorld.getConfig(CONFIG_BOOL_TAQ_KAIENRENWU))
		sWorld.SendWorldText(LANG_TAQ_11, player->GetPlayerTeam(player->GetTeam()), player->GetName(), player->GetPlayerTeam(player->GetTeam()), player->GetTaqQuestCompleteTime());
}
...
void Spell::EffectPersistentAA(SpellEffectIndex eff_idx)
{
	...
	if (m_spellInfo)
	{
		switch (m_spellInfo->Id)
		{
		    ...
		    case 10186:
		    case 10187:
			    if (pCaster->HasAura(12536))
				    pCaster->RemoveAurasDueToSpell(12536);
				else
				{
					if (pCaster->GetTypeId() == TYPEID_PLAYER)
					{
						float f_chance = 0;
						Unit::AuraList const& auraTriggerSpell = pCaster->GetAurasByType(SPELL_AURA_PROC_TRIGGER_SPELL);
						...
						{
							pCaster->CastSpell(pCaster, 12536, true);
							break;
						}
						else
							break;
    ...
    if (gameObjTarget)
    {
        switch (gameObjTarget->GetGoType())
        {
            ...
            case GAMEOBJECT_TYPE_TRAP:
                ...
                {
					if (gameObjTarget->GetEntry() == 179784)
					{
...
void Spell::EffectOpenLock(SpellEffectIndex eff_idx)
{
    ...
    if (gameObjTarget)
    {
		...
		if (player->GetTypeId() == TYPEID_PLAYER && player->getLevel() < 55 && goInfo->type == GAMEOBJECT_TYPE_CHEST && !player->IsPvP())
		{
			switch (m_spellInfo->Id)
			{
				case 2366:
...
void Spell::EffectLearnSpell(SpellEffectIndex eff_idx)
{
	...
	switch (spellToLearn)
	{
		case 197:
		{
			uint32 level = player->getLevel() * 5;
			player->SetSkill(172, level, 300);
		}break;
		case 199:
		{
			uint32 level = player->getLevel() * 5;
			player->SetSkill(160, level, 300);
		}break;
	}
...
void Spell::EffectDispel(SpellEffectIndex eff_idx)
{
	...
	if (m_spellInfo->Effect[eff_idx] == SPELL_EFFECT_DISPEL)
	{
	    if (Player* modOwner = m_caster->GetSpellModOwner())
		{
		...
		if (unitTarget->HasAura(26180))
		{
			int32 damage = 4000;
			if (damage > 0)
				unitTarget->CastCustomSpell(unitTarget, 26233, &damage, NULL, NULL, true);
		}
	}
...
void Spell::EffectSummonWild(SpellEffectIndex eff_idx)
{
	...
	if (m_CastItem)
	{
		...
		{
			case 22048:
				if (m_targets.getUnitTarget()->GetTypeId() == TYPEID_UNIT && m_targets.getUnitTarget()->isDead() && m_targets.getUnitTarget()->GetEntry() != 16042)
					return;
				break;
			default:
				break;
    ...
    for (int32 count = 0; count < amount; ++count)
    {
        ...
        if (Creature* summon = m_caster->SummonCreature(creature_entry, px, py, pz, m_caster->GetOrientation(), summonType, duration))
        {
			...
			{
			    case 8937:
				    ((Unit*)summon)->setFaction(((Player*)m_caster)->GetTeam() == ALLIANCE ? 11 : 85);
				    ((Unit*)summon)->SetLevel(m_caster->getLevel());
				    ((Unit*)summon)->GetMotionMaster()->MoveFollow(m_caster, PET_FOLLOW_DIST, PET_FOLLOW_ANGLE);
				    break;
			    default:
				    break;
			}
    ...
    for (int32 count = 0; count < amount; ++count)
    {
		...
		switch (pet_entry)
		{
			...
			case 10718:
			{
				...
				spawnCreature->CastSpell(spawnCreature, spellid, true);
			}
			break;
		}
...
void Spell::EffectWeaponDmg(SpellEffectIndex eff_idx)
{
	...
	for (uint32 i = CURRENT_FIRST_NON_MELEE_SPELL; i < CURRENT_MAX_SPELL; i++)
	{
		if (Spell* spell = m_caster->GetCurrentSpell(CurrentSpellTypes(i)))
		{
			...
			if (curSpellInfo && curSpellInfo->Id == 20662)
				spellid = curSpellInfo->Id;
		}
	}

	if (spellid == 20662 && unitTarget->GetHealthPercent() <= 20.0f)
	{
		uint32 shanghai = ((Player*)m_caster)->GetPower(POWER_RAGE) / 10 * 15 + 600;
		bonus = shanghai;
	}
	else
	{
...
void Spell::EffectSummonObjectWild(SpellEffectIndex eff_idx)
{
	...
	if (m_caster && m_caster->GetTypeId() == TYPEID_UNIT && m_caster->GetEntry() == 60011)
		gameobject_id = 520024;

	if (m_spellInfo->Id == 27627)
	{
		switch (urand(1, 4))
		{
		    case 1:
    ...
    switch (m_spellInfo->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            switch (m_spellInfo->Id)
            {
				...
				case 5249:
				{
					if (!unitTarget || unitTarget->GetTypeId() == TYPEID_PLAYER || unitTarget->GetEntry() != 14323)
						 return;

					unitTarget->CastSpell(unitTarget, 22856, true);
					((Creature*)unitTarget)->SetFactionTemporary(35, TEMPFACTION_RESTORE_RESPAWN);
					break;
				}
            }
            break;
        ...
        case SPELLFAMILY_PALADIN:
        {
            ...
            if (m_spellInfo->SpellIconID == 70)
            {
				...
				if (unitTarget->HasAuraType(SPELL_AURA_MOD_HEALING_PCT))
					heal *= 2;*/
...
void Spell::EffectSummonPlayer(SpellEffectIndex /*eff_idx*/)
{
	...
	((Player*)unitTarget)->SetSummonPoint(m_caster->GetMapId(), x, y, z, m_caster->GetInstanceId());
...
void Spell::EffectLeapForward(SpellEffectIndex eff_idx)
{
	...
	{
		if (m_caster->isAlive() && m_caster->GetEntry() == 15954)
		{
			if (Unit* pTarget = ((Creature*)m_caster)->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0))
			{
				m_caster->TauntApply(pTarget);
				if (dist < ((Creature*)m_caster)->GetDistance2d(pTarget))
					dist = ((Creature*)m_caster)->GetDistance2d(pTarget);
			}
		}
	}
    ...
    if (!unitTarget->GetMap()->GetHeightInRange(prevPos.x, prevPos.y, groundZ, 3.0f) && unitTarget->m_movementInfo.HasMovementFlag(MOVEFLAG_FALLING))
    {
        ...
        nextPos.z = prevPos.z - 5.0f; // little hack to avoid the impression to go up when teleporting instead of continue to fall. This value may need some tweak

        //
        GridMapLiquidData liquidData;
        if (unitTarget->GetMap()->GetTerrain()->IsInWater(nextPos.x, nextPos.y, nextPos.z, &liquidData))
        {
            if (fabs(nextPos.z - liquidData.level) < 25.0f)
                nextPos.z = liquidData.level - IN_OR_UNDER_LIQUID_RANGE;
        }
        else
        {
            // fix z to ground if near of it
            unitTarget->GetMap()->GetHeightInRange(nextPos.x, nextPos.y, nextPos.z, 25.0f);
        }
...
void Spell::EffectReputation(SpellEffectIndex eff_idx)
{
	...
	if (m_CastItem)
	{
		...
		case 90002:
			...
			{
				_player->m_Player_GongNeng[PLAYED_MONEY] = 1;
				_player->m_Player_GongNeng[PLAYED_MONEY_TIME] = sWorld.GetGameTime() + 2592000;
				_player->SaveToDB();
				return;
			}
			break;
		case 30048:
			...
			{
				_player->m_Player_GongNeng[PLAYED_SHUANGTIANFU_TIME] += 2592000;
				_player->SaveToDB();
				return;
			}
			else
			{
				_player->m_Player_GongNeng[PLAYED_SHUANGTIANFU] = 1;
				_player->m_Player_GongNeng[PLAYED_SHUANGTIANFU_CONT] = 0;
				_player->m_Player_GongNeng[PLAYED_SHUANGTIANFU_TIME] = sWorld.GetGameTime() + 2592000;
				_player->SaveToDB();
				return;
			}
			break;
		case 30049:
			if (_player->GetPlayerCdChongZhiLevel() >= 1)
			{
				_player->m_Player_GongNeng[PLAYED_CHONGZHI_TIME] += 7200;
				_player->SaveToDB();
				return;
			}
			else
			{
				_player->m_Player_GongNeng[PLAYED_CHONGZHI] = 1;
				_player->m_Player_GongNeng[PLAYED_CHONGZHI_TIME] = sWorld.GetGameTime() + 7200;
				_player->SaveToDB();
				return;
			}
			break;
		case 30050:
			if (_player->GetPlayerCdSuoDuanLevel() >= 1)
			{
				_player->m_Player_GongNeng[PLAYED_SUODUAN_TIME] += 2592000;
				_player->SaveToDB();
				return;
			}
			else
			{
				_player->m_Player_GongNeng[PLAYED_SUODUAN] = 1;
				_player->m_Player_GongNeng[PLAYED_SUODUAN_TIME] = sWorld.GetGameTime() + 2592000;
				_player->SaveToDB();
				return;
			}
			break;
		/*case 30149:
			...
			return;
		case 30150:
			_player->AddPlayerAura(22817);
			return;
		case 30151:
			_player->AddPlayerAura(22818);
			return;
		case 30152:
			_player->AddPlayerAura(22820);
			return;
		case 30153:
			_player->AddPlayerAura(24425);
			return;
		case 30154:
			_player->AddPlayerAura(16609);
			return;
		case 30155:
			_player->AddPlayerAura(22888);
			return;*/
		case 30149:
		case 30150:
		case 30151:
...
void Spell::EffectCharge(SpellEffectIndex /*eff_idx*/)
{
	...
	float distance;
	float x, y, z;
	float t_speed = ((Player*)unitTarget)->GetSpeed(MOVE_RUN);
	float PositionZ = unitTarget->GetPositionZ() - m_caster->GetPositionZ();
	float c_time = 0.2f;
	if (unitTarget->GetTypeId() == TYPEID_PLAYER)
	{
		if (((Player*)unitTarget)->isMoving())
		{
			unitTarget->SetRoot(true);
			if (t_speed <= 7.0f)
				distance = 1.0f;
			else
			{
				if (PositionZ >= 0.3f)
				{
					if (m_caster->GetObjectBoundingRadius() > 1.0f)
					{
						c_time = 0.5f;
						distance = 2.0f;
					}
	...
	//float distance2d = distance + m_caster->GetObjectBoundingRadius() + unitTarget->GetObjectBoundingRadius() < 0 ? 0 : distance + m_caster->GetObjectBoundingRadius() + unitTarget->GetObjectBoundingRadius();
	//unitTarget->GetNearPoint(m_caster, x, y, z, m_caster->GetObjectBoundingRadius(), distance2d, unitTarget->GetObjectBoundingRadius());
	float distance2d = distance + m_caster->GetObjectBoundingRadius() + unitTarget->GetObjectBoundingRadius() < 0.0f ? 0.0f : distance;
	unitTarget->GetContactPoint(m_caster, x, y, z, distance2d);
	...
	else
	{
		if (((Player*)unitTarget)->isMoving())
		{
			float e_distance = c_time * t_speed;
			float t_angle = unitTarget->GetOrientation();
			x = x + e_distance * cos(t_angle);
			y = y + e_distance * sin(t_angle);
			m_caster->UpdateAllowedPositionZ(x, y, z);
	...
	m_caster->MonsterMoveWithSpeed(x, y, z, c_speed, true, true);
	unitTarget->SetRoot(false);
...
void Spell::EffectSpiritHeal(SpellEffectIndex /*eff_idx*/)
{
	...
	MapEntry const* mapEntry = sMapStore.LookupEntry(unitTarget->GetMapId());
	if (mapEntry)
