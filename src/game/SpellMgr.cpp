...
{
	//return spellInfo->HasAttribute(SPELL_ATTR_PASSIVE); ÐÞ¸´Í·ÍÆ¸½Ä§
	switch (spellInfo->Id)
	{
		case 24151:
		case 24153:
...
bool IsNoShiWuAura(uint32 spellId_1, uint32 spellId_2)
{
	SpellEntry const* spellInfo_1 = sSpellStore.LookupEntry(spellId_1);
	SpellEntry const* spellInfo_2 = sSpellStore.LookupEntry(spellId_2);
	...
	if (spellInfo_1->SpellFamilyName == SPELLFAMILY_POTION && spellInfo_2->SpellFamilyName == SPELLFAMILY_POTION && spellInfo_1->SpellIconID != spellInfo_2->SpellIconID && spellInfo_1->SpellVisual != spellInfo_2->SpellVisual)
		return true;
