...
void Unit::DealDamageMods(Unit* pVictim, uint32& damage, uint32* absorb, bool critical)
{
	if (absorb)
	{
		if (absorb > 0)
			pVictim->RemoveAurasWithInterruptFlags(AURA_INTERRUPT_FLAG_UNK12);

		if (!critical)
			pVictim->RemoveAurasAtMechanicImmunity(8192, 0);

		if (Spell* curSpell = pVictim->GetCurrentSpell(CURRENT_CHANNELED_SPELL))
		    if (curSpell && curSpell->m_spellInfo->Mechanic == MECHANIC_BANDAGE)
				pVictim->FinishSpell(CURRENT_CHANNELED_SPELL, FALSE);
	}

	if (pVictim && pVictim->HasAura(20578))
		((Player*)pVictim)->TeleportTo(pVictim->GetMapId(), pVictim->GetPositionX() + 0.01f, pVictim->GetPositionY(), pVictim->GetPositionZ(), pVictim->GetOrientation());
...
uint32 Unit::DealDamage(Unit* pVictim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellEntry const* spellProto, bool durabilityLoss)
{
	...
	if (damage && GetTypeId() == TYPEID_PLAYER && pVictim->GetTypeId() == TYPEID_PLAYER)
	{
		if (!pVictim->GetMap()->IsDungeon() && IsFriendlyTo(pVictim) && !IsHostileTo(pVictim) && pVictim != this)
		{
			damage = 0;
			return 0;
		}
    ...
    if (health <= damage)
    {
		...
		if (pUnit && pVictim && pUnit != pVictim && pUnit->GetTypeId() == TYPEID_PLAYER && pVictim->GetTypeId() == TYPEID_UNIT)
		{
			if (((Creature*)pVictim)->IsWorldBoss() && sMapStore.LookupEntry(pUnit->GetMapId())->IsRaid())
			{
					...
					|| pVictim->GetEntry() == 15727 || pVictim->GetEntry() == 15544 || pVictim->GetEntry() == 15543 || pVictim->GetEntry() == 15517 || pVictim->GetEntry() == 15516 || pVictim->GetEntry() == 15511 || pVictim->GetEntry() == 15510 || pVictim->GetEntry() == 15509 || pVictim->GetEntry() == 15299 || pVictim->GetEntry() == 15276 || pVictim->GetEntry() == 15275 || pVictim->GetEntry() == 15263)
				{
					...
					{
						if (!sWorld.ChaXunGuild_IdBossFd(pVictim->GetEntry()))
						{
							if (((Player*)pUnit)->GetTeam() == ALLIANCE)
							{
								sWorld.SetBossFd(pVictim->GetEntry(), ((Player*)pUnit)->GetGuildId(), 1, 1, 0, ((Player*)pUnit)->GetTeam());
							}
							else
							{
								sWorld.SetBossFd(pVictim->GetEntry(), ((Player*)pUnit)->GetGuildId(), 1, 0, 1, ((Player*)pUnit)->GetTeam());
							}
							if (Guild* targetGuild = sGuildMgr.GetGuildById(((Player*)pUnit)->GetGuildId()))
							{
								if (pVictim->GetEntry() == 15956)
								    sWorld.SendWorldText(LANG_GONGGAO_28, targetGuild->GetName().c_str(), pVictim->GetName());
								else
								if (pVictim->GetEntry() == 15953)
									sWorld.SendWorldText(LANG_GONGGAO_30, targetGuild->GetName().c_str(), pVictim->GetName());
								else
								if (pVictim->GetEntry() == 15954)
									sWorld.SendWorldText(LANG_GONGGAO_32, targetGuild->GetName().c_str(), pVictim->GetName());
								else
						...
						if (sWorld.ChaXunGuild_IdBossFd(pVictim->GetEntry()) && !sWorld.ChaXunTeam_IdBossFd(pVictim->GetEntry(), ((Player*)pUnit)->GetTeam()))
						{
							if (((Player*)pUnit)->GetTeam() == ALLIANCE)
							{
								sWorld.SetBossFd(pVictim->GetEntry(), ((Player*)pUnit)->GetGuildId(), 0, 1, 0, ((Player*)pUnit)->GetTeam());
							}
							else
							{
								sWorld.SetBossFd(pVictim->GetEntry(), ((Player*)pUnit)->GetGuildId(), 0, 0, 1, ((Player*)pUnit)->GetTeam());
							}
							if (Guild* targetGuild = sGuildMgr.GetGuildById(((Player*)pUnit)->GetGuildId()))
							{
								if (pVictim->GetEntry() == 15956)
									sWorld.SendWorldText(LANG_GONGGAO_29, targetGuild->GetName().c_str(), ((Player*)pUnit)->GetPlayerTeam(((Player*)pUnit)->GetTeam()), pVictim->GetName());
								else
								if (pVictim->GetEntry() == 15953)
									sWorld.SendWorldText(LANG_GONGGAO_31, targetGuild->GetName().c_str(), ((Player*)pUnit)->GetPlayerTeam(((Player*)pUnit)->GetTeam()), pVictim->GetName());
								else
								if (pVictim->GetEntry() == 15954)
									sWorld.SendWorldText(LANG_GONGGAO_33, targetGuild->GetName().c_str(), ((Player*)pUnit)->GetPlayerTeam(((Player*)pUnit)->GetTeam()), pVictim->GetName());
								else
								if (pVictim->GetEntry() == 16061)
...
SpellMissInfo Unit::SpellHitResult(Unit* pVictim, SpellEntry const* spell, bool CanReflect)
{
	...
	if (pVictim && pVictim->GetTypeId() == TYPEID_PLAYER && IsAreaOfEffectSpell(spell))
		return SPELL_MISS_NONE;
    ...
    if (CanReflect)
    {
        ...
        for (Unit::AuraList::const_iterator i = mReflectSpellsSchool.begin(); i != mReflectSpellsSchool.end(); ++i)
			...
			if (spell->School == 4 && (pVictim->HasAura(6143) || pVictim->HasAura(8461) || pVictim->HasAura(8462) || pVictim->HasAura(10177) || pVictim->HasAura(28609)))
			{
				if (pVictim->HasAura(28332))
					reflectchance = 20;
				if (pVictim->HasAura(11189))
					reflectchance = 10;
			}
			if (spell->School == 2 && (pVictim->HasAura(543) || pVictim->HasAura(8457) || pVictim->HasAura(8458) || pVictim->HasAura(10223) || pVictim->HasAura(10225)))
			{
				if (pVictim->HasSpell(13043))
					reflectchance = 20;
...
bool Unit::isInAccessablePlaceFor(Creature const* c) const
{
	if (IsInWater())                                // Only swimming creatures can reach units in water
		...
		return true;
	else if (c->CanWalk())                          // Walking creatures need to find a path                                      // Without mmaps walking creatures can reach any unit not in water
		return true;

	return false;
}

bool Unit::DoubleCheckAccess(Creature const* c) const
{
	if (IsInWater())                                // Only swimming creatures can reach units in water
		return c->CanSwim();
	else if (c->CanFly())                           // Flying creatures can reach any unit not in water
	...
	{
		Unit* uCreature = ((Unit*)c);
		if ((uCreature->GetMotionMaster()->GetCurrent()->IsReachable()))
		{
			if (!uCreature->CanReachWithMeleeAttack(this))
			{
				if (!m_movementInfo.HasMovementFlag(MOVEFLAG_FALLING))
				{
					if (abs(GetPositionZ() - uCreature->GetPositionZ()) > 3.0f)
					{
						if (c->GetEntry() == 10184)
						{
							uint32 perc = (c->GetHealth() * 100) / c->GetMaxHealth();
							if (perc <= 66 && perc >= 39)
								return true;
						}
						if (MMAP::MMapFactory::IsPathfindingEnabled(GetMapId()))
						{
							...
							return path.calculate(x, y, z) && !(path.getPathType() & (PATHFIND_NOPATH | PATHFIND_INCOMPLETE));
						}
						else
							return true;
					}
    ...
    if ((!holder->IsPassive() && !holder->IsPersistent()) || holder->IsAreaAura())
    {
        ...
        for (SpellAuraHolderMap::iterator iter = spair.first; iter != spair.second; ++iter)
        {
			...
			if (foundHolder->GetCasterGuid() != holder->GetCasterGuid())
			{
				if (aurSpellInfo->StackAmount)
				{
					if (aurSpellInfo->SpellFamilyFlags & UI64LIT(0x0000000000010000) || aurSpellInfo->SpellFamilyFlags & UI64LIT(0x0000000008000000))
					{
					}
					else
					{
            ...
            for (int32 i = 0; i < MAX_EFFECT_INDEX && !stop; ++i)
            {
                ...
                switch (aurNameReal)
                {
                    ...
                    default:                                // not allow
						if (IsChanneledSpell(foundHolder->GetSpellProto()) && IsChanneledSpell(holder->GetSpellProto()))
							break;
...
bool Unit::RemoveNoStackAurasDueToAuraHolder(SpellAuraHolder* holder)
{
    ...
    for (i = m_spellAuraHolders.begin(); i != m_spellAuraHolders.end(); i = next)
    {
		...
		if (IsSpellHaveEffect(i_spellProto, SPELL_EFFECT_PERSISTENT_AREA_AURA) && IsSpellHaveEffect(holder->GetSpellProto(), SPELL_EFFECT_PERSISTENT_AREA_AURA)) continue;

		if (IsChanneledSpell(i_spellProto) && IsChanneledSpell(holder->GetSpellProto())) continue;

		if (IsAuraAddedBySpell(SPELL_AURA_PERIODIC_DAMAGE, i_spellProto->Id) && IsAuraAddedBySpell(SPELL_AURA_PERIODIC_DAMAGE, holder->GetSpellProto()->Id))
		{
			if ((*i).second->GetCasterGuid() != holder->GetCasterGuid())
				continue;
		}

		if (i_spellProto->SpellIconID == spellProto->SpellIconID && i_spellProto->SpellVisual != spellProto->SpellVisual && !i_spellProto->IsFitToFamilyMask(spellProto->SpellFamilyFlags)) continue;

		if (IsNoShiWuAura(spellId, i_spellId)) continue;
        ...
        if (is_spellSpecPerTarget || (is_spellSpecPerTargetPerCaster && holder->GetCasterGuid() == (*i).second->GetCasterGuid()))
        {
			...
			{
				if (spellProto->SpellIconID != i_spellProto->SpellIconID)
					continue;
			}
        ...
        if (!is_spellSpecPerTargetPerCaster && !is_spellSpecPerTarget && sSpellMgr.IsNoStackSpellDueToSpell(spellId, i_spellId))
        {
			...
			{
				AuraType aurNameReal = AuraType(spellProto->EffectApplyAuraName[j]);
				switch (aurNameReal)
				{
...
void Unit::RemoveAura(uint32 spellId, SpellEffectIndex effindex, ObjectGuid casterGuid, Aura* except)
{
    SpellAuraHolderBounds spair = GetSpellAuraHolderBounds(spellId);
    for (SpellAuraHolderMap::iterator iter = spair.first; iter != spair.second;)
    {
        Aura* aur = iter->second->m_auras[effindex];
		if (aur && aur != except && aur->GetCasterGuid() == casterGuid)
        {
...
void Unit::RemoveSpellAuraHolder(SpellAuraHolder* holder, AuraRemoveMode mode)
{
	...
	if (AurSpellInfo && IsChanneledSpell(AurSpellInfo) && caster)
	...
	if (mode != AURA_REMOVE_BY_EXPIRE && AurSpellInfo && IsChanneledSpell(AurSpellInfo) && !IsAreaOfEffectSpell(AurSpellInfo) &&
    ...
    if (benefit)
    {
        switch (spellProto->SpellFamilyName)
		{
			case SPELLFAMILY_PALADIN:
				...
				break;
			default:
				LvlPenalty = CalculateLevelPenalty(spellProto);
				break;
		}
...
uint32 Unit::SpellDamageBonusDone(Unit* pVictim, SpellEntry const* spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack)
{
    if (!spellProto || !pVictim || damagetype == DIRECT_DAMAGE)
        return pdamage;
	if (spellProto->Id == 24323)
		return 300;
	if (spellProto->IsFitToFamily(SPELLFAMILY_ROGUE, UI64LIT(0x0000000000002000)) || spellProto->IsFitToFamily(SPELLFAMILY_ROGUE, UI64LIT(0x0000000000010000)))
		return pdamage;
	if (GetTypeId() == TYPEID_PLAYER && spellProto->SpellFamilyName == SPELLFAMILY_GENERIC)
	{
		for (int j = 0; j < MAX_EFFECT_INDEX; ++j)
		{
	...
	if (GetTypeId() == TYPEID_UNIT && !((Creature*)this)->IsPet())
	{
		...
		case 309:
			...
			break;
		case 509:
			DoneTotalMod *= Creature::_GetSpellDamageMod(((Creature*)this)->GetCreatureInfo()->Rank, 509);
			break;
		case 409:
			DoneTotalMod *= Creature::_GetSpellDamageMod(((Creature*)this)->GetCreatureInfo()->Rank, 409);
			break;
		case 249:
			DoneTotalMod *= Creature::_GetSpellDamageMod(((Creature*)this)->GetCreatureInfo()->Rank, 249);
			break;
		case 469:
			DoneTotalMod *= Creature::_GetSpellDamageMod(((Creature*)this)->GetCreatureInfo()->Rank, 469);
			break;
		case 531:
			if (GetEntry() == 15517 || GetEntry() == 15589)
				DoneTotalMod *= 1.3f;
			else
			    DoneTotalMod *= Creature::_GetSpellDamageMod(((Creature*)this)->GetCreatureInfo()->Rank, 531);
			break;
		case 533:
			DoneTotalMod *= Creature::_GetSpellDamageMod(((Creature*)this)->GetCreatureInfo()->Rank, 533);
			break;
		case 30:
			DoneTotalMod *= Creature::_GetSpellDamageMod(((Creature*)this)->GetCreatureInfo()->Rank, 30);
			break;
		default:
			DoneTotalMod *= Creature::_GetSpellDamageMod(((Creature*)this)->GetCreatureInfo()->Rank, 1000);
			break;
		}
	}
...
void Unit::SetInCombatState(bool PvP, Unit* enemy)
{
    ...
    if (creatureNotInCombat)
    {
		...
		if (OutdoorPvP* outdoorPvP = sOutdoorPvPMgr.GetScript(GetZoneId()))
			outdoorPvP->HandleCreatureEnterCombat(pCreature);
...
bool Unit::isVisibleForOrDetect(Unit const* u, WorldObject const* viewPoint, bool detect, bool inVisibleList, bool is3dDistance) const
{
    ...
    if ((m_Visibility == VISIBILITY_ON && u->m_invisibilityMask == 0) || u->HasAura(24306))
        return true;
    ...
    visibleDistance = 11.0f - (GetTotalAuraModifier(SPELL_AURA_MOD_STEALTH) / 100.0f);
	...
	int32 detectmod = int32(u->GetTotalAuraModifier(SPELL_AURA_MOD_STEALTH_DETECT));
	if (u->HasAura(2836))
	    if (u->GetTypeId() != TYPEID_GAMEOBJECT || (u->GetTypeId() == TYPEID_GAMEOBJECT && ((GameObject*)u)->GetGoType() != GAMEOBJECT_TYPE_TRAP))
		    detectmod -= 70;
	visibleDistance += (int32(detectmod - stealthMod)) / 5.0f;
	if (u->HasInArc(float(M_PI_F / 6), this))
	{
		visibleDistance *= 1.0f;
	}
	else if (u->HasInArc(float(M_PI_F / 3), this))
	{
		visibleDistance *= 0.75f;
	}
	else if (u->HasInArc(float(M_PI_F), this))
	{
		visibleDistance *= 0.5f;
	}
...
bool Unit::IsSecondChoiceTarget(Unit* pTarget, bool checkThreatArea)
{
	...
	if (pTarget->HasAuraType(SPELL_AURA_MOD_CHARM) && pTarget->GetTypeId() == TYPEID_PLAYER && GetTypeId() == TYPEID_UNIT && pTarget->GetCharmerOrOwner())
	{
		return pTarget->GetCharmerOrOwner()->GetObjectGuid() == ((Creature*)this)->GetObjectGuid();
	}
	else
	{
		return
...
bool Unit::SelectHostileTarget()
{
    ...
    if (target)
    {
		if (!hasUnitState(UNIT_STAT_CAN_NOT_REACT_OR_LOST_CONTROL))
        {
			...
			if (!GetMotionMaster()->GetCurrent()->IsReachable() && !target->m_movementInfo.HasMovementFlag(MOVEFLAG_FALLING) && abs(GetPositionZ() - target->GetPositionZ()) > 3.0f)
            {
                // remove all taunts
...
int32 Unit::CalculateSpellDamage(Unit const* target, SpellEntry const* spellProto, SpellEffectIndex effect_index, int32 const* effBasePoints)
{
	...
	if (spellProto->Effect[effect_index] == SPELL_EFFECT_HEAL)
	{
	    if (Player* modOwner = GetSpellModOwner())
		{
	        if (modOwner->HasAura(29198) && !modOwner->HasAura(29199))
				modOwner->CastSpell(modOwner, 29199, true);
			else
			if (modOwner->HasAura(29199))
				value = 0;
		}
	}
...
void Unit::ApplyDiminishingToDuration(DiminishingGroup group, int32& duration, Unit* caster, DiminishingLevels Level, bool isReflected, const SpellEntry* spellInfo)
{
	if (spellInfo)
	{
		if (spellInfo->Id != 19185)
		{
	...
	if (GetTypeId() == TYPEID_UNIT && ((Creature*)this)->IsPet())
	{
		Unit* pVictim = GetCharmerOrOwner();
...
bool Unit::HandleStatModifier(UnitMods unitMod, UnitModifierType modifierType, float amount, bool apply)
{
    ...
    switch (modifierType)
    {
        ...
        case TOTAL_PCT:
			...
			if (((Unit*)this)->GetMapId() == 469 && val < 0.05f)
				val = 0.05f;
...
void Unit::ProcDamageAndSpellFor(bool isVictim, Unit* pTarget, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, SpellEntry const* procSpell, uint32 damage)
{
    ...
    if (procFlag & MELEE_BASED_TRIGGER_MASK)
    {
		...
		if (procExtra & (PROC_EX_CRITICAL_HIT | PROC_EX_PARRY | PROC_EX_DODGE | PROC_EX_BLOCK | PROC_EX_MISS))
        {
            // for victim
            if (isVictim)
            {
            ...
            else // For attacker
            {
				...
				if (procExtra & (PROC_EX_DODGE | PROC_EX_MISS | PROC_EX_PARRY) && GetTypeId() == TYPEID_PLAYER)
				{
					if (procSpell)
					{
						if (procSpell->powerType == POWER_ENERGY)
						{
							uint32 nengliang = procSpell->manaCost * 0.8;
							uint32 playernengliang = 100 - ((Player*)this)->GetPower(POWER_ENERGY);
							if (playernengliang >= nengliang)
								((Player*)this)->SetPower(POWER_ENERGY, ((Player*)this)->GetPower(POWER_ENERGY) + nengliang);
							else
								((Player*)this)->SetPower(POWER_ENERGY, ((Player*)this)->GetPower(POWER_ENERGY) + playernengliang);
						}

						if (procSpell->powerType == POWER_RAGE)
						{
							if (procSpell->SpellFamilyName == SPELLFAMILY_WARRIOR)
							{
								if (!(procSpell->SpellFamilyFlags & UI64LIT(0x0000000000000080)) &&
									!(procSpell->SpellFamilyFlags & UI64LIT(0x0000000000010000)) &&
									!(procSpell->SpellFamilyFlags & UI64LIT(0x0000000000080000)) &&
									!(procSpell->SpellFamilyFlags & UI64LIT(0x0000000000020000)) &&
									!(procSpell->SpellFamilyFlags & UI64LIT(0x0000000080000000)) &&
									!(procSpell->SpellFamilyFlags & UI64LIT(0x0000000000040000)) &&
									!(procSpell->SpellFamilyFlags & UI64LIT(0x0000000000400000)))
								{
									uint32 nengliang = procSpell->Id == 20647 ? 100 : procSpell->manaCost;
									uint32 playernengliang = ((Player*)this)->GetPower(POWER_RAGE) + nengliang;
									((Player*)this)->SetPower(POWER_RAGE, playernengliang);
								}
							}
							if (procSpell->SpellFamilyName == SPELLFAMILY_DRUID)
							{
								if ((procSpell->SpellFamilyFlags & UI64LIT(0x0000000000002000)))
								{
									uint32 nengliang = procSpell->manaCost;
									uint32 playernengliang = ((Player*)this)->GetPower(POWER_RAGE) + nengliang;
									((Player*)this)->SetPower(POWER_RAGE, playernengliang);
								}
								else
								if ((procSpell->SpellFamilyFlags & UI64LIT(0x0000000000000800)) && procSpell->StartRecoveryCategory == 0)
								{
									uint32 nengliang = procSpell->manaCost;
									uint32 playernengliang = ((Player*)this)->GetPower(POWER_RAGE) + nengliang;
									((Player*)this)->SetPower(POWER_RAGE, playernengliang);
    ...
    ProcTriggeredList procTriggered;    // Fill procTriggered list
    ...
    for (ProcTriggeredList::const_iterator itr = procTriggered.begin(); itr != procTriggered.end(); ++itr)
    {
        ...
        if (useCharges && procSuccess && anyAuraProc && !triggeredByHolder->IsDeleted())
        {
			...
			if (triggeredByHolder->DropAuraCharge())
			{
				...
				if (triggeredByHolder->GetId() == 12970)
				{
					RemoveAurasDueToSpell(triggeredByHolder->GetId());
					removedSpells.remove(triggeredByHolder->GetId());
				}
			}
        }
...
void Unit::SetFeared(bool apply, ObjectGuid casterGuid, uint32 spellID, uint32 time)
{
	if (apply)
	{
		if (GetTypeId() == TYPEID_UNIT)
		{
			...
			if (!HasAuraType(SPELL_AURA_MOD_CONFUSE))
			{
				...
				if (caster && spellID == 5246 && caster->getVictim() == this)
				{
					StopMoving(true);
		...
		if (GetTypeId() == TYPEID_PLAYER)
		{
			...
			if (spellID == 6789 || spellID == 17925 || spellID == 17926)
				PaladinAttacks();
		}
	}
	else
	{
		...
		if (caster && spellID == 5246 && caster->getVictim() == this)
		{
			RemoveAurasDueToSpell(20511);
			if (!HasAuraType(SPELL_AURA_MOD_ROOT) && !HasAuraType(SPELL_AURA_MOD_STUN))
				SetRoot(false);
		}
		...
		if (GetTypeId() == TYPEID_UNIT)
		{
			...
			if (!HasAuraType(SPELL_AURA_MOD_CONFUSE) && !HasAuraType(SPELL_AURA_MOD_FEAR))
			{
				...
				if (isAlive())
				{
					...
					if (!HasAuraType(SPELL_AURA_MOD_CHARM) && !HasAuraType(SPELL_AURA_MOD_POSSESS))
					{
						if (getVictim())
						{
					...
					{
						Unit* p_owner = GetCharmerOrOwner();
						if (p_owner->GetTypeId() == TYPEID_PLAYER)
						{
							c->AIM_Initialize();
							((Player*)p_owner)->SetClientControl(c, 1);
						}
					}
				}
			}
		...
		if (GetTypeId() == TYPEID_PLAYER)
		{
			...
			if (!HasAuraType(SPELL_AURA_MOD_CONFUSE) && !HasAuraType(SPELL_AURA_MOD_FEAR))
			{
				GetMotionMaster()->MovementExpired(false);
				StopMoving(true);
				if (HasAura(24327) || HasAura(24178))
				{
					if (GetCharmerOrOwner()->IsInWorld() && GetCharmerOrOwner()->isAlive() && GetCharmerOrOwner()->GetEntry() == 14834)
						((Player*)this)->SetCharmed(true, GetCharmerOrOwner()->GetObjectGuid(), HasAura(24327) ? 24327 : 24178);
				}
				if (!HasAuraType(SPELL_AURA_MOD_CHARM) && !HasAuraType(SPELL_AURA_MOD_POSSESS))
				...
				if (HasAuraType(SPELL_AURA_MOD_POSSESS))
				{
					...
					if (p_owner->GetTypeId() == TYPEID_PLAYER)
					{
						((Player*)p_owner)->SetClientControl(this, 1);
					}
				}
			}
	...
	else
	{
		if (GetTypeId() == TYPEID_UNIT)
		{
			...
			if (HasAuraType(SPELL_AURA_MOD_FEAR) && !HasAuraType(SPELL_AURA_MOD_CONFUSE))
			{
				...
				SetTargetGuid(ObjectGuid());                    // creature feared loose its target
				Unit* caster = IsInWorld() ? GetMap()->GetUnit(casterGuid) : NULL;
				GetMotionMaster()->MoveFleeing(caster, 50000);       // caster==NULL processed in MoveFleeing
			}
		}
		...
		if (GetTypeId() == TYPEID_PLAYER)
		{
			...
			if (HasAuraType(SPELL_AURA_MOD_FEAR) && !HasAuraType(SPELL_AURA_MOD_CONFUSE))
			{
				...
				CastStop(GetObjectGuid() == casterGuid ? spellID : 0);
				Unit* caster = IsInWorld() ? GetMap()->GetUnit(casterGuid) : NULL;
				GetMotionMaster()->MoveFleeing(caster, 50000);
				((Player*)this)->SetClientControl(this, 0);
			}
		}
	}
        ...
        */
		...
		if (target->GetTypeId() == TYPEID_PLAYER)
		{
			...
			for (Spell::UnitList::iterator iter = targets.begin(); iter != targets.end(); ++iter)
			{
				...
				for (uint32 i = CURRENT_FIRST_NON_MELEE_SPELL; i < CURRENT_MAX_SPELL; i++)
				{
					if ((*iter)->GetCurrentSpell(CurrentSpellTypes(i)) && (*iter)->GetCurrentSpell(CurrentSpellTypes(i))->m_targets.getUnitTargetGuid() == target->GetObjectGuid())
					{
...
void Unit::PaladinAttacks()
{
	if (GetTypeId() == TYPEID_PLAYER && getClass() == CLASS_PALADIN && m_extraAttacks > 0)
		m_extraAttacks = 0;
}
