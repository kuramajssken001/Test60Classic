...
pAuraProcHandler AuraProcHandler[TOTAL_AURAS] =
{
	...
	&Unit::HandleNULLProc,                                  // 192 SPELL_AURA_OFF_PVP_MOD_POWER
};
...
SpellAuraProcResult Unit::HandleProcTriggerSpellAuraProc(Unit* pVictim, uint32 damage, Aura* triggeredByAura, SpellEntry const* procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown)
{
    ...
    switch (auraSpellInfo->SpellFamilyName)
    {
				...
				case 22438:
				{
					...
					if (GetTypeId() == TYPEID_PLAYER)
					{
						...
						for (Spell::UnitList::iterator iter = targets.begin(); iter != targets.end(); ++iter)
						{
							...
							if (!target->IsWithinDist2d((*iter)->GetPositionX(), (*iter)->GetPositionY(), 20.0f))
								continue;

							(*iter)->CastSpell((*iter), trigger_spell_id, true, castItem, triggeredByAura);
						}
						return SPELL_AURA_PROC_OK;
					}
					break;
        ...
        case SPELLFAMILY_PRIEST:
        {
			...
			if (auraSpellInfo->SpellIconID == 1591)
			{
				switch (auraSpellInfo->Id)
				{
				    case 2652:  trigger_spell_id = 2943; break; // Rank 1
					case 19261: trigger_spell_id = 19249; break; // Rank 2
					case 19262: trigger_spell_id = 19251; break; // Rank 3
					case 19264: trigger_spell_id = 19252; break; // Rank 4
					case 19265: trigger_spell_id = 19253; break; // Rank 5
					case 19266: trigger_spell_id = 19254; break; // Rank 6
					case 25461: trigger_spell_id = 25460; break; // Rank 7
				    default:
					    sLog.outError("Unit::HandleProcTriggerSpellAuraProc: Spell %u not handled in SG", auraSpellInfo->Id);
					    return SPELL_AURA_PROC_FAILED;
				}
	...
	if (auraSpellInfo->Id == 18137 ||
		auraSpellInfo->Id == 19308 ||
		auraSpellInfo->Id == 19309 ||
		auraSpellInfo->Id == 19310 ||
		auraSpellInfo->Id == 19311 ||
		auraSpellInfo->Id == 19312 )
	{
		CastSpell(target, trigger_spell_id, true);
	}
	else
	if (auraSpellInfo->Id == 2652 ||
		auraSpellInfo->Id == 19261 ||
		auraSpellInfo->Id == 19262 ||
		auraSpellInfo->Id == 19264 ||
		auraSpellInfo->Id == 19265 ||
		auraSpellInfo->Id == 19266)
	{
		CastSpell(target, trigger_spell_id, true);
		float stun_chance = 0;
		Unit::AuraList const& auraTriggerSpell = GetAurasByType(SPELL_AURA_PROC_TRIGGER_SPELL);
		for (Unit::AuraList::const_iterator itr = auraTriggerSpell.begin(); itr != auraTriggerSpell.end(); ++itr)
		{
			...
			case 15325:
			case 15326:
				stun_chance = (*itr)->GetSpellProto()->procChance;
				break;
			}
		}
		if (roll_chance_f(stun_chance))
			CastSpell(target, 15269, true);
	}
	else
	if (auraSpellInfo->Id == 20375||
		auraSpellInfo->Id == 20915||
		auraSpellInfo->Id == 20918||
		auraSpellInfo->Id == 20919||
		auraSpellInfo->Id == 20920)
		CastSpell(target, trigger_spell_id, true);
	else
...
{
	uint32 damage_sum = 0;
	if (!procSpell || (procSpell->Mechanic == MECHANIC_ROOT || IsAuraAddedBySpell(SPELL_AURA_MOD_ROOT, procSpell->Id)))
		return SPELL_AURA_PROC_FAILED;
	...
	{
		switch (procSpell->EffectApplyAuraName[j])
		{
		    ...
		    case SPELL_AURA_PERIODIC_LEECH:
			    ...
			    break;
		    default:
				chance = float(damage_sum) / max_dmg * 100.0f;
			    break;
		}
	}
	if (roll_chance_f(chance))
